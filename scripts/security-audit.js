...
async function run() {
  ...
  const EXPECTED_PUBLIC = new Set([
    ...
    // ── مراجعة 2026-09-25 ────────────────────────────────────────────────────────────────
    // ADR-0114 — استقبال أخطاء الواجهة. **مسار الكتابة العام الوحيد في المشروع**، ومقبول لسبب
    // واحد: أهم الأخطاء بتحصل قبل الدخول (شاشة اللوجن نفسها، صفحة جاية من جوجل، توكن خلص)،
    // فمسار محمي كان هيسجّل كل حاجة إلا اللي اتعمل عشانه. الضوابط: throttle 30/د لكل IP،
    // DTO مقفول بـ@IsIn + @MaxLength على كل حقل، تطهير المسارات **في السيرفر** كمان مش في
    // العميل بس، الرد 202 فاضي دايمًا (مفيش oracle)، `user_id` من توكن متحقَّق منه وأبداً من
    // الجسم، والمتصفح/الجهاز مشتقّين من هيدر user-agent مش من حقول بيبعتها العميل.
    'modules/ops/client-errors.controller.ts',
  ]);
