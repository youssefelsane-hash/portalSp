...
**`apps/admin`, `apps/customer-app`, `apps/technician-app`, `packages/*`** — **README فقط، صفر كود.** الستاك المخطط: Next.js+shadcn/ui للوحة الإدارة، Flutter للتطبيقين. Flutter SDK مش متاح في البيئات اللي اشتغلنا فيها لحد دلوقتي — تحقق أول حاجة قبل ما تفترض إنه موجود.
