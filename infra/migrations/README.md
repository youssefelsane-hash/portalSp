...
`0183_per_unit_pricing_quantity.sql` يضيف snapshot مستقلة لكمية تسعير الوحدة على الطلب وقالب
الحجز المتكرر، مع قيود إيجابية على مستوى PostgreSQL. لا يُعاد استخدام `requested_units` لأنها
مخصّصة لحساب إنتاجية الطاقم وقد تعبّر عن مقياس مختلف.
`0184_backfill_legacy_per_unit_quantity.sql` يحفظ الدلالة القديمة (وحدة واحدة ضمنيًا) كقيمة `1`
صريحة للطلبات وقوالب التكرار القائمة، فلا تتوقف خطة قديمة عند أول توليد بعد الترقية.
