...
**لسه من غير**: ~~خريطة فعلية للتتبع~~ اتقفلت (تفاصيل فوق). الدفع بالبطاقة عنده تكامل Paymob كامل وجاهز في الباك-إند دلوقتي (`POST /orders/:id/pay-with-card` — تفاصيل `../api/src/modules/payments/README.md`)، لكن لسه مش متوصّل في واجهة `apps/customer-app` (زرار/شاشة WebView تفتح `redirect_url` وترجع لما الدفع يخلص) — فجوة UI متبقية، الباك-إند جاهز 100%.
