...
void assertProductionApiConfig({bool isRelease = kReleaseMode, String url = apiBaseUrl, String? site}) {
  ...
  // نفس الحماية لعنوان الموقع: كل روابط الفوتر والشروط والخصوصية وحذف الحساب بتتبني عليه،
  // وGoogle Play بيطلب نفس الروابط دي في Store Listing.
  // بنشتق من `url` مش من `siteBaseUrl` العام: لما المستدعي يمرّر عنوان API صريح (الاختبارات،
  // وأي بيئة بتحقن العنوان)، لازم عنوان الموقع يتبعه — مش يفضل معلّق على ثابت وقت الترجمة.
  final siteUrl = site ??
