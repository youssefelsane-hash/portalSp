...
ServiceCategory _category(String name, {String? image}) =>
...
CatalogService _service(
  ...
  'icon_url': image,
  'featured_icon_url': featuredImage,
  'featured_name_ar': featuredName,
  'pricing_model': 'fixed',
...
void main() {
  ...
  group('CategoryTile — خانة الفئة المدمجة', () {
    testWidgets('الاسم تحت الصورة مش جوّاها — ده جوهر الشكل الجديد', (
      tester,
    ) async {
      await tester.pumpWidget(
        wrap(
          SizedBox(
            width: 116,
            child: CategoryTile(category: _category('سباكة'), onTap: () {}),
          ),
          width: 116,
        ),
      );
    ...
    testWidgets('الصورة مربّعة — نسبة 1:1 زي المرجع', (tester) async {
      await tester.pumpWidget(
        wrap(
          SizedBox(
            width: 116,
            child: CategoryTile(category: _category('كهرباء'), onTap: () {}),
          ),
          width: 116,
        ),
      );
    ...
    testWidgets('اسم من ست كلمات ياخد ثلاثة أسطر متصنطرة', (tester) async {
      const longName = 'تأسيس كهرباء كامل لشقة سكنية جديدة';
      await tester.pumpWidget(
        wrap(
          SizedBox(
            width: 116,
  ...
  group('HomeHero — مفيش overflow مهما كان المحتوى', () {
    ...
    Widget hero({
      ...
      String? title,
      String trust =
          'ضمان على كل شغلانة — لو في أي عيب بعد التسليم بنرجع نصلحه',
      int imageCount = 0,
      double width = 390,
    }) => MaterialApp(
      theme: AppTheme.light(),
    ...
    testWidgets('تكبير خط النظام 1.6× (إتاحة): لسه مفيش overflow', (
      tester,
    ) async {
      expect(await renderAndCatch(tester, hero(textScale: 1.6)), isNull);
    });

    testWidgets('عنوان طويل جدًا من لوحة الأدمن: اللوحة بتكبر مش بتتقص', (
      tester,
    ) async {
      expect(
        await renderAndCatch(
          tester,
          hero(
            title:
                'محتاج مساعدة في إيه النهارده في بيتك أو شقتك أو مكتبك؟ إحنا معاك',
          ),
        ),
        isNull,
      );
    ...
    testWidgets('شاشة ضيقة (320px) + نقط شرائح: مفيش overflow', (tester) async {
      expect(
        await renderAndCatch(tester, hero(width: 320, imageCount: 3)),
        isNull,
      );
    });
    ...
    for (final entry in {
      'الداكن': AppTheme.dark(),
      'الفاتح': AppTheme.light(),
    }.entries) {
      testWidgets('الوضع ${entry.key}: مفيش تعبئة موروثة جوّه كبسولة البحث', (
        tester,
      ) async {
        await tester.pumpWidget(
          MaterialApp(
        ...
        expect(
          ...
          isFalse,
          reason:
              'التعبئة الموروثة من الثيم بترسم مستطيل غامق جوّه الكبسولة البيضا',
        );
      });
    }
  });
  ...
  group('FeaturedServiceItem — الأكثر طلبًا', () {
    test('يستخدم شعار واسم الأكثر طلبًا بدل صورة واسم صفحة الخدمة', () {
      final service = _service(
        'تركيب سخان كهربائي كامل بالمهمة',
        image: 'https://example.test/service-banner.png',
        featuredImage: 'https://example.test/featured-logo.png',
        featuredName: 'تركيب سخان',
      );

      expect(
        service.featuredCardIconUrl,
        'https://example.test/featured-logo.png',
      );
      expect(service.featuredCardName, 'تركيب سخان');
    });

    test('الخدمات القديمة ترجع تلقائيًا لصورة واسم الخدمة', () {
      final service = _service(
        'تنظيف سجاد',
        image: 'https://example.test/service.png',
      );

      expect(service.featuredCardIconUrl, 'https://example.test/service.png');
      expect(service.featuredCardName, 'تنظيف سجاد');
    });

    Widget row(
      BuildContext Function()? capture, {
      double textScale = 1,
      String name = 'تصليح حنفية',
    }) => MaterialApp(
      theme: AppTheme.light(),
      home: MediaQuery(
        data: MediaQueryData(textScaler: TextScaler.linear(textScale)),
    ...
    testWidgets(
      'من غير أيقونة: حرف أول الاسم بيفضل ليه دايرة عشان يبان إنه زرار',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            theme: AppTheme.light(),
    ...
    testWidgets('الارتفاع المحجوز بيكبر مع خط النظام — مفيش overflow', (
      tester,
    ) async {
      await tester.pumpWidget(
        row(null, textScale: 1.6, name: 'تركيب خلاط مطبخ'),
      );
      await tester.pumpAndSettle();
      expect(tester.takeException(), isNull);
    });
  ...
  testWidgets('طباعة لقطة للشبكة (اختياري)', (tester) async {
                    ...
                    builder: (context, constraints) {
                      ...
                      return GridView.builder(
                        ...
                        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                          ...
                          childAspectRatio:
                              tileWidth /
                              (tileWidth + categoryTileLabelHeight(context)),
                        ),
                        itemCount: categories.length,
                        itemBuilder: (context, i) =>
                            CategoryTile(category: categories[i], onTap: () {}),
                      );
                    },
                  ),
                ],
    ...
    await expectLater(
      ...
      matchesGoldenFile('home_redesign_preview.png'),
    );
  });
  ...
  testWidgets('طباعة لقطة للـhero في الوضع الداكن (اختياري)', (tester) async {
    ...
    await expectLater(
      find.byType(MaterialApp),
      matchesGoldenFile('hero_dark_mode.png'),
    );
  });
}
