...
export default () => ({
  ...
  database: {
    ...
    // `acquireTimeoutMs` هو اللي اتضاف وقتها: الافتراضي كان انتظارًا بلا نهاية.
    //
    // **`poolMax` كان ١٠ وده كان بيرفض عملاء حقيقيين** (قياس حي، `scripts/concurrency-booking
    // -safety.js`، ٤٠ حجز متزامن على نفس البيانات ونفس الكود):
    //
    // | poolMax | طلبات اتعملت | عملاء اترفضوا بـ503 | زمن الدفعة |
    // |---|---|---|---|
    // | ١٠ | ١٢ | **٢٨ (٧٠٪)** | ٢١.٥ ثانية |
    // | ٢٥ | ٤٠ | صفر | ١٢.٦ ثانية |
    //
    // الرقم ١٠ اتساب عمدًا وقت مطاردة قفلة الـpool (تدقيق `docs/29` P0-1) بحجة إن «تكبير الـpool
    // بيداري الاستنزاف مش بيمنعه» — وده كان صح **وقتها**، لأن القفلة كانت لسه موجودة وتكبير
    // الـpool كان هيأخّر ظهورها بس. القفلة اتصلحت من جذرها، فالحجة دي خلص دورها، والرقم فضل
    // سقفًا حقيقيًا على عدد العملاء المتزامنين.
    //
    // **قاعدة ضبط الرقم** (مش تخمين): كل نسخة من التطبيق بتاخد `poolMax` اتصال من نفس
    // `max_connections` بتاع Postgres. يعني:
    //
    //     poolMax ≤ (max_connections − ١٠ محجوزة للصيانة/الـmigrations) ÷ عدد نسخ التطبيق
    //
    // ٢٠ هو الافتراضي الجديد لأنه بيسع نسختين على Postgres بـ٥٠ اتصال، أو أربع نسخ على ١٠٠
    // (الافتراضي في أغلب المزوّدين). `DbPoolMonitorService` بيطبع الرقمين الحقيقيين وقت الإقلاع
    // وبيحذّر لو النسبة خطرة، فمفيش تخمين مطلوب من اللي بينشر.
