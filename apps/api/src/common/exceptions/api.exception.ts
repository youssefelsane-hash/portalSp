...
export const ErrorCode = {
  ...
  AUTH_005: 'AUTH_005', // محتاج تحقق MFA/Passkey تاني قبل ما تكمل (ADR-0011)
  AUTH_006: 'AUTH_006', // محتاج step-up verification حديث للعملية الحساسة دي (ADR-0011)
  ...
  ORDR_005: 'ORDR_005', // لازم صورة بعد التنفيذ قبل إنهاء الشغل (docs/08 §20 بند 12)
  ORDR_006: 'ORDR_006', // وصلت للحد الأقصى لمرات استكمال الشغل في يوم تاني (ADR-0047)
  ...
  TECH_003: 'TECH_003', // الرقم القومي مسجّل بالفعل لحساب فني تاني (ADR-0045)
  ...
  // إيقاف **مقصود** ومؤقت لخدمة بعينها بقرار أدمن (مفاتيح الطوارئ، ج-١٧/ج-١٨) — مش عطل.
  // منفصل عن SYS_001 عمدًا: SYS_001 بيتولّد من `all-exceptions.filter` لكل خطأ **غير متوقع**،
  // فلو الاتنين نفس الكود يبقى ارتفاع الإيقاف المتعمّد بيبان في المراقبة كأنه حادثة (والعكس:
  // حادثة حقيقية بتضيع وسط إيقاف مخطط له).
  SYS_002: 'SYS_002', // الخدمة موقوفة مؤقتًا بقرار إداري؛ إعادة المحاولة لاحقًا آمنة
} as const;
