...
## الاختبارات (`favorites.service.spec.ts` — ١٢ حي، تدقيق-9)

تصرّفان لو اتغيّروا بالسهو بيكسروا التجربة من غير ما يفشل أي حاجة: `addFavorite` **idempotent بالتصميم** (الضغط مرتين أو ريكوست مكرّر مش خطأ — من غير الفحص ده القيد الفريد بيرمي 500 في الإنتاج بس)، و`listFavorites` استعلام SQL خام بـ`JOIN` على جدولين وترتيب `created_at DESC` — نص مابيتحققش منه `tsc`، والاختبار الحي هو الحاجة الوحيدة اللي بتمسك أي تعديل فيه.
