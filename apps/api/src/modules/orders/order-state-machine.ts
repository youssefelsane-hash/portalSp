...
export const ORDER_TRANSITIONS: Record<OrderStatus, OrderStatus[]> = {
  ...
  [OrderStatus.SEARCHING_TECHNICIAN]: [
    ...
    // ADR-0066 §2 — نفس الحالة بس لطلب اتقفل من مسار عرض السعر: بيرجع لقايمة مرشّحي نفس
    // العرض، مش لمعاينة حجز جديدة (الحجز اتعمل من زمان).
...
/**
 * الحالات اللي **الفني لازم يشوف الطلب فيها في تطبيقه** رغم إنه مش «مشغول» بيها.
 *
 * ## البَقّة اللي وراها (بلاغ المالك 2026-09-11)
 *
 * «بيّن عند الأدمن إن الطلب راح للفني. أفتح أكاونت الفني ألاقي إن فعليًا مفيش حاجة جاية —
 * الطلب مش ظاهر أصلًا على صفحة الفني».
 *
 * `technician-order-visibility-audit.js` مشى على **كل** حالة في `order_status` بنسختين
 * (مجدول/غير مجدول) وسأل التلات مسارات اللي التطبيق بيقرا منهم. النتيجة: **٨ تركيبات** الطلب
 * فيها متعيّن لفني ومش راجع في ولا مسار:
 *
 * | الحالة | إيه اللي ضايع |
 * |---|---|
 * | `technician_assigned` | شغل اتعيّن له ومحدش عرّفه — بلاغ المالك بالحرف |
 * | `work_completed` | خلّص الشغل والطلب اختفى من تطبيقه |
 * | `awaiting_payment` | **عنده كاش يحصّله** والطلب مش ظاهر |
 * | `disputed` | نزاع مرفوع عليه ومش شايفه أصلاً |
 *
 * ## ليه مجموعة جديدة مش إضافة على `ACTIVE_TECHNICIAN_ORDER_STATUSES`؟
 *
 * لأن المجموعة دي **مش عن الرؤية** — هي تعريف «الفني مشغول» ومستخدمة في ١٥ موديول: أهلية
 * المطابقة، سعة اليوم، التتبّع اللحظي، حذف الحساب، توقّع الحِمل. إضافة `awaiting_payment`
 * ليها معناها إن فني خلّص شغله وبيستنى تحصيل **يبقى غير مؤهّل لأي شغل جديد** — كارثة تشغيلية
 * أسوأ بكتير من البَقّة نفسها. الرؤية حاجة والانشغال حاجة تانية، والخلط بينهم هو اللي وقع.
 */
export const TECHNICIAN_ATTENTION_ORDER_STATUSES: OrderStatus[] = [
  // اتعيّن له ولسه ما قبلش — محتاج يشوفه عشان يقبل أصلاً.
  OrderStatus.TECHNICIAN_ASSIGNED,
];

/**
 * حالات **بعد ما الشغل خلص** والفني لسه له فيها مصلحة قايمة (تحصيل كاش، رد على نزاع).
 *
 * مفصولة عن `TECHNICIAN_ATTENTION_ORDER_STATUSES` فوق لأن الجدولة **مالهاش أي معنى** هنا:
 * الموعد عدّى خلاص، فالطلب لازم يظهر سواء كان مجدول أو لأ.
 */
export const TECHNICIAN_POST_WORK_ORDER_STATUSES: OrderStatus[] = [
  OrderStatus.WORK_COMPLETED,
