...
describe('OrderAutoCancelService — PENDING_PAYMENT sweep + SEARCHING_TECHNICIAN مايتلغيش تلقائيًا (docs/08 §19 بند 3، قرار 2026-08-19)', () => {
  ...
  async function insertOrder(opts: {
    ...
    const [order] = await q(
      `INSERT INTO orders (commission_rate_applied,order_number, customer_id, service_id, address_id, service_zone_id, order_status, payment_status, total_amount_cents, technician_earning_cents, placed_at)
       VALUES (20,$1,$2,$3,$4,$5,$6,$7,$8,0, now() - ($9 || ' minutes')::interval) RETURNING id, order_number`,
      [
