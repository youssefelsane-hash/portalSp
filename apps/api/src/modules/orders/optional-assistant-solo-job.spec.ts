...
describe('ADR-0052 — مساعد اختياري واحد للشغلانة الفردية', () => {
  ...
  it('ضم فني (مش مساعد) لشغلانة فردية لسه ممنوع', async () => {
    // الرفض بقى بيتقاس على **خانة الطاقم المفتوحة** مش على وضع الحجز (`assertCrewSlotOpen`):
    // شغلانة فردية مطلوب فيها فني واحد والقائد متعيّن، فمفيش خانة فني مفتوحة. الرسالة القديمة
    // («اعتماد») كانت بتتكلم عن وضع الحجز، وهو مش المسطرة دلوقتي — نفس المنع، سبب أدق.
    await expect(
      orderTeamService.recruitMember(ids.leaderUser, ids.soloOrder, ids.technicianProfile, 'technician'),
    ).rejects.toThrow(/عدد الفنيين المطلوب مكتمل بالفعل/);
  }, 20000);
