...
export class CreateOrderDto {
   ...
   *
   * > «لما يكون الطلب فيه deposit، يكون فيه option برضه إن الشخص يدفع الطلب كامل… يعني لما
   * >  بيدوس إن هو عايز يدفع يختار deposit ولا يدفع كامل.»
   *
   * القرار بيتاخد **وقت إنشاء الطلب** ومابيتخزّنش كعَلَم تاني: لو العميل اختار الدفع الكامل،
   * `deposit_amount_cents` مابيتحطّش على الصف أصلاً. كل المسار المالي تحته (`amountOwedNow`،
   * التحصيل، الاسترداد) بيشتغل زي أي طلب بلا عربون بالظبط — صفر فرع جديد في كود الفلوس.
   *
   * مالهوش أي أثر لو الخدمة أصلاً مافيهاش عربون.
   */
  @IsOptional()
  @IsBoolean()
