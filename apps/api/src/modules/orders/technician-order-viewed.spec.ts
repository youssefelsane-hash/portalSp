...
describe('OrdersService.markViewedByTechnician() (docs/08 §56 بند 2)', () => {
  ...
  beforeAll(async () => {
    ...
    const [service_] = await q(`INSERT INTO services (category_id, name_ar, slug, pricing_model, base_price_cents, commission_percentage, warranty_days) VALUES ($1,$2,$3,'fixed',30000,20,0) RETURNING id`, [category.id, `خدمة VIEW ${runId}`, `svc-view-${runId}`]);
