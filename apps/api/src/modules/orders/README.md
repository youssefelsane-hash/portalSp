...
- **`OrderTrackingGateway`** (S6, WebSocket namespace `/tracking`): الاتصال والانضمام يعيدان فحص المستخدم الحي والملكية وحالة الطلب. الحالات هي المصدر المشترك `ACTIVE_TECHNICIAN_ORDER_STATUSES` (وتشمل `awaiting_quote_approval`)؛ حالة منتهية ترفض join وتزيل الغرفة. location DTO يرفض string/NaN/Infinity/خارج النطاق/حقول زائدة، مع حد 10 تحديثات/10 ثوانٍ لكل socket. `tracking:joined` يعيد الحالة و`state_version` authoritative، وكل location يحمل `observed_at`.
  - **مصدرا الحقيقة اللي الـgateway بيسأل منهم (تدقيق L-4/L-5، مش نسخة محلية جوّاه)**:
    - **مين يقدر ينضم؟** `OrdersService.isTechnicianAssignedToOrder()` — **القائد (`orders.technician_id`) أو أي عضو في `order_team_members`**. قبل كده كان الشرط `order.technicianId === profile.id` بس، فباقي طاقم طلب الفريق كان بياخد «الطلب ده مش بتاعك» على طلب هو شغّال عليه فعلاً.
    - **البث بيروح لمين؟** `OrdersService.findOrdersInTransitForTechnician()` — **`technician_on_way` بس**، بترتيب حتمي (`scheduled_at ASC, id ASC`). سببان: (1) التتبّع ليه معنى واحد «الفني جاي، هو فين» — بعد `technician_arrived`/`in_progress` الفني عند/جوّه بيت العميل، فبث إحداثياته تسريب خصوصية بلا فايدة؛ (2) بعد ADR-0070 الفني بقى يقدر يمسك أكتر من طلب نشط في نفس اليوم، والكود القديم كان `findOne` **بلا `ORDER BY`** فالصف اللي بيترجع كان بيعتمد على خطة تنفيذ Postgres — يعني البث كان ممكن يروح لعميل الطلب الغلط.
    - **`apps/customer-app` متسّق مع القاعدة دي**: زرار «تتبّع الفني لحظياً» (`order_detail_screen.dart`, `_liveTrackingStatuses`) بيظهر في `technician_on_way` بس. كان بيظهر في أربع حالات، والتلاتة التانية مافيش فيهم أي إحداثيات — فالشاشة كانت بتفضل على «في انتظار تحديث موقع الفني» للأبد.
    - التغطية الحيّة: `order-tracking-crew-and-transit.spec.ts` (٨ اختبارات على Postgres حقيقي).
...
## `orders.assigned_company_id` — مساحة عمل الشركة (ADR-0033، 2026-08-22)

عمود جديد (migration 0173)، **مختلف تمامًا** عن `requested_technician_company_id` فوق: ده snapshot
النتيجة الفعلية (الشركة اللي الفني المُعيَّن فعلاً عضو فيها وقت التعيين)، مش تفضيل العميل الأولي.
بيتسجّل في `MatchingService` (مكانين — التأكيد التلقائي والقبول الصريح، تفاصيل كاملة في
`../technicians/README.md` قسم "مساحة عمل الشركة")، وبعد كده **مش بيتغيّر أبدًا** حتى لو الفني سايب
الشركة. `null` لأي طلب فني مستقل. مُستهلك في `GET /technician/company/orders`/`GET /admin/technician-companies/:id/orders`
(`technicians` module) — القرار الكامل والبدائل المرفوضة في `docs/adr/0033-company-workspace-orders.md`.
...
- **"إعادة زيارة"** — `POST /orders` بقى ياخد `original_order_id` اختياري (بيتترجم داخليًا لعمود `parent_order_id` الموجود، الاسم في الـ API أوضح دلالياً للعميل). لازم: بتاعة نفس العميل، `order_status=completed`، **نفس `service_id` ونفس `address_id` بالظبط**، و`warranty_expires_at` لسه في المستقبل — أي واحدة من دول غلط بترمي `400`/`404` واضح. `order_type` بيتفرض `revisit` تلقائيًا (بيتجاهل `dto.order_type`)، و`requested_technician_id` بيتفرض لنفس فني الطلب الأصلي. السيرفر يختار أول ساعة متاحة حقيقية للفني بعد نافذة الـ48 ساعة، بنفس بوابة التداخل والسعة اليومية والحظر المستخدمة في المطابقة، ثم يؤكدها تلقائيًا للفني الأصلي ويبلغ العميل بالتاريخ. لو بيانات فني تاريخية ناقصة فقط يبقى موعد الثلاثة أيام شبكة أمان بدل إسقاط حق الضمان. **مجانية بالكامل** (`estimated/inspection/total = 0`) — كود خصم أو إضافات كتالوج مع إعادة زيارة بيترفضوا بوضوح (`400`) بدل ما يتقبلوا ويعملوا حسابات غريبة على إجمالي صفر.
...
**تحديث Script 2 (2026-08-25) — طلب تأجيل من الفني بدل تغيير صامت**: الفني يقدر يقترح سلوت جديد
مع سبب عبر `POST /technician/orders/:id/reschedule-requests`. الطلب يفضل `pending` والسلوت الحالي
يفضل محجوزًا كما هو حتى يوافق العميل أو يرفض من
`POST /orders/:id/reschedule-requests/:requestId/{approve|reject}`. الموافقة وحدها تعيد استخدام
`TechnicianScheduleService.rescheduleSlot()` داخل نفس transaction مع قفل الطلب والطلب المقترح؛
ضغطتان متزامنتان تنفذان تغييرًا واحدًا فقط. الحد الأقصى مصدره إعداد الأدمن
`orders.technician_reschedule_max_requests` (الافتراضي 2)، ويحسب الطلبات المحسومة أيضًا حتى لا يتحول
الرفض إلى حلقة إزعاج بلا نهاية. إنشاء الطلب والقرار يكتبان إشعار `in_app` دائمًا داخل نفس المعاملة؛
فشل push لا يستطيع إخفاء طلب التأجيل أو نتيجته. أي إعادة جدولة مباشرة من العميل/الأدمن تلغي تلقائيًا
الطلب المعلّق القديم حتى لا يظل قرارًا قابلًا للتنفيذ بعد تغيّر الحقيقة.

اختبار PostgreSQL الحقيقي في `reschedule-and-address-warning.spec.ts` يغطي حفظ الإشعار مع الطلب،
الرفض بلا تغيير للسلوت، موافقتين متزامنتين (واحدة فقط تنجح وسطر تاريخ واحد)، والحد الإداري.

**اتصال الطرفين بعد تأكيد الحجز (Script 2، 2026-08-25)**: نفس مجموعة الحالات المشتركة
`TECHNICIAN_CONTACT_VISIBLE_STATUSES` تحكم الآن ظهور رقم الفني للعميل ورقم العميل للفني. يبدأ
الظهور من `accepted` وليس `technician_assigned`، ويستمر أثناء التنفيذ/الدفع؛ لذلك قوائم الفرص
والتعيين غير المقبول لا تسرّب رقم العميل. مسارات الفني تحصل على البيانات فقط بعد نجاح فحص ملكية
الطلب أو عضوية فريقه (`findVisibleForTechnician`). تطبيق الفني يعرض بطاقة اتصال مباشرة بالاسم
والرقم. `technician-order-contact-visibility.spec.ts` يثبت الظهور بعد القبول وعدم حتى استعلام الرقم
قبله؛ اختبارات BOLA الموجودة تظل تغطي رفض الفني الغريب.
...
## الحجز المتكرر كقدرة رسمية + "كرّر الحجز ده" من مسار الحجز العادي (migration 0176، 2026-08-24)

الهدف: التكرار مجرد **جدولة بتطلّب طلبات عادية** — مفيش نظام طلبات/دفع/توزيع موازي خالص. البنية
كانت موجودة من 0064/0124 (قالب → `OrdersService.create()` بنفس التسعير/الدفع/التوزيع بالحرف)،
والشريحة دي قفلت الفجوات المتبقية:

1. **قدرة لكل خدمة**: `services.allows_recurring_booking` (افتراضي false — صفر تغيير سلوك).
   false يعني: مفيش خيارات تكرار في واجهة العميل، و`POST /me/recurring-orders` بيرفض بوضوح،
   و`repeat_frequency` في `POST /orders` مرفوض. بتتحكم فيها الأدمن من فورم الخدمة.
2. **"كرّر الحجز ده" وقت الحجز**: `CreateOrderDto.repeat_frequency` (weekly/monthly/yearly) —
   الطلب الحالي بيتعمل بالمسار الطبيعي زي زمان، والقالب بيتإنشاء جوّه **نفس** transaction الطلب
   (ذرّي: الاتنين أو ولا حاجة) بأول موعد = الموعد المحجوز + دورة (`nextOccurrence()` في
   `recurring-schedule.util.ts` — منطق واحد مشترك). مرفوض للطوارئ/إعادة الزيارة/بلا موعد محدد.
3. **القالب بقى يحمل المدخلات الكاملة** (`field_values`/`duration_hours`/`scheduled_end_at`/
   `requested_technician_company_id`) — بدونهم أي قالب لخدمة formula بحقول إجبارية أو خدمة
   بتوقيت دقيق كان يفشل عند التوليد للأبد. المدخلات دي **مش سعر مجمد**: كل نوبة بتتسعّر بمحرك
   التسعير الحي وقت توليدها (مثبت باختبار تكامل: تغيير سعر الخدمة بيأثر على الجديد بس).
   promo/building codes والإضافات مش بتتخزن عمدًا — خصومات/إضافات لمرة واحدة.
4. **عميل متبلوك/محذوف بيوقف التوليد** بنفس سياسة retry/dead-letter المرئية (كان بينتج طلبات
   لعميل ممنوع بصمت — الفحص كان موجود بس في طبقة JWT اللي التوليد الداخلي بيتجاوزها).
5. **إشعار انتظار الدفع**: طلب متولّد PENDING_PAYMENT (كارت/InstaPay) ماكنش بيوصله أي إشعار
   أصلًا (ORDER_CREATED_EVENT مش بيتصدّر قبل الدفع عمدًا) — `recurring_order_awaiting_payment`
   جديد بيبلّغ العميل فورًا بدل ما يلاقي طلبه اتلغى بصمت بعد مهلة الدفع.

### قرارات معمارية موثقة

- **الحجوزات المتولدة = طلبات عادية 100%**: بتظهر في `/admin/orders` بفلتر `recurring=true|false`
  (جديد على ListOrdersQueryDto)، وتفاصيلها فيها `recurring_template_id`/`recurring_occurrence_at`
  للتتبع. صفحة `/admin/recurring-orders` بقت "خطط" (تعريف التكرار: عميل/تليفون/خدمة/عنوان/
  التالي/آخر حجز/حالة الفشل) بعد ما كانت بتعرض UUIDs خام.
- **idempotency التوليد**: زي ما هي — `uq_orders_recurring_occurrence` + claims بـSKIP LOCKED
  (0124). `sweep()` اخد فلترة اختيارية `templateIds` للاختبارات الحية فقط (الإنتاج بيفلتر كل
  الجدول زي ما هو)، وكل specs التكرار بقت مقفولة بنفس الـadvisory lock (71_208_019) عشان workers
  متوازية ما تسرقش نوبات بعض.
- **شهري 29/30/31**: clamp لآخر يوم فعلي في الشهر (نفس سلوك 0064 المصحح، اتنقل لـutil مشترك).

### خارج نطاق الشريحة دي (قرارات عمل محتاجة)

- مهلة الدفع 15 دقيقة (`orders.payment_timeout_minutes`) بتطبق على المتولد تلقائيًا زي أي طلب —
  لو الأعمال عايزة نافذة أطول للمتولدات (العميل ممكن ميكونش قدام التطبيق لحظة التوليد) ده قرار
  منفصل. InstaPay نفسه فيه تعارض موثق أصلًا (نافذة تأكيد 24 ساعة مقابل sweep الـ15 دقيقة).
- الشحن التلقائي بكارت محفوظ (Paymob tokenization) مش مستخدم عمدًا — الطلبات المتولدة بـ
  payment_method=card بتروح PENDING_PAYMENT عادية لحد ما الشحن التلقائي يتأكد أمنيًا كآلية.

## البحث برقم الطلب في قائمة الأدمن (docs/08 §67، طلب مالك صريح 2026-08-26)

`GET /admin/orders` بقى بياخد `search` اختياري (`MaxLength(64)`) بيدوّر على `order_number` بس:

```sql
o.order_number ILIKE :search ESCAPE '\'
```

**التهريب مش تفصيلة تجميلية**: من غير `ESCAPE` + تهريب `% _ \` في الكود، مستخدم بيكتب `%`
بيرجّع **كل** الطلبات، و`_` بترجّع مطابقات عشوائية. دلوقتي `%` بتتعامل كحرف عادي.

الفلتر بينضاف لباقي الفلاتر بـ`andWhere` فبيتراكب معاها (حالة + تاريخ + بحث مع بعض)، والترتيب
الحتمي الموجود ما اتغيّرش. غياب `search` = صفر تغيير في السلوك القديم.

البحث **مقصور على رقم الطلب** — التوسيع لاسم/تليفون العميل محتاج JOIN على `users` وقرار خصوصية
منفصل (مين من الموظفين يشوف تليفونات وهو بيدوّر)، فاتأجّل لحد ما يتطلب صراحةً.

اختبار حي: `admin-orders-search.spec.ts` (6 حالات، منها `%` كحرف عادي وصفر نتائج بترجّع قائمة
فاضية مش error).

## نوع عضو الطاقم من الأدمن — `role_label` مش `member_type` (docs/08 §70، بلاغ مالك 2026-08-27)

عمودين مختلفين تمامًا في `order_team_members`، وكان بينهم خلط في مسار الأدمن:

- `role_label` — **نص حر للعرض بس** ("سباك مساعد"). صفر أثر على أي حسبة.
- `member_type` — `team_member`/`assistant`. **ده** اللي `computeCrewComposition()` بتعدّ منه.

`addCrewMember()` كانت بتكتب `role_label` بس، فالصف بينزل بافتراضي الداتابيز (`team_member`)
مهما كان الدور المكتوب. النتيجة: الأدمن يكتب "مساعد" والنظام يعدّه فني ⇒ `missingAssistants`
تفضل موجودة و«الطاقم ناقص» تفضل مزمّرة مهما ضاف، **من غير أي طريقة يسدّها**.

دلوقتي `AddCrewMemberDto.member_type` (اختياري، افتراضي `team_member` عشان صفر انحدار)، والقيمة
بتتكتب فعليًا وبتتسجّل في الـaudit log.

**بَقّة تانية في نفس العائلة اتلقطت بالمسح**: `replaceCrewMember()` كانت بتنشئ البديل بلا
`memberType`، فاستبدال مساعد كان بيرجّعه `team_member` ويقلب الحسبة. بقى بيتوارث من العضو المُستبدل.

## `crew_status` في ردود أفعال الفني (docs/08 §70)

`crew_status` كانت بتتحسب في `toDtoWithTeamInfo()` بس (`getOne`/`team-assigned`)، وكل الأفعال
التنفيذية بترجّع `toDto()` الأساسية. تطبيق الفني بيحطّ رد الفعل مكان الطلب الحالي ⇒ كارت
"الطاقم ناقص" وأزرار الضم **بيختفوا بعد أول فعل** لحد ما الشاشة تتقفل وتترجع.

`toDtoAfterAction()` الجديدة بتحلّها: طلب فريق ⇒ `toDtoWithTeamInfo`، غير كده ⇒ `toDto` زي ما هي.
الاستعلام الزيادة لطلبات الفريق بس، فالتحفّظ الأصلي على أداء المسار الساخن لسه محترم.

## `orders.customer_inputs` — إجابات العميل كـsnapshot معروض (docs/08 §71، migration 0201)

إجابات الفورم الديناميكي كانت بتتخزن في `service_pricing_evaluations.field_values` **لخدمات
`formula` بس**، فأي خدمة تانية عندها حقول ديناميكية كانت إجابات العميل بتضيع بعد حساب السعر —
مفيش أي مكان يعرضها للأدمن ولا للفني.

`OrdersService.buildCustomerInputsSnapshot()` بتبني المصفوفة وقت الحجز بتسميات عربية **محلولة**:

- `dropdown`/`multi_select`: القيمة بتتحوّل لـ`label_ar` بتاع الاختيار (مش الكود الخام).
- `checkbox`: «نعم»/«لأ».
- حقل اتمسح من الخدمة بعد الحجز: بيتعرض بمفتاحه بدل ما يختفي.
- أي فشل ⇒ `null` + تحذير في اللوج. **سطر عرض ما ينفعش يكسر إنشاء طلب حقيقي.**

ليه التسميات بتتحل وقت الحجز مش وقت العرض؟ الأدمن ممكن يعيد تسمية/يمسح الحقل بعدين، والطلب لازم
يفضل يقول اللي العميل شافه واختاره وقتها — نفس فلسفة الـsnapshot في `order_earning_shares` وبيانات
الإنتاجية المحسوبة. وكمان العرض في 3 واجهات بصفر استعلامات إضافية.
...
مبني للـsupport_chat بس).

عرض `GET /admin/orders/:id/earning-shares` اتوصل بالملخص المالي في صفحة تفاصيل الطلب: الأدمن
بيشوف اسم كل مشارك ودوره ومستواه ووزنه الفعلي ومستحقه، مع إجمالي التوزيع وحالة واضحة قبل إنشاء
الحصص. المسار والواجهة إداريان فقط؛ تطبيق الفني لا يستدعي المسار ولا يعرض حصص زملائه، وكل فني
يظل يرى مستحقه الشخصي فقط من عقد تنفيذ الطلب الخاص به.
...
## زيارات متعددة لطلب واحد — `order_work_sessions` (ADR-0047، docs/08 §77-D1)

الطلب كان دايمًا **زيارة واحدة** في النموذج: `scheduled_at` واحد و`started_at` واحد. البند ده
كسر الافتراض لسبب واقعي: الفني بيكتشف وسط الشغل إنه محتاج قطعة غيار نادرة.

**مش `order_reschedule_requests` (migration 0185)**: دي بتنقل زيارة **لسه ما حصلتش** بموافقة
العميل. هنا الشغل بدأ فعلاً والفني بيبلّغ — العميل بيتخطر مش بيوافق.

**تفصيلة تصميمية مهمة**: `continueWorkAnotherDay()` بتحدّث `orders.scheduled_at` لليوم الجديد،
وده **بيحجز طاقة الفني في اليوم ده تلقائيًا** عبر الآلية القائمة — منطق تعارض الأهلية في
`technician-eligibility.sql.ts` بيقارن أيام الطلبات النشطة بـ`scheduled_at`، و`in_progress`
موجودة في `ACTIVE_TECHNICIAN_ORDER_STATUSES` و`ENGAGED_TECHNICIAN_ORDER_STATUSES` الاتنين.
**ما تضيفش منطق حجز موازي** — هيبقى مصدر حقيقة تاني لازم يفضل متزامن مع الأول للأبد.

**درس في اختبارات الـfixtures**: أي fixture بيعمل عميل لازم ينضّف `notification_campaign_sends`
و`customer_service_intents` قبل ما يمسح المستخدم — محرك الحملات (ADR-0046) بيمشي في الخلفية
وممكن يكون سجّل إرسال لعميل الاختبار أثناء ما الـsuite شغّالة. حصل فعلاً وكسر الحذف بـFK.

## وضع الحجز بقى **مشتق** مش مختار (ADR-0048، docs/08 §85، 2026-08-28)

`dto.booking_mode` **متجاهَل تمامًا** في `create()` و`previewPrice()`. الاشتقاق في
`booking-mode-resolver.ts` (دوال نقية، صفر اعتماديات):

| المدخل | الناتج |
|--------|--------|
| اليوم المختار = النهارده (بتوقيت القاهرة) | `emergency` + رسوم استعجال |
| بكرة فأكتر + الشغل محتاج أكتر من فرد | `team` |
| بكرة فأكتر + فرد واحد | `individual` |
| **مفيش تاريخ خالص** | **مش مستعجل** — شوف تحت |

**مرحلتين مقصودتين**: الاستعجال بيتحسب **قبل** التسعير (لأنه مدخل لرسوم الطوارئ)، والحجم **بعده**
(لأنه مابيأثرش على السعر). دي مش تفصيلة تنفيذ — دي اللي بتمنع دورة "لازم أسعّر عشان أعرف الوضع،
ولازم أعرف الوضع عشان أسعّر".

**«بلا تاريخ = مش مستعجل» قرار اتصحح بعد ما الاختبار الحي كشفه**: المنطق الأول كان "بلا تاريخ =
دلوقتي = طوارئ"، والسويت الكاملة أثبتت إنه بيحصّل رسوم من قنوات مابتبعتش تاريخ (مركز اتصال، أدمن)
**ومحدش وراها شاف تنبيه الرسوم**، وكمان بوابة `allows_emergency` كانت بترفض طلبات مالهاش علاقة
بنفس اليوم. الشرح الكامل في الدالة نفسها.

**السلوت المحجوز بيلغي الاستعجال** حتى لو في نفس اليوم — فني بعينه التزم بوقت محدد، وده تعيين
مؤكّد مش بث طوارئ.

**فحوصات اتشالت عمدًا** (بقت بلا معنى بعد الاشتقاق، مش سهو):
- «طوارئ + موعد مستقبلي يترفض» — بقت حالة مستحيلة بالبناء.
- «اختيار شركة متاح بس لوضع اعتماد» — ربط تفضيل العميل بناتج محسوب كان بيرفض اختياره لأن التقدير
  طلع "فرد واحد".
- «سلوت مش متاح لطلبات الطوارئ» — العلاقة اتقلبت: السلوت هو اللي بيلغي الاستعجال.
...
| الفلترة | أهلية كاملة (`listForServiceBooking`) | **مفيش** — (كل اللي له علاقة بالطلب) ∪ (معتمدي مدينة الطلب) |
...
### بَقّة حقيقية (بلاغ مالك 2026-09-19، docs/08 §167): المفتّش مكانش بيعرف يفسّر **صاحب الطلب**

`explain-candidates` كانت «كل معتمد في **مدينة** الطلب» وبس. وده جواب على سؤال تاني خالص عن
اللي الأدمن بيسأله. فالشخص اللي شايل الطلب كان بيختفي لو غطّيته للمدينة اتلغت بعد التعيين، أو
اتعيّن من مدينة تانية باستثناء إداري، أو اعتماده اتسحب — وكمان الطلب بلا `service_zone_id` كان
بيرمي **400** والواجهة بتبلعه فالقايمة تبان فاضية بلا سبب.

**القياس على قاعدة التطوير وقت الإصلاح: ١٣ من ٣٧ طلب (٣٥٪)** مالهمش إجابة على «ليه الشخص ده؟».

دلوقتي القايمة = **(`orders.technician_id` ∪ `order_team_members` ∪ `order_assignments` ∪
`technician_work_opportunities`) ∪ (مجمّع المدينة)**. المجموعة الأولى **مش** متقيّدة بمدينة ولا
باعتماد عن قصد — المستبعد دلوقتي هو بالظبط اللي السؤال عنه. كل صف بيرجع معاه `relation_to_order`
(`assigned`/`crew`/`offered`/`city_pool`) والواجهة بتجمّع بيه بدل الدور، فاللي على الطلب فوق.

**فجوة موثّقة**: `explainTechnicianForOrder()` لسه بترمي لطلب بلا نطاق (أهلية النطاق مالهاش معنى
من غيره). الاسم بيظهر في القايمة، بس التفسير التفصيلي لأ — و`scope_note_ar` بيقول كده **قبل**
الضغط بدل ما الأدمن يكتشفه بخطأ. التدهور الجزئي للتفسير نفسه (checks بثلاث حالات بدل `passed`
بوليان) قرار أكبر من جولة عرض، متسجّل هنا مش متنفّذ جانبيًا.

الدليل الحي: `node scripts/verify-explain-candidates-coverage.js` (خمس حالات، أربعة منهم كانوا
بيفشلوا قبل الإصلاح).
...
## سبب الإلغاء إجباري لما يكون فيه قايمة (docs/08 §112)

`OrdersService.cancel()` كانت بتحسب رسوم الإلغاء جوّه `if (dto.cancellation_reason_id)` والحقل
ده اختياري — يعني العميل اللي بيدفع الرسوم كان هو اللي بيقرر لو هي تنطبق عليه (ما يختارش سبب =
صفر رسوم مهما كانت سياسة الأدمن). دلوقتي: لو `listActive(CUSTOMER)` بترجّع أي سبب، الاختيار
إجباري ورفضه `VAL_001`. لو القايمة فاضية، الإلغاء بنص حر بيفضل شغّال — عشان ما نقفلش باب
الإلغاء على العميل بسبب داتا ناقصة عند الأدمن (وده الوضع الافتراضي فعلاً: migration 0216 بتزرع
أسباب الفني بس، وصفر أسباب للعميل).
...
## سلطة السعر مفصولة لتلات صلاحيات (ADR-0068)

`orders.adjust_price` لوحدها كانت بتفتح سبع عمليات مختلفة الخطورة — من «خفّض سعر لعميل زعلان»
لحد «اعتمد سعر أعلى من السقف اللي العميل شافه». يعني ماكانش ممكن تدي موظف خدمة عملاء الأولى من
غير التانية.

| الصلاحية | إمتى بتتطلب |
|---|---|
| `orders.adjust_price` | الأساس على كل مسارات التسعير (زي ما هي) |
| `orders.approve_price_increase` | **إضافة** لما `adjust-price` بتزوّد الإجمالي، وعلى اعتماد عرض خارج النطاق |
| `orders.waive_fees` | **إضافة** لما الإجمالي الجديد بينزل تحت رسوم التقييم/المعاينة المسجّلة على الطلب |

التعريف الوحيد لـ«إيه اللي يُعتبر زيادة/إعفاء» في `price-change-authority.ts`. الـguard بيتأكد من
الأساسية بس (بيشتغل قبل ما نعرف السعر الحالي)، والـcontroller بيحلّ التنتين ويمرّرهم، والقرار
بيتاخد **جوّه الترانزاكشن بعد قفل الصف** — مفيش نافذة السعر يتغيّر فيها بين الفحص والتنفيذ.

**التوافق**: migration 0253 بتدي الصلاحيتين لكل دور عنده `orders.adjust_price` النهاردة، فصفر
تغيير سلوك يوم النشر — المكسب إنهم بقوا قابلين للسحب لوحدهم من شاشة الأدوار.
...
## بصمة تذكرة الحجز (`context_hash`) — قايمة حقول واحدة، مش طرفين بيجمّعوا لوحدهم (docs/08 §121-ب)

بَقّة حقيقية اتلقطت باختبار حي (بلاغ مالك 2026-09-03): البصمة كانت بتتحسب من مصدرين:
`BookingMatchPreviewService` بيعمل spread لكل الـDTO، و`bookingPreviewInputFromCreate` بيعدّد
حقوله بإيده. أي حقل عند طرف وناقص عند التاني = رفض حتمي لكل حجز.

اللي حصل فعلاً: `booking_mode` — الحقل اللي ADR-0048 بيقول عنه «متجاهَل تمامًا» — كان بيتبعت
في `POST /orders` بس، فكل طلب من مسار اختيار الفني كان بيترفض بـ«تفاصيل الحجز تغيّرت».

**القواعد اللي اتثبتت في `booking-match-context.ts`:**

1. `FINGERPRINT_FIELDS` هي **المصدر الوحيد** لأي حقل داخل البصمة، والطرفين بيمرّوا على
   `bookingFingerprintInput()`. أي حقل جديد في `PreviewOrderDto` مش هيدخل البصمة غير لما
   يتضاف هنا صراحة — وده المقصود.
2. حقل يدخل البصمة **لو وبس لو** بيغيّر السعر أو المرشّح فعلاً. الحقول المتجاهَلة برّه.
3. التواريخ بتتقارن كلحظة (`new Date(v).toISOString()`) مش كنص خام — `@IsDateString()` بيقبل
   صيغ كتير لنفس اللحظة، وأي اختلاف صيغة كان بيبوّظ الحجز.
4. الرفض بيقول **اسم الحقل** (`bookingFingerprintDiff` + `fingerprint_input` من migration 0256)
   بدل رسالة مبهمة، وبيتسجّل في اللوج بأسماء الحقول بس — من غير قيم.

## انتماء العمارة يستمر مع الطلبات المتكررة (migration 0257، docs/08 §125، طلب مالك صريح 2026-09-03)

فجوة حقيقية اتلقطت بمراجعة الكود (مش تخمين): `RecurringOrderTemplate` كان فيه تعليق صريح بيحط
`building_code` في نفس سلة "حاجات one-time متعمّدين ما نخزّنهاش" مع `promo_code`. النتيجة: طلب
أول بـ`building_code` وبعده `repeat_frequency` كان بيولّد كل النوبات الجاية **من غير خصم العمارة
خالص** — انقطاع صامت في نظام موجود ومختبر أصلاً.

**القرار المعماري**: تخزين `building_id` (FK) على القالب — مش `building_code` ومُطلقًا مش نسبة
الخصم نفسها. السبب: لو خزّنّا النسبة وقت الإنشاء وبعدين الأدمن غيّرها، النوبات الجاية هتفضل شايلة
رقم قديم غلط. تخزين الـid بس يضمن إن كل نوبة بتعمل `findActiveByIdOrNull` **وقت التوليد نفسه**،
يعني بتاخد نسبة الخصم *الحالية* دايمًا — من غير أي كاش/snapshot يحتاج invalidation.

- **مفيش مسار تسعير مواز**: `processOccurrence()` بيقرأ العمارة الحالية وبعدين يمرّر
  `building_code` عادي جوّه `CreateOrderDto` لنفس `OrdersService.create()` اللي أي طلب تاني بيعدّي
  عليه — حساب الخصم والتعارض مع البرومو والمعاملة كلها نفس الكود القديم بالحرف.
- **تدهور آمن**: عمارة اتقفلت/انمسحت وقت التوليد → `findActiveByIdOrNull` بترجّع `null`، الطلب
  بيتولّد بسعر كامل من غير خصم، والقالب نفسه يفضل نشط (الاشتراك مايتلغيش بسبب عمارة واحدة).
- **البرومو كوده لسه زي ما هو بالحرف** — التغيير مقصور على انتماء العمارة بس، زي ما اتطلب صراحة.
- المسار الضمني (`OrdersService.create()` مع `repeat_frequency` في نفس الطلب) بيورّث
  `building_id` المتحلّل بالفعل في نفس المعاملة — صفر استعلامات إضافية.
- الأدمن (`/admin/recurring-orders`) بقى شايف اسم/كود العمارة المربوطة بكل خطة — مش بس الطلب الأول.

اختبارات حية جديدة (`recurring-orders-building-affiliation.spec.ts`، 6/6 ضد Postgres حقيقي):
ربط الطلب الأول بعمارة، استمرار نفس الربط في النوبة الجاية، انعكاس تغيير نسبة الخصم فورًا على
نوبة جديدة، عدم تكرار البرومو كود تلقائيًا، معاملة سليمة (مفيش قالب/طلب يتيم) لما كود العمارة يبقى
غلط، ومفيش خصم مضاعف (نفس المبلغ بالجنيه في كل نوبة على نفس الأساس).

## تقسيم `OrdersService` بالفلو (تدقيق A-1) — شغل جارٍ بمراحل

`orders.service.ts` وصلت **٤٣١٩ سطر و٢٥ اعتمادية في الـconstructor**. الملف نفسه كان بقى فيه
تعليقات بتشرح **إزاي نتفادى إضافة اعتمادية** («آخر بند عمدًا… عشان ياخد أقل بلاست-رديوس ممكن
على الاختبارات») — ولما بنية الاختبار تفرض ترتيب الـconstructor، ده مش تفضيل، ده **coupling**.

### القاعدة المتّبعة في كل شريحة

1. الشريحة بتاخد **اعتمادياتها هي بس**، صريحة ومسمّاة — مش الـ٢٥.
2. `OrdersService` بتفضل **واجهة رفيعة** بنفس التوقيعات العامة بالحرف ⇒ صفر تغيير على أي منادي
   (controllers، gateways، والـ٢٥ spec اللي بتبنيها بوسائط ترتيبية).
3. الشريحة **مبنيّة داخليًا** (`private get` كسول) مش محقونة — حقنها كان هيضيف بند تاني لنفس
   القايمة اللي التقسيم موجود عشان يقصّرها.
4. المنادي اللي محتاج الشريحة **بس** بيحقنها هي مباشرة، ويبطّل اعتماده على الخدمة الكبيرة.

### الشرايح

| # | الشريحة | الاعتماديات | الحالة |
|---|---------|-------------|--------|
| ١ | `OrderQueriesService` — القراءات (١٢ دالة) | **٤** | ✅ خلصت |
| ٢-أ | `order-schedule-interval.ts` — حساب الفترة وفحص التعارض (دوال خالصة) | **صفر DI** | ✅ خلصت |
| ٢-ب | `OrderRescheduleService` — إعادة الجدولة (٦٧٥ سطر) | **١١** | ✅ خلصت |
| ٣ | `OrderDisputeService` — الزيارة الفاشلة + نزاع الكاش (٥٩٠ سطر) | **٩** | ✅ خلصت |
| ٤ | `OrderTechnicianOpsService` — عمليات الفني (٨٦٨ سطر) | **١٣** | ✅ خلصت |
| ٥ | `OrderCancellationService` — إلغاء العميل (٢٠٨ سطر) | **٩** | ✅ خلصت |
| ٦ | `OrderCreationService` — الإنشاء والتسعير (١٨٨٣ سطر) | **١٧** | ✅ خلصت |

### مكسب الشريحة الأولى — ملموس مش نظري

`OrderTrackingGateway` كان محتاج **دالتين** بيلمسوا الـrepository بس، وعشان كده كان لازم يبني
`OrdersService` بـ**٢٣ وسيط `{} as never`** في كل spec. كان فيه ملف كامل (`orders.testing.ts`)
موجود **بس عشان يعزل الألم ده**.

دلوقتي الـgateway بيحقن `OrderQueriesService` مباشرة، و`orders.testing.ts` **اتحذف** — الـspecs
بتبني الشريحة بأربع وسائط.

### الشريحة ٢-أ — وفرق صامت اتقفل معاها

فحص «الفني ده عنده شغل تاني على الفترة دي؟» كان **مكتوب مرتين** في نفس الملف:

| | فحص الدقة (`create`) | فحص إعادة الجدولة |
|---|---|---|
| نهاية الطلب المتعارض | `scheduled_at + duration` بس | `COALESCE(scheduled_end_at, …)` |
| طلب له `scheduled_end_at` بلا `duration` | **بيتجاهَل تمامًا** | بيتحسب |

يعني حجز بنهاية صريحة بلا مدة كان **بيعدّي بلا تعارض** وقت الإنشاء ويتمسك وقت إعادة الجدولة —
نفس البيانات، إجابتين. النسخة الموحّدة أخدت التعريف الأصح، والحالة اللي كانت بتفوت بقى عليها
اختبار حي (`order-schedule-interval.spec.ts`).

الدوال دي اتحطّت في ملف خالص (صفر DI) مش جوّه خدمة إعادة الجدولة عمدًا: مستهلكيها **مش كلهم**
في فلو إعادة الجدولة — `create()` بتفحص التعارض و`resolveFailedVisit()` بتحسب الفترة.

### الشريحة ٢-ب — فلو إعادة الجدولة

٥٩٠ سطر كانوا جوّه خدمة بـ٢٥ اعتمادية، رغم إنهم محتاجين **١١** بس. جمعهم في مكان واحد خلّى
عقد الفلو مقروءًا: مين يقدر يعيد الجدولة، وامتى، وإيه اللي بيتقفل ذرّيًا وقت التغيير.
...
| `rescheduleByAdmin` | الإدارة | نفس الآلية، بسبب إلزامي |

كلهم بيعدّوا على `rescheduleCore()` — مصدر واحد للحجز الذرّي.

اتنقل معاهم `insertDurableInAppNotification` لملف مستقل (`durable-in-app-notification.ts`):
مستهلكيه اتفرقوا على أكتر من فلو، فلو فضل دالة خاصة جوّه الخدمة الكبيرة كان أي فلو بيتفصل
هيضطر يعيد كتابته.

**النتيجة التراكمية**: `orders.service.ts` نزلت من **٤٣١٩** لـ**٣٥٧٦** سطر (‎-١٧٪‎) بلا أي
تغيير في توقيع دالة عامة واحدة.

### الشريحة ٣ — فلو النزاعات

الحالتان اللي بيدخل فيهما الطلب `DISPUTED` مختلفتان في السبب ومتطابقتان في آلية الحل:
...
الاتنين بيعدّوا على `lockDisputedOrderForUpdate()` — واللي كان وراه **بَقّة `lost update` حقيقية**:
الحل كان بيقرا الطلب بلا قفل قبل الـtransaction وبعدين يكتب الـobject القديم جوّاها، فأدمنان
بيحلّوا نفس النزاع في نفس اللحظة ⇒ التاني يغلب الأول بكامل الحالة القديمة. تجميع الفلوين هنا
بيحطّ القاعدة دي في مكان واحد بدل ما تتكرر.

**النتيجة التراكمية بعد أربع شرايح**: `orders.service.ts` من **٤٣١٩** لـ**٣٠٩٠** سطر (‎-٢٨٪‎).

### الشريحة ٤ — عمليات الفني، وبَقّة اختبار كشفها التقسيم

كل حاجة الفني بيعملها على طلب هو ماسكه: دورة التنفيذ (`depart`→`arrive`→`start`→`complete`)،
التمديد (`continueWorkAnotherDay`)، الخروج (`technicianCancel` + سياسة نافذة الإلغاء)،
والاستبدال (`requestRematch`). كلهم بيعدّوا على `transitionAsTechnician()` — مصدر واحد بيحترم
الـstate machine ويسجّل التاريخ، فمفيش انتقال بيتكتب بإيد.

**اللي التقسيم كشفه**: `technician-continue-another-day.spec.ts` كان بيموّه **دالة خاصة
بالاسم** (`insertDurableInAppNotification`) على نسخة الخدمة. يعني الاختبار كان مربوط بدواخل
الكود، مش بسلوكه — وأول ما الدالة بقت مستقلة، التمويه اختفى والإشعار الحقيقي اتكتب فكسر تنظيف
الاختبار على مفتاح أجنبي.

الإصلاح **مش إرجاع الدالة لمكانها**: الاختبار بقى يسيب السلوك الحقيقي يحصل وينضّف وراه. اختبار
بيموّه دالة خاصة بالاسم هو اختبار بيمنع إعادة الهيكلة، مش بيحمي سلوك.

**النتيجة التراكمية بعد خمس شرايح**: `orders.service.ts` من **٤٣١٩** لـ**٢٣٨٤** سطر (‎-٤٥٪‎).

### الشريحتان ٥ و٦ — والنتيجة النهائية

**الإلغاء** (٩ اعتماديات) مش تغيير حالة بس: جوّاه **قرار مالي** (رسوم حسب مرحلة الطلب وسببه،
استرداد المدفوع، وفكّ كود الخصم). فصله بيخلي القرار ده مقروءًا في مكان واحد.

**الإنشاء والتسعير** أكبر شريحة (١٧ اعتمادية) — و**ده مش عيب فيها**: إنشاء طلب هو النقطة اللي
بيتلاقى فيها كل النظام مرة واحدة (عنوان، مبنى، كتالوج، محرك تسعير، كود خصم، نطاق جغرافي،
جدولة، فني/شركة مطلوبين، ضمان اختياري، صور الفورم). الفرق إن ده بقى **مكتوب صراحة**: الشريحة
بتقول محتاجة إيه بالاسم، بدل ما تكون بند وسط ٢٥ اعتمادية بتخدم ست فلوهات مختلفة.

`create` و`previewPrice` في شريحة **واحدة** عمدًا: بيشاركوا نفس المساعدات (snapshot إجابات
العميل، صور الحقول، الضمان). لو اتفرقوا، السعر المعروض والسعر المكتوب هيبقى لهم **مصدران**.

### النتيجة النهائية

| | قبل | بعد |
|---|-----|-----|
| `orders.service.ts` | **٤٣١٩ سطر** | **٤٨٢ سطر** (‎-٨٩٪‎) |
| أطول «دالة» فيها | `create` بـ١٢٠٠ سطر | getter بـ٢٠ سطر بيبني شريحة |
| منطق عمل فيها | كله | **صفر** — كل دالة سطر تفويض واحد |
| توقيعات عامة اتغيّرت | — | **صفر** (اتأكد بمقارنة القايمة حرفيًا قبل/بعد) |

`OrdersService` بقت **واجهة رفيعة** بالمعنى الحرفي: بتبني الشرايح من اعتمادياتها وبتفوّض. القرار
إن الشرايح **مبنيّة داخليًا مش محقونة** خلّى الـ٢٥ spec اللي بتبنيها بوسائط ترتيبية تفضل شغّالة
بلا لمسة واحدة.

### اختباران تانيان كانوا مربوطين بدواخل الكود

زي `technician-continue-another-day.spec.ts` في الشريحة ٤:

- `pricing-field-images-validation.spec.ts` كان بيمسك دالة خاصة من `OrdersService.prototype` —
  اتحوّل يمسكها من `OrderCreationService.prototype`، لأنه بيختبر **منطق التحقق** مش مكان الدالة.

**القاعدة**: اختبار بيوصل لدالة خاصة بالاسم بيمنع إعادة الهيكلة. لما التقسيم يكشف واحد، الإصلاح
إن الاختبار يمشي ورا السلوك — مش إن الكود يفضل مكانه عشان الاختبار.

### حارس الواجهة (تدقيق T-4)

التقسيم بيضيع بسهولة: أسهل حاجة على أي مطوّر بعد كده إنه يكتب «كام سطر بس» جوّه دالة في
`OrdersService` بدل ما يفتح الشريحة الصح. عشر مرات كده والملف رجع تاني.

`orders.service.spec.ts` بيمنع ده **بنيويًا** بدل ما يعتمد على انضباط المراجعة:

| بيتأكد من | ليه |
|-----------|-----|
| كل دالة عامة بتفوّض لشريحة | مفيش منطق عمل بيرجع للواجهة |
| الملف تحت ٧٠٠ سطر | مساحة لشرايح جديدة، مش لمنطق |
| الشرايح الست مبنيّة داخليًا | البناء الداخلي هو اللي بيخلي الـ٢٥ spec الترتيبية شغّالة |

وده كمان بيقفل ملاحظة T-4 في التدقيق («أكبر ملفين مالهمش `.spec.ts` مجاور، فمفيش مكان واحد
بيوصف عقد الملف»).

**أول حاجة الحارس مسكها**: `markViewedByTechnician` كانت لسه فيها منطق حقيقي (استعلامان
+ `try/catch`) في الواجهة — اتنقلت لشريحة عمليات الفني، مكانها الصح.

## ج-٣ (2026-09-09) — آلة الحالة مقاسة حيًا، مش مستنتجة من الجدول

`node scripts/order-state-machine-audit.js` بيمشي تسع مسارات بالـAPI الحقيقي وبعد كل خطوة بيتحقق
من: الحالة الفعلية = المتوقعة، **الانتقال مسموح في `ORDER_TRANSITIONS`** (بيستورد نفس الملف
المترجم، مش نسخة منسوخة)، و`order_status_history` سجّله. وبعدين تمن أفعال خارج الترتيب — المطلوب
`4xx` برسالة عربية والحالة ما تتغيرش. النتيجة الحالية ٣٤/٣٤ وصفر `5xx`، والتفاصيل في
`docs/29-e2e-production-readiness-audit-2026-09-08.md`.

**بَقّة حقيقية اتلقطت**: `cancellation_reasons` كانت فيها أسباب الفني بس (٧ من `0216`) و**صفر
للعميل** — قايمة اختيار فاضية في التطبيقين، و**سياسة رسوم إلغاء العميل معطّلة عمليًا** لأن الرسم
بيتحسب من صف السبب. اتقفلت بـ`0303_default_customer_cancellation_reasons.sql`.

**التغطية بصراحة**: ١٠ من ٧١ حافة معلنة. السكريبت بيطبع الباقي بالاسم — الحواف الناقصة بتخص
معاينة-ثم-سعر، العرض عن بُعد، فرز الأدمن، `draft`/`expired`. مين هيكمّل يلاقيها مكتوبة.

## صلاحيات الأدمن أثناء تنفيذ الطلب (ADR-0083، docs/08 §138)

بلاغ مالك 2026-09-11: «شغل معاه حد وعايزين نضيف لنكمل الفريق… السيستم مش بيرضى يضيف حد رغم إن
الطلب محتاج ناس وفيه ناس فاضية». `scripts/repro-crew-add.js` (سكريبت إعادة إنتاج قابل لإعادة
التشغيل) أثبت **سببين منفصلين**:

**١. `CREW_MUTABLE_STATUSES` كانت بتقف عند `technician_arrived`.** يعني طلب فريق شغّال فعلاً
(`in_progress`) — وهو بالظبط الوقت اللي بتكتشف فيه نقص العمالة — كان بيرجّع 409. الحد ده كان
بيحمي من خطر **مش موجود** في النص اللي حجبه: `order_earning_shares` مابتتكتبش غير في
`PaymentsService.settleAndComplete()` عند الانتقال لـ`completed`، فالطاقم ساكن ماليًا قبل كده.
الحد بقى **الإغلاق المالي**: مسموح لحد `awaiting_*`، وممنوع من `work_completed` وبعدها،
و`disputed` ممنوع لأن ليه مسار حل خاص بحسابات رسوم/استرداد.

**٢. نافذة فحص القدرة كانت بتبدأ من يوم بداية الطلب الأصلي.** طلب ممتد بدأ من ١٠ أيام وممتد ٢٠
كان بيتقاس على العشرين كلهم، منهم ١٠ عدّوا — فأي مرشّح عنده شغل في أي يوم منهم يبان «مشغول».
البداية بقت `GREATEST(يوم البداية, النهاردة)` في التلات أماكن اللي بتقيس نافذة
(`dailyCapacityExceededExpr`، `blockedExistsExpr`، و`classifyTechnicianCapacity`). **النهايات
مربوطة بالبداية الأصلية عن قصد** — لو اتحسبت من النهاردة كانت هتمدّ النافذة لقدّام بعدد الأيام
اللي عدّت. لأي طلب بيبدأ النهاردة أو بعدها `GREATEST` بترجّع نفس القيمة بالحرف.

**إعادة الجدولة والإلغاء بقوا متاحين للأدمن طول ما الطلب مفتوح** (طلب مالك: «يكون دايمًا عنده
أكسس ليهم هما الاتنين»):
- `ADMIN_RESCHEDULABLE_STATUSES` في `order-reschedule.service.ts` أوسع من قايمة العميل، و
  `assertReschedulable()` بقت بتاخد `isAdmin` (افتراضيها `false` عشان أي كولر جديد ينزل على
  الأضيق بلا قصد). مسار العميل **ما اتغيرش**.
- `ACCEPTED`/`TECHNICIAN_ON_WAY`/`TECHNICIAN_ARRIVED` بقى ليهم انتقال لـ`CANCELLED_BY_SYSTEM`.
  التلاتة قبل أي حركة مالية، و`IN_PROGRESS` (الأبعد منهم) كان مسموح أصلاً.

التغطية: `admin-crew-management.spec.ts` (٦ اختبارات حية للحد الجديد والنافذة)،
`admin-reschedule-day-based.spec.ts` (٣ اختبارات لفرق صلاحية الأدمن عن العميل)،
`order-state-machine.spec.ts` (٣ للإلغاء).

**درس اتكرر في الاختبارات نفسها**: إضافة عضو ناجحة بتخلي الفني **ملتزم فعلاً**، فمشاركة نفس
الفني بين تستين بتقيس التلوث مش السلوك — التستات الجديدة بتعمل فني جديد لكل استخدام.

## تبرير إجباري لأي فعل بيزوّد فلوس الطلب (ADR-0084 §2، docs/08 §139)

`QuoteItemDto.description` و`SubmitInitialQuoteDto.diagnosis` بقوا **مطلوبين** بحد أدنى ١٠ حروف.
قبل كده كانوا `@IsOptional()`، يعني فني يقدر يضيف ٥٠٠ جنيه «قطعة غيار» بلا كلمة واحدة تشرح،
والأدمن يراجع رقم بلا سبب.

**على مستوى العقد (DTO)، مش على عمود القاعدة، عن قصد**: الصفوف التاريخية اتكتبت تحت العقد
القديم، وتحويل العمود لـ`NOT NULL` بيحتاج اختراع نص لبيانات حقيقية — وده تزوير سجل. الصفوف
القديمة بتفضل مقروءة وبتتعلّم في مركز المراجعة بـ`justification_missing`.

الحد الأدنى ١٠ حروف مش تجميل: `@IsString()` لوحدها بتعدّي نقطة واحدة، والخانة الإجبارية اللي
بتتملا بنقطة مابتراجعش حاجة. التغطية في `money-action-justification.spec.ts`.

## استبدال المنفّذ (ADR-0084 §4)

`REASSIGNABLE_STATUSES` بقت تشمل `technician_on_way` و`technician_arrived`. السيناريو اللي
المالك وصفه (الفني قابل ومعتذر بعد ساعات) كان مغطّى من `accepted`، لكن اللي بعده مباشرة —
اتحرّك وبعدين اعتذر، أو وصل ولقى نفسه مش قادر — كان بيلاقي الزرار ميت.

الاستبدال بيرجّع الحالة لـ`ACCEPTED` عن قصد: المنفّذ الجديد ما اتحركش ولا وصل. وسجل الحالات
بقى بيكتب **الحالة السابقة الحقيقية** بدل `TECHNICIAN_ASSIGNED` الثابتة (كانت صح لما القايمة
كانت بتوقف عند `accepted`، وبقت بتكتب تاريخًا غلط بعد التوسيع).

**`in_progress` بيفضل مقفول وده قرار صريح**: الشغل بدأ يبقى للفني الأصلي حق مالي، وتبديل
القائد بيمسح مشاركته من التسوية بصمت. المسار الصح هناك الزيارة الفاشلة أو الشكوى — الاتنين
بيحسبوا رسوم/تعويض. رسالة الرفض بتقول ده بالنص.

## نافذة اختيار الموعد (ADR-0097، docs/08 §151)

`booking-window.ts` — العميل مايقدرش يختار **بداية** برّه ٥ص–٧م (إعدادين:
`booking.selectable_start_hour` / `booking.selectable_end_hour`، والساعة الأخيرة **شاملة**).

**القيد على البداية بس، وده نص طلب المالك**: المدة والطاقة اليومية وامتداد الشغل لأيام
والتوزيع والتسعير كلهم زي ما هم. شغلانة بتبدأ ٦ م وبتاخد ٤ ساعات بتخلص ١٠ م عادي.

الحساب **بتوقيت المنصّة** مش UTC: العميل بيختار بتوقيت القاهرة والقيمة بتتبعت UTC، والمقارنة
الخام كانت هتزحزح النافذة ساعتين/تلاتة (ومصر عندها توقيت صيفي فالإزاحة نفسها بتتغيّر).

مستثنى عمدًا: الطلب المستعجل، السلوت المعلَن من الفني، التكرار المتولّد، وإعادة الجدولة
الإدارية. إعادة الجدولة **من العميل** عليها نفس القيد — وإلا فيه باب تاني بيوصل لنفس النتيجة.

إعداد مقلوب بيرجع للافتراضي بدل ما يوقف كل الحجوزات على المنصّة.

## أهلية الخانة ≠ الطبقة المالية (ADR-0101، docs/08 §156)

### السؤالين اللي كانوا مدموجين

| السؤال | مصدره | الحقل |
|---|---|---|
| «بياخد كام؟» | `technician_profiles.technician_kind` | `order_team_members.member_type` |
| «يقدر يملا الخانة دي في الخدمة دي؟» | إعداد الخدمة (`requires_technician_lead`) | `order_team_members.crew_slot` |

تجنيد الطاقم كان بيجاوب السؤال التاني بالحقل الأول: `technicianKindCondition({ kind: 'technician' })`
— قاعدة **مطلقة** بتقرا الشخص. وقايمة اختيار العميل بتستخدم قاعدة **خاصة بالخدمة**. النتيجة إن
نفس الشخص كان **ظاهر كقائد للعميل ومختفي كمنفّذ للقائد**، والكتابة بترفضه بـ409.

⚠️ **ADR-0087 كان قرّر ده خلاص** وكاتب صراحةً إن `technicianKindCondition()` ممنوع على مسارات
القيادة. مسار القيادة اتنقل، ومسار التجنيد اتنسي. فالفجوة كانت **انحراف عن قرار نافذ**.

### القاعدة الواحدة دلوقتي

`crewSlotQualificationCondition({ slot })` في `technician-eligibility.sql.ts`:

| الخانة | الشرط |
|---|---|
| `lead` / `execution` | اعتماد التخصص + (`requires_technician_lead` ⇒ `kind='technician'`) |
| `helper` | اعتماد التخصص وبس |

`execution` **نفس نداء `lead` بالحرف** — مش تشابه. المنطق: الشخص اللي الخدمة بتسمح له يقودها
كاملة قدام العميل، مفيش معنى لإنه مايشاركش في تنفيذها.

### ⚠️ لو بتضيف مسار كتابة جديد للطاقم

1. **الفحص بالخانة المطلوبة، مش بطبقة الشخص.** كان في `admin-orders.service.ts` بيشتق الدور من
   `effectiveMemberType` (اللي بيتفرض `assistant` لأي مساعد) — يعني أدمن بيضم مساعد لخانة تنفيذ
   كان بيتفحص بقواعد خانة المساعدة، فشرط «الخدمة محتاجة فني كامل» ماكانش بيتطبّق. اتصلح، وأي
   مسار جديد لازم يقرا نفس المدخل في الفحص والكتابة.
2. **`crew_slot` لازم يتكتب** مع أي `INSERT` في `order_team_members` — من **الدور المطلوب**،
   مش من `technician_kind`. الافتراضي `execution`.
3. **العدّ بـ`countCrewSlots()`** مش بـ`GROUP BY member_type`. كان نفس الاستعلام مكرر في خمس
   أماكن؛ العدّ بالطبقة المالية بيسيب خانة التنفيذ ناقصة للأبد لما مساعد يملاها.

### الأرباح مالمستش

`crew-earnings.service.ts` بيقرا `otm.member_type` وبس، و`resolveEffectiveMemberType()` زي ما هي.
مساعد بيملا خانة تنفيذ **بياخد نسبة المساعد** — طلب المالك الصريح.

### `bookingMode === TEAM` اتشال من مسار الطاقم

`orderRequiresCrewBeyondLeader()` بتجاوب من `required_technicians/required_assistants`.
`resolveBookingMode()` بتطلّع `individual` لطلب محتاج فنيين لو الخدمة `allows_team = false` —
فطلب متطلباته فنيين كان قائده **ممنوع** يجنّد، والطاقم مايكتملش، والشغل يقف. الـenum فضل
موجود كـmetadata مشتقّة (بيانات تاريخية + عقود) بس مابقاش مصدر قرار في المسار ده.

## مسار تكوين سعر العميل (ADR-0107، docs/08 §161)

`GET /admin/orders/:id/price-trail` بيشرح **كل جنيه** في `total_amount_cents` من **لقطة
تاريخية** على الطلب — مفيش قراءة إعدادات حيّة ولا إعادة حساب.

- **اللقطة**: عشرة أعمدة على `orders` (migration 0352) بتتكتب مرة واحدة وقت الإنشاء من نفس ناتج
  `CatalogService.estimate()`، ومعاها **قيد في القاعدة** (`chk_orders_price_formation_sums`)
  بيضمن إن المراحل بتجمع لسعر الشغل.
- **القارئ**: `order-price-trail.ts` — **دالة نقية**، مفيش استعلام جوّاها، ومابتعرفش تضرب في
  مضاعف ولا تقصّ على حد. فمفيش تكرار لخوارزمية التسعير.
- **المراحل**: ناتج المحرك → تعديل المنطقة → مضاعف فئة المهارة/الشركة → قصّ الحدود → كشف →
  استعجال → إضافات → ضمان → خصم.

### تحذيرين مهمين لأي حد بيلمس ده

1. **`orders.addons_amount_cents` عمود راكد** — مفيش كود بيكتب فيه. الإضافات بنود في
   `order_items` بنوع `addon`. قراءة العمود بترجّع صفر دايمًا وتخلي الشرح يبان متماسك وهو ناقص.
2. **`PreviewOrderResponseDto.base_price_cents` اسم مضلّل** — قيمته سعر الشغل **بعد** المراحل
   التجارية كلها، مش الناتج الخام. محفوظ للتوافق مع نسخ التطبيق المنشورة؛ استخدم
   `work_price_cents` في أي قارئ جديد.

### الخصم: مين جوّاه إيه (ADR-0107 تعديل ١، docs/08 §162)
...
أي قارئ بيجمع العمودين بيحصّل الحافز مرتين (وده حصل فعلاً في المسار قبل الإصلاح، والأدمن شاف
«غير مفسَّر 30 ج»).

| البند | القيمة الصح | مكانه في المسار |
|---|---|---|
| خصم الحجز | `discount_amount_cents - instapay_discount_cents` | مرحلة تكوين سعر |
| حافز InstaPay | `instapay_discount_cents` | تغيير بعد الحجز |

وخصم الحجز بيتسمّى بمصدره من `promo_code_id`/`building_id` — لقطة على صف الطلب، فالسطر يفضل
صادق حتى لو الكود اتشال أو نسبة العمارة اتغيّرت.

### العروض المعتمدة: استبدال مش إضافة

العرض المعتمد **بيستبدل** سعر الشغل، فأثره على الإجمالي هو **الفرق** عن المرجع اللي قبله —
نفس حساب `inspection-quote.service.ts` بالحرف:
...
المسار بياخد `approvedQuotes` مرتّبة بوقت قرار العميل، وكل عرض مرجعه اللي قبله — فسلسلة
مراجعات بتتفسّر كلها بلا تحصيل مزدوج، ولكل عرض `before_cents`/`after_cents`/`at`.

### ⚠️ حافز InstaPay لازم يعدّي أي تحديث سعر

`consumeReplacementPreview()` بتحدّث السعر من لقطة تذكرة المعاينة — واللقطة دي بتعرف خصم الحجز
بس. الإسناد المباشر منها كان بيبخّر الحافز: `discount_amount_cents` بيفقده و`total_amount_cents`
بيرجع أعلى بقيمته، فعميل دفع بالسعر المخفّض يتطلب منه الفرق تاني. **أي مسار جديد بيكتب
`total_amount_cents` من لقطة لازم يطرح `instapayDiscountCents` منها.**

### اشتقاق وضع الحجز في قايمة المنفّذين (ADR-0106)

`GET /services/:id/technicians` بيشتقّ الوضع بـ`resolveBookingMode()` — **نفس** الدالة اللي
`POST /orders` بيستخدمها — مش من `query.booking_mode`. قبل كده `booking_mode === 'team'` كان
بيغيّر فضاء الأهلية، والويب (مستحيل يطلّع `team`) والتطبيق كانوا بيشوفوا **قايمتين مختلفة**
لنفس الشغلانة. `booking_mode` بقى مقروء كحقيقة استعجال بس.

### سقف رتبة التجنيد بينطبق على خانة التنفيذ بس (بلاغ مالك 2026-09-19)

**البلاغ**: «الأوردر راح لفني هو أصلاً مساعد، دوس محتاج مساعد، قايمة المساعدين فاضية دايمًا —
حتى بعد ما إدّيت لمساعد تاني تغطية نفس منطقة الطلب».

**السبب الجذري**: `listRecruitCandidates()` كانت بتفلتر بـ
`CASE current_level ... END <= رتبة القائد` لكل الأدوار. القاعدة دي («ماتجنّدش حد أعلى منك رتبة»)
أصلها خانة **التنفيذ**، وهناك ليها معنى: المُجنَّد بيشتغل منفّذ جنب القائد. في خانة **المساعدة**
مالهاش معنى — المساعد بيساعد، والرتبة بتوصف مستواه كمنفّذ مش كمساعد.

والضرر كان يتراكم مع ADR-0087: المساعد بقى يقدر **يقود** طلب، والمساعد اللي بيقود عادةً رتبته
الأدنى (`new` = صفر). فالسقف بيقع على صفر ويخفي **كل** مساعد أعلى من مبتدئ — مهما كانت مناطقه
وتخصصاته واعتماداته مظبوطة. الأعراض بالظبط: قايمة فاضية مهما عمل الأدمن أي حاجة.

**الإصلاح**: `rankCeiling = role === 'assistant' ? MAX_TECHNICIAN_LEVEL_RANK : leaderRank`. الرفع
بيتم على **قيمة الـparameter** مش بحذف الشرط من الـSQL — نص الاستعلام بيفضل زي ما هو بالحرف،
و`$3` يفضل مربوط ومُعرَّف النوع (حذف الإشارة ليه بيكسر الاستعلام بـ
«could not determine data type of parameter $3»، حصل فعلاً أثناء التنفيذ).

**والكتابة اتغيّرت مع القراءة**: `recruitMember()` كان فيه **نفس** فحص الرتبة بلا وعي بالدور،
فتصليح القايمة لوحدها كان هينتج أسوأ حالة — اسم ظاهر في القايمة والضغط عليه بيدي
«الفني ده رتبته أعلى منك». الاتنين بقوا `role !== 'assistant'`.

**اللي ما اتغيّرش عمدًا**: سقف الرتبة في خانة التنفيذ (لسه شغّال ومختبَر)، واعتماد التخصص
(خدمة أو فئة) المطلوب للمساعد — ده **قرار المالك نفسه في §105** اللي ألغى §103
(«لا يجوز لمساعد سباكة أن يظهر للكهرباء»)، وحدود المدينة، وفحص الشركة، والجدول، والموقع.
