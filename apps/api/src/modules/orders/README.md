...
### تحديث (docs/08 §19 بند 3+5) — PENDING_PAYMENT sweep + استرداد تلقائي عند إلغاء طلب مدفوع

تدقيق جاهزية إطلاق لاحق من المالك لقى فجوتين حقيقيتين إضافيتين في الملف ده صفر أي كود ليهم:

1. **`sweepPendingPayment()` جديدة** — طلبات `PENDING_PAYMENT` قديمة (العميل بدأ دفع إلكتروني
   مسبق — كارت/InstaPay، ADR-0013 "PAY BEFORE DISPATCH" — بس مخلّصش) بتتلغى تلقائيًا بعد إعداد
   جديد `orders.payment_timeout_minutes` (افتراضي 15 دقيقة، `infra/migrations/0100_pending_payment_timeout_setting.sql`
   — عمدًا أقصر من `orders.auto_cancel_after_minutes` — مفيش داعي الطلب يحجز نافذة توزيع فني وهو
   أصلاً مدفوعش لسه). مفيش استرداد هنا — الدفع نفسه مكملش، مفيش فلوس اتاخدت أصلاً.
2. **استرداد فوري تلقائي** — لو `cancelIfStillSearching()` لغت طلب `SEARCHING_TECHNICIAN` كان
   بالفعل `paymentStatus=PAID`، بتنادي `PaymentsService.refundSystemCancelledOrder()` جديدة (تفاصيل
   كاملة في `../payments/README.md`) فورًا بعد الإلغاء. فشل الاسترداد (استثناء غير متوقع) بيتلقط
   ويتسجّل بس مايوقفش بقية الفحص الدوري — الطلب فضل ملغي صح حتى لو الاسترداد فشل واحتاج متابعة يدوية.

**فجوة موثّقة صراحة، خارج نطاق هذا الإصلاح عمدًا**: استخدام `promo_code` وقت إنشاء الطلب بيزوّد
`PromoCode.usedCount` بس مفيش أي decrement/release ليه في أي مسار إلغاء بالكامل في النظام — مش
بس هنا، ولا في `cancel()` فوق ولا إلغاء الفني. قصور نظامي أوسع سابق على هذا التعديل، محتاج قرار
عمل منفصل (Business Decision Required، مش جزء من بند 3+5 تحديدًا).

اتعمله اختبار حي (`order-auto-cancel-pending-payment.spec.ts`، 3 اختبارات): طلب `PENDING_PAYMENT`
قديم يتلغى بلا استرداد، طلب `SEARCHING_TECHNICIAN` مدفوع (كارت) قديم يتلغى **ويترد فورًا تلقائيًا**
(orderStatus فضل `CANCELLED_BY_SYSTEM`، paymentStatus بقى `REFUNDED`)، وطلب `SEARCHING_TECHNICIAN`
غير مدفوع (كاش) قديم يتلغى بلا أي محاولة استرداد كاذبة (regression).
