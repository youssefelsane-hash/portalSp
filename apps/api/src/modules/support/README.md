...
**فجوة موثّقة على مستوى الصلاحيات:** كل مسارات `@Roles(ADMIN)` في المشروع (هنا وفي `payments`) بتسمح لأي حساب `user_type=admin` يعمل أي حاجة، من غير تفرقة بين أدوار الإدارة الدقيقة (`super_admin`, `ops_manager`, `support_agent`, `finance`, `recruiter` — موجودين في جدول `roles` من `infra/migrations/0003` بس `permissions`/`role_permissions` لسه فاضيين). يعني حالياً أي أدمن يقدر يحل شكاوى ويوافق على صرف فلوس مع بعض. الصح إن `support_agent` بس يحل شكاوى و`finance` بس يوافق على الصرف — البنية التحتية (الجداول) موجودة من اليوم الأول، بس الـ guard اللي بيتحقق منها لسه مش مبني. موثّقة هنا كقرار نطاق واعي، مش سهو.
