...
## `ClientErrorsService` — أخطاء واجهة المستخدم (ADR-0114)

**الفجوة اللي اتقفلت**: كل رصد الأخطاء كان من ناحية السيرفر — `RequestMetricsService` (نسبة 5xx
في الذاكرة) و`.dev-logs/errors.log` (متوقّف في الإنتاج عمدًا). خطأ بيحصل في متصفح العميل — رندر
مكسور، شبكة قاطعة، استدعاء بيرجع 500 — مكانش بيترك **أي** أثر. كنا بنعرف بالمشكلة لما حد يكلّم
الدعم، لو كلّم.

| المسار | مين | بيعمل إيه |
|---|---|---|
| `POST /telemetry/client-errors` | `@Public()`، throttle 30/د لكل IP | استقبال بلاغ، بيرد `202` فاضي دايمًا |
| `GET /admin/ops/client-errors?hours=24` | `operations.view` | تجميع بالبصمة، مرتّب **بعدد الزوّار** |

### ليه المسار عام، وإيه اللي بيخلّيه مقبول

أهم الأخطاء بتحصل **قبل** الدخول: شاشة اللوجن نفسها، صفحة خدمة جاية من جوجل، توكن خلص. مسار
محمي كان هيسجّل كل حاجة *إلا* اللي اتعمل عشانه. الضوابط الستة مكتوبة بالتفصيل في
`client-errors.controller.ts` وفي ADR-0114 §4، وأهمها: DTO مقفول بـ`@IsIn`/`@MaxLength`، تطهير
المسارات **في السيرفر** كمان مش في العميل بس، `user_id` من توكن متحقَّق منه وأبداً من الجسم،
والمتصفح/الجهاز مشتقّين من هيدر `user-agent`. الملف مسجّل في قايمة `@Public()` المراجَعة في
`scripts/security-audit.js` مع سببه.

### الخصوصية: `visitor_hash` بملح يومي

عدّ «كام مستخدم» محتاج **تمييز** مش **هوية**. المفتاح مشتقّ بـHKDF بلافتة غرض خاصة من سر موجود
(فصل مجالات، مش إعادة استخدام سر)، والملح هو تاريخ اليوم — فالعدد اليومي صحيح وربط نفس الزائر
عبر أيام مستحيل.

### الإشارة دخلت لوحة المراقبة مش لوحة تانية

`OpsMetricsService` بياخد `countLastHour()` وبيطلّع إنذار `client_errors` بعتبتين من الإعدادات
(`ops.alert_client_errors_per_hour[_critical]`). لو عملناها لوحة منفصلة بإنذار منفصل كان بيبقى
عندنا مكانين نبصّ فيهم — وده نفس الخطأ اللي ج-٧ اتعمل عشان يقفله.

### `ClientErrorsRetentionService`

دورة يومية بقفل استشاري (`runExclusiveSweep`) بتمسح الأقدم من `ops.client_errors_retention_days`
(٣٠ يوم افتراضيًا). **خدمة لوحدها مش سطر جوّه `QueueWatchdogService`**: الـwatchdog بيتعطّل
بمفتاح إعدادات، والتنظيف كان هيتوقف معاه بالصدفة.

### الشاشة

`apps/admin/src/app/ops/page.tsx` — الحالة العامة + الإنذارات + أربع إشارات + جدول أخطاء الواجهة.
قبلها `GET /admin/ops/health-metrics` ماكانش ليه **ولا سطر** في `apps/admin`: الإشارات كلها كانت
لأداة مراقبة خارجية بس.
