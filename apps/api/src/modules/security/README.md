...
- **B — recordDenial متزامن** لنفس `(actor+event_type+action)`: **كانت بَقّة حقيقية اتلقطت
  واتصلحت فعليًا (Script 7 Phase 30، 2026-08-19)** — مش "فجوة موثّقة مقبولة" زي ما كان موثّق هنا
  قبل كده. التوثيق القديم كان بيدّعي "الضمان الفعلي صفر فقدان بيانات (مجموع occurrence_count=5
  دايمًا)" — ده غلط: لو سباق الإنشاء الأول عمل صفين `open` مكرّرين، أي `UPDATE ... WHERE ...`
  لاحق (من غير `LIMIT 1` ولا id محدد) بيطابق **الصفين مع بعض** ويزوّد occurrence_count على
  الاتنين دفعة واحدة — يعني المجموع بيتضخّم **أسرع** من عدد النداءات الفعلية، مش يفضل صح. اتأكد
  حيًا في الريجريشن الكامل: 5 نداءات متزامنة → مجموع occurrence_count = 7 (مش 5). الإصلاح
  الحقيقي: `pg_advisory_xact_lock(hashtext($1), hashtext($2))` على مفتاح الـdedup
  (actor، `type:action`) قبل أي قراءة/كتابة جوّه `recordDenial()` — نفس النمط بالحرف المستخدم في
  `AuthService.requestOtp()` لمنع سباق إعادة إرسال OTP. بيسلسل النداءات المتزامنة بالكامل، فمفيش
  صفين مكرّرين يتعملوا خالص. الاختبار بقى صريح: صف واحد بالظبط، occurrence_count = 5 بالظبط —
  اتأكد 10 تشغيلات متتالية نظيفة بعد الإصلاح (كانت فلاكي قبل كده، اتكررت 4 مرات عبر سيشنز سابقة).
