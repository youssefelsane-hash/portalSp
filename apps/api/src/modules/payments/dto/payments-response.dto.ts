...
export interface PaymentChannelResponseDto {
  ...
  admin_note?: string;
  /**
   * أسماء خانات الإعداد الناقصة **كقايمة مستقلة** — للأدمن بس زي `admin_note`.
   *
   * كانت ملزوقة جوّه `admin_note` نفسه (`…غير مكتمل: API Key, Secret Key, …`)، فبتوصل
   * للوحة كجملة عربية فيها أسماء إنجليزية مفصولة بفواصل — واتجاه النص بيتلخبط وبقى صعب
   * تعرف عدد الخانات الناقصة أصلاً (بلاغ مالك 2026-09-17). القايمة المستقلة بتخلّي اللوحة
   * تعرضها شرائح مقروءة، والجملة تفضل جملة.
   */
