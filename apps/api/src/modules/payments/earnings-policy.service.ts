...
export class EarningsPolicyService {
  ...
  async resolveParticipants(
    ...
    return rows.map((row) => {
      // توافق تاريخي: القسمة على `participant_role` لا `technician_kind`. ADR-0086 يمنع إنشاء
      // مساعد قائد جديد، لكن طلبات قديمة بهذه الصورة لازم تظل قابلة للتسوية بلا تغيير رجعي.
