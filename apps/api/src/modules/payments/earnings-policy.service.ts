...
export class EarningsPolicyService {
  ...
  async resolveParticipants(
    ...
    return rows.map((row) => {
      // **معامل المساعد بيتحدد من نوع الشخص كمان، مش من خانته في الطلب بس** (قرار مالك
      // 2026-09-20 — يعدّل الأثر المالي في ADR-0055 §3، شوف ADR-0108).
      //
      // القاعدة اللي المالك طلبها بالحرف: «طالما هو مساعد يسري عليه اللي بيسري على زمايله…
      // لو اتنين مساعدين في نفس الرتبة شغالين في نفس الفريق، واحد فيهم قائد فريق والتاني هو
      // اللي جايبه تحته، الاتنين ياخدوا زي بعض». قبل كده القائد كان بياخد `technician` دايمًا
      // حتى لو بروفايله `technician_kind = 'assistant'`، فمساعدين بنفس المستوى كانوا بياخدوا
      // 55.6% / 44.4% بدل 50/50 — والفرق سببه خانته في الطلب مش رتبته.
      //
      // **ليه ده مايأثرش على المساعد اللي شايل الطلب لوحده** (نتيجة ADR-0055 §3 اللي اتحافظ
      // عليها): `effectiveWeight` بيحكم التوزيع **النسبي** بس — الوعاء نفسه (`الإجمالي −
      // العمولة`) مش بيتغيّر. مشارك واحد معناه `نصيبه = الوعاء × وزنه ÷ وزنه` = الوعاء كامل،
      // أيًا كان المعامل. فالمساعد اللي عمل الشغلانة كلها لسه بياخدها كلها.
      //
      // ملاحظة للي بيقرا بعدنا: الحارس المقابل في `calculateEarningsV2` (اللي كان بيرفض
      // `isLeader` + `assistant`) **اتشال مع التعديل ده** — لازم الاتنين يتغيّروا مع بعض، وإلا
      // التسوية بترمي والطلب يتعلّق على `work_completed/unpaid` للأبد.
      const earningRole: EarningRole =
        row.participant_role === 'assistant' || row.technician_kind === 'assistant' ? 'assistant' : 'technician';
      return {
