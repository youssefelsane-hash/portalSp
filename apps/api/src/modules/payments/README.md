...
**واجهة الأدمن كانت الحلقة الناقصة (بلاغ مالك 2026-09-13، docs/08 §147)**: القاعدة فوق صحيحة،
لكن فورم الاسترجاع في صفحة الطلب مكانش فيه أي مدخل لـ`payment_id` خالص — فالأدمن على طلب مركّب
كان بياخد «الطلب فيه أكثر من دفعة قابلة للاسترداد» بلا أي طريقة يستجيب بيها. وكمان شرط ظهور
الزرار كان `payment_status === 'paid'` بس، فبعد أول استرداد (`partially_refunded`) الزرار كان
بيختفي وباقي الدفعات تتقفل من الواجهة رغم إن الخدمة بتقبلها. الاتنين اتصلحوا، والملخص المالي بقى
بيرجّع `payments[].payment_number` و`refunds[].payment_id` عشان القايمة تقدر تحسب المتبقي لكل
دفعة وتعرضه. ملاحظة مهمة للي هيبني على ده: `order_item_batch_id` **مش** علامة موثوقة على «دفعة
شغل إضافي» — بيتملي في مسار الخصم التلقائي بالكارت بس، ودفعة الزيادة المدفوعة بـInstaPay بتيجي
بلا batch (التفاصيل بالأرقام في docs/08 §147).
...
> **⚠️ تحديث 2026-09-20 (ADR-0108، قرار مالك): الجملة الأخيرة دي ما بقتش صحيحة.**
>
> المالك طلب العكس صراحةً: «طالما هو مساعد يسري عليه اللي بيسري على زمايله… لو اتنين مساعدين
> في نفس الرتبة شغالين في نفس الفريق، واحد فيهم قائد فريق والتاني هو اللي جايبه تحته، الاتنين
> ياخدوا زي بعض». فـ`resolveParticipants` رجعت تقرا `technician_kind` كمان، **و**حارس «القائد
> مايكونش مساعد» في `calculateEarningsV2` **اتشال** — الاتنين مع بعض، وده بالظبط اللي البَقّة
> فوق بتحذّر منه لو حد حرّك واحد وساب التاني.
>
> اللي اتغيّر فعليًا: مساعد قائد + مساعد منضم بنفس المستوى بقوا 50/50 بدل 55.6% / 44.4%.
> اللي **ما** اتغيّرش: المساعد اللي شايل الطلب لوحده لسه بياخد الوعاء كامل (خاصية رياضية —
> الوزن نسبي)، والفني القائد، والفرق الطبيعي بين فني ومساعد. التفاصيل والأرقام المقيسة في
> ADR-0108.
...
## خانة InstaPay بتختفي بعد قفل الطلب (docs/08 §150 بند ٤)

بلاغ المالك: «الدفع بيفضل ظاهر للشخص بعد الطلب ما يتقفل… ما يظهروش أصلاً الجزء بتاعه لسه
خالص طالما الطلب اتقفل».

الخانة اتبنت «دايمًا موجودة» عن قصد (ADR-0089) عشان العميل اللي اختار كاش يلاقي الخيار قدامه.
لكن «دايمًا» دي كانت بتمتد لبعد ما الطلب يخلص، فطلب مقفول كان بيعرض «تحب تدفع أونلاين بدل
الكاش؟ / لسه بيتحدد» وزرار معطّل بيقول «هتقدر تحوّل أول ما السعر يتحدد» — وعد بحاجة مش هتحصل.

`GET /orders/:id/instapay-preview` بقى بيرجّع `is_closed` من `CLOSED_ORDER_STATUSES`، والويب
والتطبيق بيخفوا القسم كله عليه. **القرار في السيرفر عمدًا**: نفس الخانة موجودة في السطحين،
وشرط متكتب في كل واحد لوحده بينحرف.

`DISPUTED` **مش** في المجموعة دي: النزاع طلب مفتوح لسه بيتحل، وإخفاء الدفع فيه بيشيل مخرج
حقيقي. وكل الحالات المقفولة مرفوضة في `assertPayable()` أصلاً، فالعَلَم مابيخفيش أي مستحق.

## درجة أجر المهارة — الاسم في القاعدة ≠ الاسم على السلك (docs/08 §172)

| الطبقة | الاسم |
|---|---|
| القاعدة | النوع `technician_wage_tier`، الأعمدة `wage_tier` / `service_wage_tier_snapshot` |
| الكود | `TechnicianWageTier`, `serviceWageTier`, `serviceWageFactorBps` |
| **السلك (API)** | **`skill_level`, `service_skill_snapshot`, `service_skill_factor_bps_snapshot`** |

الفرق مقصود: الأسماء جوّه بقت تقول إنها عن **الأجر** (مقابل `technician_pricing_tier` اللي عن
**سعر العميل**)، وعقد الـAPI مااتكسرش. الجسر `AS` في الـSQL وmapping في الـDTOs.

⚠️ **الاستعلامات هنا بترجّع صفوف خام بتروح للرد كما هي** — فأي عمود بيتسمّى لازم يتعمله `AS`
بالاسم القديم، وإلا مفتاح الرد بيتغيّر من غير ما `tsc` ولا أي اختبار يقع. حصلت فعلاً في §172.
`admin-earnings-policy-sql.spec.ts` بيقفل ده دلوقتي.

## بَقّة حقيقية اتلقطت واتصلحت — نافذة شحنة مزدوجة في تحصيل الأقساط (2026-09-19)

**إزاي اتلقطت**: التشغيل الكامل لسويت الاختبارات (369 suite) فشّل
`installment-collection.spec.ts` بـ`attempt_count=3` بدل `2`، بينما نفس السبيك بيعدّي لوحده.
الفشل ده سهل جدًا يتقري كـflake ويتشطب — وهو ماكانش flake.

**السبب الجذري**: استعلام الـclaim في `InstallmentCollectionService.claimDue()` بيعمل
`status='processing'` و`attempt_count + 1`، لكنه **ماكانش بيكتب `last_attempt_at`**. العمود ده
كان بيتكتب لأول مرة جوه `PaymentsService.attemptInstallmentCharge()` — يعني **بعد** حفظ صف
الدفعة. في نفس الوقت، فرع استرداد الـclaim المعلّق داخل **نفس** استعلام الـclaim بيقول:

> الصف اللي حالته `processing` و`last_attempt_at` فاضية أو أقدم من نص ساعة = claim ميت، خده.

فصف **لسه** اتاخد من ثانية بيطابق الشرط ده فورًا (لأن العمود لسه NULL/قديم)، وclaimer تاني
بياخده على طول. الـlease بتاعة النص ساعة ماكانتش بتبتدي أصلاً.

**ليه ده خطير مش تجميلي**: مفتاح الـidempotency هو `installment:{id}:{attempt}` — فيه **رقم
المحاولة**. يعني المحاولة التانية بتاخد مفتاح مختلف (`:2` بدل `:1`)، بتعدّي الـunique constraint
بشكل شرعي، وبتعمل **شحنة حقيقية تانية على نفس القسط**. الحماية الوحيدة اللي كانت قايمة
(`FOR UPDATE SKIP LOCKED`) بتحمي من التزامن اللحظي بس، مش من إعادة المطالبة بعد ما الأول سابها.

**الإصلاح**: `last_attempt_at = now()` جوه UPDATE الـclaim نفسه — النص ساعة بتبتدي من الـclaim،
وهو المعنى المقصود من الـlease من الأصل.

**فجوة تانية اتقفلت بنفس السطر**: صف بيقع `failed` من `catch` بتاع `sweep()` (اللي مابيكتبش
العمود ده) كان بيفضل بـ`last_attempt_at IS NULL`، وشرط الـbackoff
(`last_attempt_at <= now() - N days`) بيرجّع `NULL` مش `true` — يعني القسط ده ماكانش هيترجع
للمحاولة **أبدًا**. دلوقتي الختم موجود دايمًا.
