...
- **إعدادات عمولة `booking_mode` القديمة**: المفاتيح `commission.{individual|team|emergency}_adjustment_percentage` كانت جزءًا من تسوية V1. اتقاعدت مع توحيد سياسة الأرباح؛ لا يقرأها أي طلب جديد، وواجهة الـAPI تعلّمها `is_deprecated=true` وترفض تعديلها. الصفوف باقية فقط لتفسير السجلات التاريخية. أي قرار مالي جديد يُدار من سياسة المستحقات وسنابشوت الطلب، لا من مفاتيح V1.
...
## المبلغ المتبقي للتحصيل بعد الإيداع أو التقسيط — إصلاح 2026-08-25

`PaymentsService.getCollectionBreakdownForOrder()` هو مصدر الحقيقة الوحيد لما يجوز للفني تحصيله:
إجمالي الطلب ناقص دفعات الطلب المباشرة الناجحة بعد الاستردادات المكتملة، وناقص أصل السعر الذي
تغطيه خطة تقسيط معتمدة. دفعات الأقساط نفسها تسدد مديونية العميل للمنصة ولا تُحسب كاشًا على
الفني مرة ثانية. أي رسوم تمويل أو أقساط مستقبلية تظل في `installment_outstanding_cents` منفصلة.

مثال الاختبار الحقيقي: طلب `620000` قرش وإيداع 15% (`93000`) يرجع
`amount_due_to_technician_cents=527000`، وليس الإجمالي. وخطة تقسيط معتمدة تغطي أصل طلب
`620000` ترجع صفر تحصيل للفني حتى لو جدول العميل ما زال عليه أقساط. الباك-إند يرفض
`collect-cash` عندما يكون الرقم صفر، والطلب المغطى بالكامل يُسوّى تلقائيًا وبشكل idempotent.
لو أُضيف شغل جديد بعد snapshot خطة التقسيط، الفرق وحده يظل قابلًا للتحصيل.
...
## فصل رسائل العميل عن تشخيص المشغّل في `GET /payment-channels` (docs/08 §76-ز)

**بلاغ مالك حقيقي (2026-08-27)**: شاشة الدفع في تطبيق العميل كانت بتعرض «بيانات تشغيل الطريقة
غير مكتملة»، و«الطريقة مقفولة من إعدادات الأدمن»، وللكارت تحديدًا **أسماء متغيّرات إعداد Paymob
الناقصة حرفيًا** (`إعداد Paymob غير مكتمل: PAYMOB_...`). ده مش بس شكل غير احترافي — ده تسريب
معلومات عن البنية التحتية لأي عميل عنده حساب.

الـendpoint كان `@Roles(CUSTOMER, ADMIN)` وبيرجّع **نفس الرد للاتنين**. دلوقتي:

- `unavailable_reason` → جملة واحدة للعميل: «الطريقة دي مش متاحة دلوقتي — اختار طريقة تانية».
  العميل مش محتاج يعرف **ليه**؛ ده مالوش أي أثر على قراره.
- `admin_note` → التشخيص الكامل، **الحقل بيتحذف من الرد تمامًا** (مش بيترجع `null`) لو الطالب
  مش `UserType.ADMIN`. أقل سطح تسريب ممكن.

`apps/admin/src/app/settings/page.tsx` بيعرض `admin_note` فما خسرش أي معلومة تشغيلية.

**القاعدة لأي طريقة دفع جديدة**: أي نص بيوصف حالة إعداد داخلي مكانه `admin_note` — لو لقيت
نفسك بتكتب «الأدمن» أو اسم بوابة أو اسم متغيّر بيئة في حقل بيوصل للعميل، ده الفصل ده اتكسر.
اختبار `payment-channels.controller.spec.ts` بيحرس ده صراحة.

## QR كود InstaPay (migration 0211، docs/08 §78-د، 2026-08-27)

`InstaPayQrService` (`gateways/instapay-qr.service.ts`) — إعداد **واحد** اسمه
`payments.instapay.qr_image` قيمته إما `storage://<key>` (ملف اترفع من لوحة الأدمن) أو
`https://…` (رابط خارجي) أو فاضية (مفيش QR).

**ليه خدمة مستقلة مش جوّه `InstaPayProvider`؟** الـprovider بيحتفظ بإعداداته في الذاكرة ويحدّثها
بـ`SETTING_UPDATED_EVENT` (§33) — نمط صح لقيمة نصية. لكن رابط الـQR **مش قابل للتخزين المؤقت**:
لو التخزين S3، الرابط موقّع وبينتهي، فلازم يتولّد طازة وقت كل قراءة (نفس درس `branding`).

**ليه `settings.manage` مش `payments.confirm_manual`؟** ده ضبط بيانات استقبال الفلوس، نفس فئة
`ipa_address` بالظبط. موظف الـFinance المسموح له يأكّد تحويلات مش مسموح له يغيّر الحساب اللي
الفلوس بتروح له.

**`http` مرفوض** (فحص أمان، مش تجميل): صورة QR بتتحمّل بلا تشفير قابلة للتبديل في الطريق،
والنتيجة إن فلوس العميل تروح لحساب المهاجم.

**`getCustomerUrl()` أبدًا ما بترميش** — الـQR راحة إضافية فوق التعليمات النصية، مش شرط للدفع.
وقوع التخزين بيرجّع `null` + تحذير في اللوج، والعميل بيكمّل عادي.

المُتحقق نفسه بتاع البراندنج (`validateBrandingFile`) مُعاد استخدامه عمدًا — magic bytes + تطابق
الـMIME المُعلَن + حجم + أبعاد، ومفيش SVG. نسخة تانية من نفس المنطق كانت هتفترق عنه بمرور الوقت.

اختبار حي: `gateways/instapay-qr.spec.ts` (8/8) — بيغطّي المصدرين، رفض `http`، رفض ملف مش صورة،
إن `getUrl` بتتنادى في **كل** قراءة، والشيل، ووقوع التخزين.

## بَقّة حقيقية — "مستحقاتي" (كشف الشهر) كان بيقدر يختلف عن "محفظتي" (docs/08 §90.1، طلب مالك مباشر 2026-08-28)

المالك طلب تدقيق شامل قبل الإنتاج، وبالتحديد إن `GET /technician/earnings/statement`
("مستحقاتي") و`GET /wallet` ("محفظتي") يطلعوا نفس الحقيقة. الفحص لقى **ثلاثة اختلافات حقيقية** —
مش وهم عرض، اختلاف فعلي في الأرقام:

1. **عضو الطاقم كان بيشوف صفر لشغلانات اشتغل فيها فعلاً**، والقائد كان بيشوف وعاء الطاقم كله
   كأنه نصيبه هو — الاستعلام كان بيفلتر على `orders.technician_id = $1` بس، بينما اللي بينزل
   محفظة كل واحد فعليًا هو حصته من `order_earning_shares` (نفس الجدول اللي `settleAndComplete()`
   بيكتب منه حركات المحفظة، أعلاه سطر ٦٤٠-٦٩٧).
2. **الطلبات المستردة كانت لسه بتتحسب بالكامل** في الكشف رغم إن `refundOrder()` بيكون فعليًا
   عكس جزء من أرباح الفني من محفظته وقت الاسترداد.
3. **الكاش الذي استلمه قائد الطلب لم يكن يدخل المقاصة في الكشف**: طلب كاش 1000ج ونصيب 850ج
   كان يظهر +850ج في "مستحقاتي"، بينما المحفظة تسجل -150ج عمولة مستحقة للمنصة. أصبح كل سطر
   يعرض `grossTechnicianEarningCents` و`cashCollectedCents` ثم
   `netTechnicianDueCents = gross - cash - refundReversal`، وهي نفس معادلة التسوية الفعلية.

**الإصلاح** (`technician-earnings.service.ts`): الاستعلام بقى `LEFT JOIN order_earning_shares`
(بيرجع لسلوك "قائد فردي" القديم لو الصف مش موجود — طلبات قبل ADR-0040 معندهاش صفوف حصص خالص، ده
الواقع الفعلي وقتها)، وبيطرح قيود عكس الاسترداد الفعلية من محفظة كل مشارك. ولو الاسترداد قديم ومفيش
له قيود تفصيلية، بيرجع مؤقتًا لمعادلة القائد القديمة للتوافق مع البيانات التاريخية. حقلين جداد في
`TechnicianStatementJob`:
`participantRole` و`refundReversalCents` و`grossTechnicianEarningCents` و`cashCollectedCents`،
لشفافية كاملة بدل ما الرقم يتغيّر من غير تفسير. قائمة الشهور تستخدم حصص الطاقم أيضًا، لذلك عضو
الفريق لا يختفي من منتقي الشهر رغم ظهور الطلب داخل الكشف نفسه.

اختبار حي إضافي في `technician-monthly-statement.spec.ts` بيغطّي: عضو الطاقم
بيشوف نصيبه الصحيح، القائد بيشوف نصيبه هو بس مش الوعاء كله، استرداد جزئي بيتعكس بالنسبة الصح،
واسترداد طلب طاقم بيتعكس على كل مشارك حسب حصته، وكاش كامل، وإيداع أونلاين مع
باقي كاش، واسترداد جزئي لطلب كاش من غير محو تاريخ استلامه.

وفي صورة طلب الفني، `onlinePaidAmountCents` بقى يجمع الدفعات غير الكاش فقط. قبل الإصلاح، دفعة
`cash` نفسها كانت تدخل في `paidOnlineCents`؛ وبعد التحصيل يصبح المتبقي صفرًا فتظهر عبارة "مدفوع
أونلاين بالكامل" كذبًا. الاختبار الحي `cash-settlement-direction.spec.ts` يثبت الآن الحالات الثلاث:
كاش كامل، إلكتروني كامل، ومختلط يظل مختلطًا بعد تحصيل الكاش.

**توزيع الاسترداد على الطاقم**: `refundOrder()` يعكس الاسترداد على كل مشارك بنسبة حصته المثبتة في
`order_earning_shares`، وليس على القائد وحده. الاستردادات الجزئية تستخدم حسابًا تراكميًا
(`target_after - target_before`) لمنع فرق القروش من التراكم، وفرق التقريب الأخير يُحمّل على القائد.
عند استرداد 100% يرجع كل فرد حصته الأصلية بالكامل. الطلبات التاريخية التي لا تحمل صفوف حصص ترجع
للسلوك المتوافق القديم (القائد الفردي) بدل تعطيل الاسترداد.

## حماية الدفع من ضغط مزدوج/انقطاع (docs/08 §90.2، طلب مالك مباشر 2026-08-28)

تدقيق شامل على `payWithWallet()`/`payWithProvider()` (المشتركة بين card/fawry/instapay) ضد: ضغط
مزدوج، إغلاق التطبيق فجأة أثناء عملية جارية، وانقطاع الشبكة. **مش كله كان محتاج إصلاح** — الحماية
الأساسية (قفل الطلب `pessimistic_write` + `assertPayable()` تحت القفل) كانت موجودة وشغالة صح
فعلاً في `settleAndComplete()` و`finalizeGatewayWebhook()` و`refundOrder()`، وأكّدها الفحص:

- **الدفع بالمحفظة**: آمن تمامًا — حركة ذرّية واحدة جوّه transaction واحدة بقفل صف.
- **الاسترداد (`refundOrder()`)**: كان **مُصلَح من الأساس** (تعليق كامل في الكود بيوثّق بَقّة
  distributed-transaction اتصلحت قبل كده، docs/08 §19 بند 4) — نمط 3 مراحل بيمنع تجاوز المبلغ
  المتبقي أو استرداد مزدوج حتى بضغطتين متزامنتين على نفس الطلب. **مفيش تعديل مطلوب هنا**.
- **`assertPayable()` في `finalizeGatewayWebhook()`**: بيحمي من تسوية طلب اتسوّى بالفعل حتى لو
  فتحنا شحنتين مستقلتين عند البوابة (تحت) — الطلب مايتقفلش مرتين، مهما عدد الشحنات.

**الاختلافين الحقيقيين اللي اتلقيوا واتصلحوا**:

1. **سباق ضغطتين متزامنتين بنفس مفتاح idempotency** — الفحص `findOne({idempotencyKey})` check-then-act
   خارج أي قفل في `payWithWallet()`/`payWithProvider()`. القيد الفريد في الداتابيز كان بيمنع صف
   مكرر فعلاً، لكن الخاسر كان بياخد 500 خام بدل رد ناجح بنفس نتيجة اللي كسب. اتصلح: try/catch على
   الكود `23505` بيرجّع نتيجة اللي كسب السباق.
2. **الفجوة الأهم**: مفتاح idempotency بيتولّد ويتخزّن في **ذاكرة الشاشة بس** (مش تخزين دائم على
   الجهاز). لو التطبيق اتقفل فجأة أثناء دفع بكارت/فوري/InstaPay لسه معلّق عند البوابة، والعميل
   فتح التطبيق تاني وجرّب يدفع، بيتولّد مفتاح **جديد تمامًا** — وكان ممكن يفتح شحنة مستقلة تانية
   عند البوابة الحقيقية. الطلب في نظامنا محمي (webhook التاني هيرفضه لو الأول خلص)، لكن **فلوس
   العميل عند البوابة نفسها مش بترجع تلقائي**. اتصلح: `payWithProvider()` بيرفض يفتح شحنة جديدة
   لو فيه دفعة `PENDING`/`PROCESSING` لنفس الطلب اتعملت خلال آخر 5 دقايق (بغض النظر عن مفتاح
   idempotency)، برسالة واضحة "استنى نتيجتها أو جرّب تاني بعد شوية". نافذة 5 دقايق مقصودة: قصيرة
   كفاية إنها متمنعش عميل بدّل رأيه فعلاً (لغى الدفع وعايز طريقة تانية)، طويلة كفاية تغطي إدخال
   بيانات كارت طبيعي + إعادة فتح تطبيق بعد قفل مفاجئ.

اختبار حي جديد: `payment-double-submit-protection.spec.ts` (4/4) — يغطي: منع شحنة تانية لو فيه
دفعة حديثة نشطة، إكمال طبيعي لو مفيش، وسباق نفس المفتاح لكل من `payWithProvider`/`payWithWallet`.

**فجوة موثّقة صراحة (خارج نطاق الإصلاح الآمن في الجولة دي)**: لو العميل فعلاً كمّل **دفعتين**
حقيقيتين عند البوابة قبل ما الفحص الجديد يمنعه (نادرة جدًا — محتاجة يفتح شحنتين خلال نفس ثواني
معدودة قبل ما أي webhook يوصل)، مفيش استرداد تلقائي للشحنة الزيادة — بتفضل `PENDING` للأبد
ومحتاجة مراجعة يدوية عبر سجلات الـwebhooks الفاشلة (`markWebhookFailed`). حل جذري (auto-refund
لشحنة webhook اترفض لأن الطلب اتسوّى بالفعل من شحنة تانية) قرار تصميم أعمق يحتاج نظر منفصل.

## استرداد رسم المعاينة بعد زيارة حصلت فعلاً (ADR-0069)

`refundCancelledPrepaidOrder` كانت بترجّع **المبلغ كله** لأي إلغاء من حالة قابلة للإلغاء. المسار
اللي كشف الفجوة: خدمة معاينة في الموقع مدفوعة مقدمًا → الفني سافر وعاين وسعّر → الطلب بقى
`AWAITING_INITIAL_QUOTE_APPROVAL` → العميل لغى → رسم المعاينة رجع كامل مقابل زيارة اتعملت،
والفني ماخدش ولا مليم (تسوية أرباحه عند `WORK_COMPLETED` بس).

القرار في `orders/assessment-fee-refund-policy.ts` — **المصدر الوحيد** للحساب:
- `services.assessment_fee_refundable_after_visit` (**الافتراضي `true` = سلوك ما قبل ADR-0069
  بالحرف**) بيتاخد منه snapshot على الطلب وقت الإنشاء.
- «الزيارة حصلت» = فيه عرض سعر مصدره `technician_onsite` على الطلب.
- التقييم **بالصور** بيرجع كامل مهما كانت السياسة — قرار مالك محسوم (docs/08 §115 بند 9).
- الاسترداد بيبقى `PARTIAL`، والمحجوز بيتسجّل في الـaudit
  (`withheld_for_completed_visit_cents`، أو `order.refund_withheld_for_completed_visit` لو
  المحجوز غطّى المبلغ كله فمفيش استرداد أصلاً).

## ج-٢ (2026-09-09) — idempotency مالي مقاس حيًا، مش مستنتج من الكود

`node scripts/financial-idempotency-audit.js` بيمشي ستة سيناريوهات حقيقية على المكدس كامل
وبيقيس الثوابت من `wallet_transactions` نفسه: نفس المفتاح ×١٠ متزامن، نفس الطلب بمفاتيح مختلفة،
رصيد يكفي طلب واحد وطلبين متزامنين، نفس الـwebhook ×١٠، استرداد مزدوج متزامن، واسترداد جزئي بعد
الدفع. النتيجة الحالية ٨/٨ (التفاصيل والأرقام في `docs/29-e2e-production-readiness-audit-2026-09-08.md`).

**اتنين اتصلحوا نتيجة القياس ده:**

- **«نفس المفتاح = نفس النتيجة» كانت بتنكسر تحت التزامن.** تسعة من عشرة نداء بنفس المفتاح كانوا
  بياخدوا `409 الطلب مدفوع بالفعل` — لأن الخاسر بيقف على قفل صف الطلب و`assertPayable()` بترميه
  **قبل** ما يوصل لمحاولة الإدراج اللي كانت هتكسر القيد الفريد، والمعالجة كانت مقصورة على
  `isUniqueViolation`. دلوقتي `payWithWallet()` بتسأل تاني عن صف بنفس المفتاح بعد **أي** فشل.
- **`deadlock detected` كان بيوصل للعميل كـ`500`.** ترتيب قفل المحافظ الثابت مضمون **لكل نداء
  `doubleEntry` لوحده**، مش للـtransaction كلها اللي بتعمل أكتر من قيد على محافظ متداخلة. الحل:
  `common/db/transaction-retry.ts` — إعادة محاولة على `40P01`/`40001` بس، مربوطة في
  `payWithWallet` و`collectCash`. رد التزاحم بقى `402 رصيد غير كافٍ` زي المفروض.

**فجوة مفتوحة موثّقة**: تسع سويتات مالية هنا بتفشل بـ`EarningsPolicyService is required to
settle a unified earnings order` — بتبني `PaymentsService` بالإيد بلا الخدمة الاختيارية دي
بينما `settlement_policy_version` بقى `2` افتراضيًا. سابقة لج-١/ج-٢ (نفس الرقم بالظبط على
`884fd65` و`1a2a2dc`)، ومسجّلة كمهمة مستقلة.

## تدقيق مسارات الفلوس كلها (طلب المالك 2026-09-11) — `scripts/money-paths-audit.js`

> «عايزك تمشّي كل مسارات الفلوس: الداخلة والخارجة، العمولة، التوزيع على الفنيين والمساعدين
> والفرق، **والمساعد اللي قائد فريق لازم يتعامل بالظبط زي الفني**. وتتأكد إن أرقام الـKPI
> متطابقة مع اللوحة المالية والتقارير ولوحة الأدمن، وإن أي تغيير بيتعكس في كل حتة.»

السكربت بيبني خمس تشكيلات طاقم حقيقية، بيسوّيها **عبر الـAPI** (`collect-cash` ←
`settleAndComplete`)، وبيقارن كل سطح قراءة بالقاعدة: حصص الطلب، أعمدة الطلب، حركات المحافظ،
شاشتَي الأدمن (الحصص + الملخص المالي)، كشف حساب كل مشارك، اللوحة المالية، والصرف. **131 فحص،
كلهم نضاف.**

### البَقّة اللي اتكشفت: المساعد القائد ما كانش بياخد فلوسه أصلاً — الطلب كان بيتعلّق للأبد

ADR-0055 (طلب مالك صريح) بيقول بالنص: «القسمة بتشتغل على `participant_role` مش على
`technician_kind`… مساعد شايل طلب لوحده → **نصيب القائد الكامل**»، وبيختم بـ«**مفيش سطر كود
مالي اتغيّر**». الجملة الأخيرة دي كانت غلط: `EarningsPolicyService.resolveParticipants()` كانت
لسه بتقرا `technician_kind`:

```ts
row.technician_kind === 'assistant' || row.participant_role === 'assistant' ? 'assistant' : 'technician'
```

فالمساعد القائد كان بيوصل `calculateEarningsV2` بـ`earningRole = 'assistant'` و`isLeader = true`
— التركيبة اللي الحاسبة بترفضها صراحةً. المقاس حيًا **قبل** الإصلاح:

```
collect-cash → 409 PAY_003 «بيانات المنفّذين غير مكتملة»
حالة الطلب : work_completed / unpaid   الحصص: []
```

يعني: **لا العميل يقدر يدفع، ولا المساعد ياخد مليم، ولا المنصة تاخد عمولتها — للأبد.** مش خطأ
عرض، ولا فرق في التسعير: الفلوس كلها واقفة.

**الإصلاح** (نقطة واحدة، لأن الاشتقاق كله كان في مكان واحد):
1. `resolveParticipants` بقت تقرا `participant_role` بس — زي ما الـADR بيقول بالحرف.
2. حارس «المساعد لازم ياخد دور مساعد» في `earnings-calculator.ts` اتقيّد بـ**غير القائد**،
   فبيفضل شغّال على المساعد المنضم لطاقم حد تاني (وهو الغرض اللي اتكتب عشانه) ومش بيقف قدام
   المساعد اللي شايل الشغلانة كلها.

حماية المساعد كعضو طاقم مافقدتش: `resolveEffectiveMemberType` بيفرض `member_type = 'assistant'`
وقت الضم، فبيوصل بدور مساعد وتسعيرة مساعد زي ما هي.

بعد الإصلاح، المساعد القائد بياخد **نفس أرقام الفني القائد بالظبط** — متقاس في السيناريوهين
«د» و«هـ» وفي تستين regression بيقارنوا التشكيلتين رقم برقم.

### إيه اللي اتأكد إنه سليم

| المسار | الثابت المتقاس |
|---|---|
| الداخل مقابل الخارج | مدفوعات العميل الناجحة = عمولة المنصة + مجموع حصص المنفّذين |
| القسمة | مجموع الحصص + العمولة = إجمالي الطلب، على كل تشكيلة |
| المحافظ | القائد: حصته − الكاش اللي ماسكه (سالب = دَين مشروع). الأعضاء: حصتهم كاملة. المجموع + الكاش = وعاء المنفّذين |
| لوحة الأدمن | شاشة الحصص + الملخص المالي = القاعدة بالقرش |
| «مستحقاتي» | حصة كل مشارك، وصافي المستحق = أثر الطلب على محفظته بالظبط (§90.1) |
| اللوحة المالية | **فرق** لقطة قبل/بعد = بالظبط اللي الطلبات دي ضافته (مبيعات، إيراد منصة، أرباح فنيين، أرباح مساعدين) |
| الصرف | الحجز ينقل من المتاح للمحجوز، التنفيذ يخصم **الصافي**، وحركة المحفظة = −الصافي |

### مخالفات محفظة المنصة على قاعدة التطوير — انحراف اختبارات، مش عطل إنتاج

`/admin/analytics/money/reconciliation` بيقول ٢٣٤ مخالفة على قاعدة التطوير (١ فرق رصيد + ٢٣٣
انقطاع سلسلة)، **كلها على محفظة المنصة المشتركة**: الرصيد −1,490,054.06 ج.م مقابل مجموع حركات
−639,192.00 ج.م.

ده **مقيس مش مفترض**: تسوية واحدة عبر الـAPI حرّكت الرصيد +250.00 ومجموع الحركات +250.00 وصف
واحد — متطابقين تمامًا؛ و`clean-test-data.js` وحذف مستخدمي الهارنس مالمسوش أي رقم فيهم. السبب
إن ~٢٠ سبيك بتمسح `wallet_transactions` بتاعتها في التنظيف (`DELETE FROM wallet_transactions
WHERE …`) من غير ما ترجّع رصيد محفظة المنصة — وهي محفظة مشتركة مابتتمسحش أبدًا، فبتحتفظ بأثر كل
حركة اتشالت.

عشان كده التدقيق بيفحص **الدلتا** (المسارات دي ما ضافتش ولا مخالفة جديدة) مش الرقم المطلق.

> ✅ **اتقفل (2026-09-11)**: `wallet-cleanup.testing.ts` بقى المسار الوحيد لحذف حركات المحافظ
> في السبيكات — بيطرح أثر الصفوف المحذوفة من الرصيد **وبيعيد حياكة سلسلة** الحركات الباقية.
> الـ٢١ موضع اتحوّلوا ليه، والانحراف المتراكم اتصلح مرة واحدة بـ`scripts/repair-dev-wallet-drift.js`.
> العدّاد بقى **٠ / ٠ / ٠** بعد ما كان ٢٣٤.

## دورة الاسترداد — `scripts/refund-flow-audit.js` (طلب المالك 2026-09-11)

> «شوف فلو الاسترداد… كل الحالات الممكنة، الدورة مقفولة؟ فيه فلوس بتضيع في النص أو بتتحسب
> غلط؟ ولما النظام يقبل الاسترداد تلقائيًا لازم يبان للأدمن عشان يتابع، والفني يشوف تلميح
> بسيط **جوّه** الطلب — لازم يفتحه عشان يشوفه، مش بره.»

٦٣ فحص على سبع حالات، كلهم نضاف. الثابت الحاكم اللي بيتقاس على كل حالة:

> **اللي العميل دفعه − اللي رجعله = عمولة المنصة الصافية + حصص المنفّذين الصافية**

| الحالة | اللي اتأكد |
|---|---|
| استرداد كامل بعد التسوية | المنصة صفر، المنفّذين صفر، الطلب والدفع الاتنين `refunded` |
| استرداد جزئي ٤٠٪ | ٤٠٪ من **كل** وعاء — مش كلها من جيب طرف واحد |
| استردادين متتابعين | بيجمعوا للطلب كامل بالظبط، بلا انحراف تقريب (largest-remainder بيخلّيهم يتلسكبوا) |
| استرداد فوق المدفوع | بيترفض، وصفر مليم بيتحرّك |
| استرداد كامل مرتين | التاني بيترفض، المسترد مرة واحدة بس |
| طلب طاقم | كل مشارك بيتعكس بنسبة حصته هو، مش القائد لوحده |
| **الاسترداد التلقائي** | النظام بيعمله لوحده على طلب مدفوع مقدّمًا اتلغى، ومسجّل باسم النظام |

كمان: **مفيش صف استرداد عالق في حالة وسيطة** — أي صف `processing` بلا طريق خروج معناه فلوس
معلّقة، والفحص ده بيمسكها.

### الفجوة اللي اتقفلت: الاسترداد التلقائي ما كانش ليه أي سطح متابعة
...
إنك تفتح الطلب اللي **تشك فيه أصلاً**، فاسترداد النظام التلقائي كان عمليًا غير مرئي. وحتى لو
اتفتحت القايمة، العقد ماكانش فيه أي علامة تفرّق صف النظام عن صف الأدمن.

اللي اتعمل:
- العقد بقى فيه `is_automatic` (النظام هو اللي طلبه) و`needs_reconciliation` (فلوس معلّقة
  مستنية تثبيت نتيجة المزود، AUD-012) و`order_number` (القايمة من غيره أرقام بلا سياق).
- `GET /admin/refunds` بقى بيقبل `?automatic=true` و`?needs_reconciliation=true`.
- صفحة `apps/admin/src/app/refunds` جديدة، **الفلتر الافتراضي فيها «محتاج متابعة»** عمدًا:
  أول حاجة تتفتح لازم تكون الفلوس المعلّقة في النص.

### تلميح الفني جوّه الطلب

`has_customer_refund` في عقد الفني — **واقعة بلا أي رقم**، لأن docs/08 §60.2 بيمنع أرقام فلوس
العميل عن الفني (`refunded_amount_cents` و`total_amount_cents` محذوفين من العقد ده أصلاً).
الغرض إن الفني يفهم ليه مستحقه اتغيّر، مش إنه يعرف رجع للعميل كام.

> ⚠️ مصدر العَلَم `getTechnicianMoneyView` مش `base.refunded_amount_cents`. الحقل ده اختياري
> ومابيتعبّاش في مسار الفني خالص، فأول نسخة كانت بترجّع `false` دايمًا — **التدقيق الحي هو
> اللي مسك ده**، والتستات الوحدوية كانت هتعدّيه.

## InstaPay وسيلة الدفع الأساسية (طلب المالك 2026-09-11)

> «بما إن الخدمة اللي إحنا عايزين الناس تدفع بيها دايمًا هي InstaPay… عايزين InstaPay تبقى أول
> واحدة فوق وتحتها الكاش وبعد كده الباقي، ومكتوب جنبها إنها الأنسب. ورقم التليفون مكتوب بوضوح
> سطر لوحده عشان ما تتدخلش العربي مع الإنجليزي، ورقم الطلب ورقم التليفون بيتاخدوا copy.»

### الترتيب والترشيح — مصدر واحد

`GET /payment-channels` بقى بيرجّع القايمة **مرتّبة**: InstaPay، الكاش، وبعدين الباقي. وكل صف
معاه `is_recommended` و`recommended_label_ar` جاهز للعرض.

الترتيب في الكود (شكل عرض ثابت)، **والترشيح إعداد** (`payments.recommended_method`، migration
0319): لو المنصة قرّرت ترشّح وسيلة تانية أو توقف الترشيح، ده قرار تشغيلي من لوحة الأدمن بلا
deploy. والوسم مابيظهرش خالص لو الوسيلة نفسها مش متاحة — ترشيح حاجة العميل مش قادر يستخدمها
بيضايقه مش بيساعده.

### الأرقام حقول مستقلة — مش مدفونة في نص عربي

`instructionsAr` بقى **خطوات بالكلام بس**. الحساب والمبلغ ورقم الطلب رجعوا حقول مستقلة
(`recipient_address`, `recipient_name`, `amount_cents`, `reference_code`).

**ليه ده مش تجميل**: رقم لاتيني جوّه جملة عربية بيتعرض بترتيب خانات مقلوب (bidi)، فالعميل
بينسخه أو يكتبه غلط — على **تحويل بنكي حقيقي رايح لحساب**. الواجهة دلوقتي بتعرض كل رقم في سطر
مستقل بـ`Directionality(ltr)` ومعاه زرار نسخ.

### الاستئناف — `GET /orders/:id/instapay-transfer`

العميل **لازم** يسيب تطبيقنا ويفتح تطبيق البنك ويرجع؛ ده الاستخدام الطبيعي لـInstaPay مش حالة
شاذة. المسار ده **قراءة بحتة**: بيرجّع تفاصيل الدفعة المعلّقة بلا `Idempotency-Key` وبلا ما
ينشئ أي حاجة، فالعميل يقدر يفتح ويقفل الشاشة قد ما يحب.

المسارين (الإنشاء والاستئناف) بيقروا الحقول من **نفس الدالة** في `InstaPayProvider` — أي فرق
بينهم كان معناه إن العميل الراجع يشوف حساب غير اللي حوّل عليه.

وتطبيق العميل بيجرّب الاستئناف **قبل** ما يعمل دفعة جديدة: `_instapayIdempotencyKey` بيحمي من
الدوس مرتين في نفس الجلسة بس، ولو التطبيق اتقفل خالص كان الدوس تاني هيبدأ دفعة تانية.

### وعد وقت التأكيد — رقم صريح بدل تحذير

> «مش عايزين نبين للناس إن هي بتاخد وقت… ٢٠ دقيقة ده الطبيعي، وmaximum ساعة.»

النص القديم كان: «ممكن ياخد وقت أطول من الكارت/فوري» — يعني بيقارن وسيلتنا الأساسية بغيرها في
غير صالحها. دلوقتي: «التأكيد عادةً خلال ٢٠ دقيقة، وبحد أقصى ساعة»، والرقمين إعدادات
(`payments.instapay_confirm_typical_minutes` / `_max_minutes`) عشان لو أداء المراجعة اتغيّر
يتعدّل الوعد بلا deploy.

### العربون: العميل يختار

> «لما يكون الطلب فيه deposit، يكون فيه option برضه إن الشخص يدفع الطلب كامل… ومسموح له إن هو
> يدفع deposit والباقي بيدفعه كاش عادي جدًا بعد الشغل.»

`pay_full_amount` في `POST /orders`. لو اتبعت، `deposit_amount_cents` **مابيتحطّش على الصف
أصلاً**، فالطلب بيتصرّف زي أي طلب بلا عربون في كل المسار المالي (`amountOwedNow`، التحصيل،
الاسترداد) — **صفر فرع جديد في كود الفلوس**، وده كان الاعتبار الأول في اختيار التصميم ده.

### التحقق

`scripts/instapay-deposit-audit.js` — ٢٦ فحص حي على الـAPI: الترتيب، الترشيح، الحقول المستقلة،
إن الأرقام **مش** موجودة في نص التعليمات، الاستئناف (نداءين متتاليين = نفس الأرقام وصفر دفعة
جديدة)، ووعد الوقت. وكمان العربون: الافتراضي بيثبّت النسبة، و`pay_full_amount` بيلغيه فيتحوّل
الطلب كامل.

> ⚠️ التدقيق بيظبط الإعدادات عبر `PATCH /admin/settings/:key` مش بـSQL مباشر: `InstaPayProvider`
> بيكاش عنوان الـIPA في الذاكرة وبيعيد تحميله على `SETTING_UPDATED_EVENT` اللي
> `SettingsService.update()` بس بيطلقه. الكتابة المباشرة بتعدّي من تحت الكاش فالمزوّد يفضل
> `isConfigured = false` — وده اللي خلّى أول تشغيلة للتدقيق ترجّع 503 وهي سليمة.

### فجوة مفتوحة موثّقة: InstaPay مش موجودة في `customer-web` خالص

الويب بيبعت `prepayment_method: 'card'` بس، ومفيهوش شاشة تحويل InstaPay ولا اختيار وسيلة.
يعني عميل بيحجز من الموقع **مش قادر يستخدم وسيلة الدفع الأساسية للمنصة**. ده سابق للشغل ده
(مش انكسر هنا)، ومسجّل كمهمة مستقلة لأنه شاشة كاملة + اختيار وسيلة + استئناف — نفس حجم اللي
اتعمل في التطبيق.

## سياسة المستحقات — تغطية حية لطبقة حلّ السياسة (تدقيق 2026-09-11)

`earnings-calculator.spec.ts` بيغطّي الحسبة، و`earnings-policy.service.spec.ts` بيغطّي الخدمة
بـmocks. الطبقة اللي بينهم — استعلام `resolveParticipants()` اللي بيلمّ من ٧ جداول ويقرّر مين
بياخد كام — مكانش عليها أي تغطية حية. `earnings-policy-resolution.spec.ts` بيقفلها بـ٢٤ اختبار
على Postgres حقيقي.

**بَقّة اتلقطت منه**: `ORDER BY (tea.service_id = o.service_id) DESC` كان بيخلّي استثناء الشخص
**العام** يغلب استثناءه **المخصوص للخدمة** — لأن `service_id` بتكون NULL في العام،
و`(NULL = …)` بتطلع NULL، وPostgres بيرتّب الـNULLs الأول في `DESC`. التفاصيل الكاملة والأرقام
...
الفجوة القديمة («الويب بيبعت `card` بس») **اتقفلت**: `apps/customer-web` بقى فيه اختيار وسيلة
الدفع بترتيب السيرفر مع وسم الترشيح، وصفحة تحويل على `/orders/[id]/instapay` بتعمل
**استئناف-أو-إنشاء** (GET أول، وإنشاء لو ٤٠٤، وقراءة تانية لو ٤٠٩)، ومدخل «ادفع عبر InstaPay»
على صفحة الطلب. متغطّى في `scripts/instapay-deposit-audit.js` (٣٦/٣٦).

## الدفع أونلاين في أي وقت + حافز مرة واحدة لكل طلب (ADR-0091، docs/08 §143، طلب مالك 2026-09-13)

### القاعدة في سطر

العميل يقدر يدفع أونلاين **من لحظة فتح الطلب** — وهو بيدوّر على فني، والفني في الطريق، والشغل
شغّال — والحافز بيتاخد **مرة واحدة على الطلب** مهما اتقسّم الدفع.

### الخريطة

| الحتة | الدور |
|---|---|
| `PREPAYABLE_ORDER_STATUSES` | الحالات الحية اللي يجوز الدفع فيها قبل نهاية الشغل |
| `assertPayable(order, { allowPrepayment })` | البوابة. `false` (الافتراضي) = سلوك ما قبل ADR-0091 بالحرف |
| `assertAmountOwed(order, owed)` | بوابة المبلغ — بقت هي اللي بترمي «مدفوع بالفعل» بدل فحص الحالة |
| `handlePaymentConfirmed()` الفرع الثالث | بيسجّل الدفع **بلا** قفل الطلب وبلا توزيع أرباح |
| `PaymentConfirmedEffects.prepaidMidFlight` | بيمنع حدث الاكتمال الكاذب في مرحلة الآثار |
| `eligibleInstaPayDiscountCents()` | قاعدة «مرة واحدة»: حافز متمنوح، أو `payment_status = paid` ⇒ صفر |

### ⚠️ الخط الأحمر

`collectCash()` و`payWithWallet()` بينادوا `assertPayable()` وبعدها **`settleAndComplete()` بلا
شرط**. أي مسار بياخد `allowPrepayment: true` **لازم** يمرّ بـ`handlePaymentConfirmed()` — الفرع
الثالث هو اللي بيمنع قفل طلب لسه الفني شغّال فيه وتوزيع أرباحه بدري. الكاش والمحفظة **مقصود**
إنهم فضلوا على البوابة القديمة؛ ده مش سهو.

### التسوية بعدين، مش هنا

الطلب اللي اتدفع مبكرًا بيفضل شغّال طبيعي، وبيتسوّى في مكانه الطبيعي لما الشغل يخلص:
`PrepaidOrderSettlementListener` → `settleAlreadyPaidOrder()` (ADR-0015). مستحق صفر ⇒ تسوية
فورية وقفل؛ مستحق موجب (بند إضافي اتوافق عليه) ⇒ `AWAITING_PAYMENT` للدلتا. **صفر آلية جديدة
اتخلقت للتسوية.**

### بَقّة حقيقية اتلقطت في التدقيق الحي

`previewInstaPayTransfer()` كانت بتاخد `order.instapayDiscountCents` كـfallback للعرض. الحافز
بيفضل متسجّل على الطلب بعد الدفع، فكان بيتعرض تاني على **دلتا الزيادة** — «وفّر ٣٠ ج.م.» على
مبلغ مالوش خصم، وسعر مشطوب مخترع. الحساب نفسه كان صح (`amount_cents` كان الدلتا بالظبط)،
والعرض بس هو اللي كان بيكدب. اتربط الحافز المعروض بحالة الدفع.
