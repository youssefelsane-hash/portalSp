...
("مستحقاتي") و`GET /wallet` ("محفظتي") يطلعوا نفس الحقيقة. الفحص لقى **اختلافين حقيقيين** —
...
`participantRole` و`refundReversalCents`، لشفافية كاملة بدل ما الرقم يتغيّر من غير تفسير.

اختبار حي إضافي في `technician-monthly-statement.spec.ts` (11/11، كان 8/8) بيغطّي: عضو الطاقم
بيشوف نصيبه الصحيح، القائد بيشوف نصيبه هو بس مش الوعاء كله، استرداد جزئي بيتعكس بالنسبة الصح،
واسترداد طلب طاقم بيتعكس من القائد بس من غير ما يأثر على العضو.
