...
الواقع الفعلي وقتها)، وبيطرح استرداد مكتمل بنفس معادلة `refundOrder()` بالحرف
(`round(technician_earning_cents × amount_refunded / total_amount_cents)`)، بس على القائد لأن
الاسترداد الفعلي بيتعكس من محفظته هو بس (عضو الطاقم محفظته ما بتتلمسش وقت الاسترداد — سلوك
موجود بالفعل في `refundOrder()`، مش قرار جديد اتاخد هنا). حقلين جداد في `TechnicianStatementJob`:
...
اختبار حي إضافي في `technician-monthly-statement.spec.ts` (14/14) بيغطّي: عضو الطاقم
بيشوف نصيبه الصحيح، القائد بيشوف نصيبه هو بس مش الوعاء كله، استرداد جزئي بيتعكس بالنسبة الصح،
واسترداد طلب طاقم بيتعكس من القائد بس من غير ما يأثر على العضو، وكاش كامل، وإيداع أونلاين مع
...
**فجوة موثّقة صراحة (مش اتصلحت في الجولة دي)**: الاسترداد نفسه (`refundOrder()`) بيعكس **كل**
مبلغ الاسترداد من محفظة القائد بس، حتى لو الوعاء كان متقسّم على طاقم — يعني في شغلانة طاقم
مستردة، القائد بيتحمّل كل العكس بينما الأعضاء يفضلوا ماسكين نصيبهم كامل. ده سلوك موجود من الأساس
(مش حاجة اتغيّرت في الإصلاح ده)، والكشف دلوقتي بيعكسه بأمانة (يطابق المحفظة فعليًا) بدل ما
يخترع رقم "عادل" مختلف عن الواقع. تغيير سلوك الاسترداد نفسه (تقسيم العكس على الطاقم) قرار مالي
أعمق يحتاج نظر منفصل، مش تحت مسمى "تطابق العرض".
