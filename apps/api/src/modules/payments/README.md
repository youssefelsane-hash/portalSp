...
- **بَقّة مالية حقيقية اتلقطت واتصلحت — `WalletsService.releaseReservation()` كانت بتسمح برفض صرف مكرر يخترع فلوس (docs/08 §20 بند 9)**: `finalizePayout()` (بتنادى من `adminComplete()`) عندها فحص واضح `if (wallet.reservedBalanceCents < amountCents) throw` قبل ما تحرك أي فلوس. `releaseReservation()` (بتنادى من `adminReject()` بس) كانت **من غيره** — بتطرح `amountCents` من `reservedBalanceCents` وتضيفه لـ`balanceCents` من غير أي تحقق. لو `adminReject()` اتنادت مرتين على نفس الصرف `UNDER_REVIEW` (double-click من الأدمن، إعادة محاولة بعد timeout شبكة) — النداءين بيعدّوا فحص `payoutStatus` (بيقروا الحالة القديمة قبل ما أي حد يلتزم)، بيتسلسلوا على قفل المحفظة (`lockWallet`)، والنداء التاني كان بيضيف نفس المبلغ **تاني** لرصيد الفني — فلوس حقيقية من العدم، و`reservedBalanceCents` يروح سالب بصمت. **الإصلاح**: نفس فحص `finalizePayout()` بالظبط، سطر واحد. **اتأكدت البَقّة حياً بالـrevert المؤقت**: من غير الفحص، اختبار حقيقي على Postgres رجّع `balanceCents=170000` بدل `150000` الصح. اتعمله اختبار حي: `payout-double-release.spec.ts` (2 اختبار — `adminReject()` مرتين على نفس الصرف، و`releaseReservation()` مباشرة بلا حجز كافٍ).
...
**اتأكد حي بالكامل** (`refund-transaction-safety.spec.ts`، 6 اختبارات ضد Postgres حقيقي، provider
