...
- **إشعارات المشروعات الدائمة (`migration 0198`)**: كل تغيير حساس في المشروع (إنشاء، إرسال/اعتماد
  ...
  `in_app` و`push` بعد الـcommit، ويسترجع تلقائيًا أي صف `processing` عالق، ويعيد المحاولة بحد
  أقصى خمس مرات ثم ينقله إلى `manual_review`. العمود `notifications.source_outbox_id` مع المفتاح
  الفريد `(source_outbox_id,user_id,channel)` يمنع تكرار الإشعار لو العملية وقعت بعد إنشاء صف
  الإشعار وقبل تعليم الـoutbox بأنه اتسلّم. العميل والأدمن بياخدوا رابط الشاشة المناسبة؛ الفني
  يستلم التحديثات التشغيلية المرتبطة بطلب مشروع من غير رابط عميل غير صالح. التعليق الداخلي لا
  يخرج للعميل. فشل بوابة push لا يمسح صف الإشعار: حالة القناة وسبب الفشل يظلان مسجلين، بينما
  `in_app` هو السجل الدائم داخل التطبيق.
...
الرسالة الافتراضية ("بندوّرلك على أقرب فني متاح") كانت هتبقى مضلّلة للحالة دي — الفني معروف
ومتأكّد بالفعل. الحل: فرع `isDomesticWorkerAssignment = order.domesticWorkerProfileId !== null`
بيغيّر العنوان/النص لـ"حجزك اتأكّد" بدل ما يحتاج event/listener جديد بالكامل — تفاصيل الشريحة
...
## ADR-0067 — أربع قرارات في مسار التقييم كانت بتحصل في سكوت (§115 بند 15)

بنود 7/8/10 خلّوا الأدمن يقدر يفرز التقييم ويقرر، والفني يعدّل السعر بعد التشخيص. الكود كان صح
تشغيليًا، بس أربع نقط ماكانش ليها **أي** إشعار:

| القرار | اللي كان بيحصل | اللي بقى بيحصل |
|---|---|---|
| سعر خارج النطاق اتبعت | يتحجز في `pending_admin_review` من غير أي حدث — طابور «طلبات التقييم» هو قناة الاكتشاف الوحيدة | `ORDER_QUOTE_ABOVE_RANGE_SUBMITTED_EVENT` → توجيه لـ`ops_manager` |
| الأدمن رفض السعر | مفيش حدث خالص (المسار مابيغيّرش حالة الطلب) — الفني مطلوب منه سعر جديد ومحدش قاله | `ORDER_QUOTE_ABOVE_RANGE_DECIDED_EVENT` → الفني اللي بعت العرض، في الاعتماد والرفض |
| عرض انتهت صلاحيته | مفيش كاسح أصلاً — العرض مايتعلّمش `expired` غير لما العميل يحاول يوافق متأخر | `QuoteExpiryService` كل دقيقة → `ORDER_QUOTE_EXPIRED_EVENT` → العميل + الأدمن + الفني |
| التحويل لمعاينة في الموقع | `SEARCHING_TECHNICIAN` مش في `CUSTOMER_MESSAGES` (وده صح للتوزيع العادي) — فالعميل ماكانش بياخد ولا إشعار رغم إن **رسم معاينة اتضاف على طلبه** | `ORDER_ROUTED_TO_ONSITE_ASSESSMENT_EVENT` → إشعار بيذكر الرسم بالجنيه |

الأربعة بيتبعتوا **بعد** نجاح الترانزاكشن، ومكتوبين كأحداث دومين مخصوصة مش كتغييرات حالة — تلاتة
منهم مش انتقالات حالة أصلاً، والرابع حالته الجديدة بيتشاركها مع مسار مالوش إشعار.

**فجوة اتقفلت في نفس الشغل**: `routeToRole()` كانت بتكتب صف `in_app` لكل موظف في الدور المستهدف
**من يوم ما اتبنت، ومافيش أي شاشة في لوحة الإدارة كانت بتقراه**. كل التوجيهات (نقص طاقم، طوارئ،
