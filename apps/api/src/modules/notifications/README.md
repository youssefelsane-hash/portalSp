...
- **الاستماع للأحداث** (`listeners/`): بدون أي استدعاء مباشر من الموديولات التانية — كل حاجة عبر `EventEmitter2`:
  ...
  - `order.status_changed` (حدث جديد اتضاف في `orders.service.ts` لكل انتقال حالة فني أو إلغاء عميل) → إشعار العميل عند `technician_on_way`/`technician_arrived`/`in_progress`/`work_completed`، وإشعار الفني عند `cancelled_by_customer`.
