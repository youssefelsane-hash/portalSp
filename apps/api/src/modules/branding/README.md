...
Super Admin بيرفع/يستبدل أصول البراندنج (لوجو أساسي، رمز مختصر، وصورة الشاشة الرئيسية)
من لوحة الإدارة — بلا أي تعديل كود ولا deployment. مفيش hardcoding لملف لوجو
...
`assetType` واحد من: `primary_logo`, `logo_mark`, `splash`.
...
كان فيه **ستة** `asset_type`. بقوا **اتنين** في migration 0210، ثم عاد `logo_mark` فقط في
migration 0327 مع مستهلك فعلي داخل شِل لوحة الإدارة.

`logo_light`/`logo_dark`/`login_logo` اتشالوا لأن تتبّعًا فعليًا (grep على كل
...
قبله. `logo_mark` هو أول تطبيق فعلي للقاعدة دي (ADR-0092).
...
## `home_banner` — بانر الشاشة الرئيسية (ADR-0095، migration 0346)

طلب المالك: «تضيفلي مكان أحط فيه الصورة… بالهيئة المستطيلة الجميلة دي… وتترفع من يا إما
البراندينج يا إما الواجهة الرئيسية».

**اتحط في البراندينج مش في «الواجهة الرئيسية»** لأن ده الموديول الوحيد اللي عنده مسار **رفع
ملف** حقيقي (تحقق أبعاد/حجم + تخزين + رابط بيتولّد وقت القراءة). صفحة الواجهة الرئيسية بتاخد
**روابط** بس، فكانت هتسيب المالك يدوّر على استضافة بره المنصّة.

**مالوش محتوى افتراضي حقيقي**: العنصر النائب في `branding-defaults.ts` موجود بس عشان
`GET /branding` يفضل عقده ثابت (نفس قاعدة ADR-0014)، والواجهات بتخفي القسم كله طالما
`is_default = true`. يعني إضافة النوع لوحدها **صفر تغيير مرئي** لحد ما الأدمن يرفع صورة.

الاستهلاك: `home_screen.dart` بيرسمه بـ`NetworkImageBox` (نسبة 3:1) — ودي اللي فيها الحماية
اللي المالك طلبها بالاسم: `frameBuilder` بدل `loadingBuilder` (مفيش وميض مع إعادة البناء)،
`gaplessPlayback` (مفيش فراغ أبيض عند تغيير الرابط)، `cacheWidth`، وعنصر نائب بدل أيقونة
الصورة المكسورة. وفي `customer-web/src/app/page.tsx` بـ`aspect-[3/1]` فالمساحة محجوزة من أول
رسمة ⇒ مفيش قفزة تخطيط.
