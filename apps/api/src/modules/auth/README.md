...
## وضع اختبار الـOTP — مؤقت لفترة Google Play Testing (docs/08 §173)

**السياق**: CEQUENS بيحتاج ~٣٠ يوم تحقّق على الدومين قبل ما الـSMS يشتغل فعلًا. خلال النافذة دي
لازم مختبري Google Play (Internal/Closed Testing) يقدروا يدخلوا التطبيق، وده مستحيل من غير كود
بيوصل. الحل: السيرفر يقدر **يصدر كود ثابت ويوقف إرسال الـSMS** — بقرار من بيئة السيرفر وحدها.

### القرار المعماري المهم: الوضع بيأثر وقت **الإصدار**، مش وقت **التحقق**

ده مش تفصيلة تنفيذ — ده اللي بيخلي الميزة دي مش ثغرة. التنفيذ البديهي (وضع فرع في
`consumeOtpLocked` بيقول «لو الكود == 111111 اقبله») كان هيبقى **بايباس حقيقي**: بيتخطى الصلاحية
وعدّاد المحاولات وقاعدة «كود واحد صالح» والقفل التشاؤمي كلهم مرة واحدة، ولو الإعداد اتسرّب بأي شكل
النتيجة «أي حد يدخل أي حساب».

اللي اتعمل بدله: `requestOtp()` بيغيّر **الكود المولَّد** بس (بدل `randomInt` بياخد القيمة الثابتة)،
وبيرجع بدري قبل نداء بوابة الـSMS. الكود بيتهَش بـbcrypt وبيتحفظ في `otp_codes` **زي أي كود تاني
بالظبط**. `consumeOtp`/`consumeOtpLocked` **مالهمش ولا فرع** للوضع ده — مش «اتأكدنا إنهم شغالين»،
دول حرفيًا نفس الكود. يعني:

- الصلاحية، `attempts_count`/`max_attempts`، إبطال الكود الأقدم عند إعادة الإرسال،
  `pg_advisory_xact_lock`، `SELECT ... FOR UPDATE`، وكشف الكود المُستبدَل — كلهم شغالين **بالبناء**.
- الـ`@Throttle` على `otp/request` (٥/دقيقة) و`otp/verify` (١٠/دقيقة) ما اتلمسوش.
- أسوأ نتيجة لإعداد غلط = «كود الدخول متوقّع»، **مش** «أي كود بيتقبل». فرق الدرجة ده مقصود.
- الشيل بعد ما CEQUENS يشتغل = ملف واحد + فرعين في دالة واحدة.

### مكان مفتاح التشغيل

| | |
|---|---|
| المنطق كله | `src/modules/auth/otp-test-mode.ts` (ملف واحد — `usesFixedOtp` هي كل القرار) |
| نقطة التأثير الوحيدة | `auth.service.ts` → `requestOtp()` بس |
| قراءة الإعداد | `src/config/configuration.ts` → `otp.testMode` |
| حارس الإقلاع | `src/config/env.validation.ts` (آخر `.custom()` في السلسلة) |
| متغيّرات البيئة | `OTP_TEST_MODE`, `OTP_TEST_MODE_CODE`, `OTP_TEST_MODE_PHONES` |

### حارس الإقلاع — ليه فشل إقلاع مش تحذير

`NODE_ENV=staging|production` + `OTP_TEST_MODE=true` ⇒ **السيرفر بيرفض يقلع**. متغيّر بيئة منسي في
لوحة الاستضافة بعد انتهاء الاختبار سيناريو واقعي جدًا، والفصل بين «اختبار» و«إنتاج» مابيتسابش
لمراجعة بشرية. فشل الإقلاع بيتصلّح في دقيقة؛ بايباس صامت في الإنتاج ممكن يفضل شهور.

`isProductionLikeEnv` (مش `=== 'production'`) مقصودة: النشر الحقيقي شغّال بـ`NODE_ENV=staging` —
الفحص ضد `'production'` لوحدها كان هيخلي الحارس بلا أثر بالظبط في البيئة اللي بتخدم مستخدمين حقيقيين.

### الـclient مالوش أي دور

التطبيقات (`customer-app`, `technician-app`) و`customer-web` **ما اتغيروش ولا سطر**. الـ`RequestOtpDto`
فيه `phone_number` و`purpose` بس — مفيش حقل يقدر يفعّل الوضع، ومفيش أي مؤشّر في الواجهة إن في وضع
اختبار أصلًا. تجربة المستخدم مطابقة تمامًا: رقم → شاشة كود → إدخال.

### قايمة الأرقام (`OTP_TEST_MODE_PHONES`)

فاضية = أي رقم بياخد الكود الثابت — **مقبول بس لو قاعدة بيانات الاختبار منفصلة عن الإنتاج**. لو
بتشارك بيانات الإنتاج، املا القايمة بأرقام المختبرين بس. المقارنة بتتم بعد التطبيع لـE.164، وأي
صيغة مش قابلة للتحليل (زي بادئة `0020`) بتفضل زي ما هي فمابتطابقش — يعني **الفشل مقفول**: المختبر
بياخد كود حقيقي بـSMS بدل ما القايمة تتفتح لحد بالغلط.

### الإثبات

`otp-test-mode.spec.ts` — ٢٠ اختبار: الكود الثابت بيتقبل، أي كود تاني بيترفض، بوابة الـSMS
مابتتنداش خالص، الوضع مقفول بيرجّع المسار الحقيقي، الـclient مالوش تأثير، عدّاد المحاولات والصلاحية
وإلغاء الكود الأقدم لسه شغالين، الـallowlist بيشتغل في الاتجاهين، وحارس الإقلاع بيرفض
staging/production ومابيكسرش إنتاج سليم.

---

## أدوار الحساب (ADR-0110) — الدور النشط مش نوع المستخدم

`users.user_type` بقى **نوع التوظيف** (موظف ولا مستهلك)، و`user_role_grants` هو مصدر الحق للأدوار
الاستهلاكية اللي الحساب يقدر يشتغل بيها. الملفات:

| الملف | مسؤوليته |
|---|---|
| `entities/user-role-grant.entity.ts` | `AccountRole` (`customer`/`technician`) + الجدول |
| `account-roles.service.ts` | `resolveActiveRole` (القرار الأمني المركزي)، `grantRole`، `revokeRole` |
| `strategies/jwt.strategy.ts` | فحص **حي** إن الدور النشط لسه ممنوح — على كل طلب |
| `infra/migrations/0363` | الجدول + تريجرين منع الموظفين + نقل الحسابات الموجودة |
| `infra/migrations/0364` | `refresh_tokens.active_role` — التدوير مايوسّعش الدور |

### `RolesGuard` مالوش أي علم بده — وده مقصود

الحارس لسه بيقارن `request.user.userType` بقايمة `@Roles(...)` بلا أي تعديل. اللي اتغيّر هو **معنى**
القيمة: بقت «الدور النشط في الجلسة» بدل «نوع المستخدم الأبدي». ٥٦ موضع في الكود بيقروا `userType`
وماحتاجوش أي تعديل — لو الحل كان بتغيير الحارس، أثر التغيير كان هيكبر بلا داعي حقيقي.

### `role` من الـclient = طلب، مش صلاحية

`PinLoginDto.role` و`VerifyOtpDto.role` اختياريين. السيرفر بيتحقق من المنحة قبل ما يوقّع التوكن،
فعميل بيبعت `role: 'technician'` ياخد `AUTH_008` مش صلاحيات فني. والغياب بيرجع لـ`user_type` —
توافق خلفي كامل مع أي نسخة تطبيق منشورة.

### الفرق بين الدورين مقصود

دور **العميل** بيتمنح تلقائيًا عند أول دخول من تطبيق العميل: مالوش أي تحقّق أصلاً (أي حد يعمل حساب
من الصفر)، فخطوة إضافية عرقلة بلا مقابل أمني. دور **الفني** بيتطلب صراحةً بـ`POST /auth/roles/technician`
لأنه بيتحقّق منه (KYC، مستندات، اعتماد أدمن) — منحة تلقائية معناها حساب `pending` بيتولد لأي حد
فتح التطبيق بالغلط. والنداء ده **مابيوسّعش** الجلسة الحالية: المستخدم لازم يعمل دخول من تطبيق
الفني عشان ياخد جلسة بالدور الجديد، فتوكن مسروق مايرقّي نفسه في نداء واحد.

### الحُرّاس الناقصة — اتقفلت

`/addresses` كان **مالوش `@Roles` ولا تعليق يشرح ليه**، وده كان السبب المكانيكي إن الفني بياخد
تطبيق عميل «نصّ شغّال» (بيضيف عنوان بنجاح وبعدين يتصدّم في `/orders` اللي عليها حارس). بقى
`@Roles(UserType.CUSTOMER)`.

`/wallet` **كانت شبهة غلط**: عليها `@Roles(CUSTOMER, TECHNICIAN)` بالفعل، فوصول الفني ليها شرعي.

الباقي بلا حارس **بقصد وبتعليق في كل ملف**: `notifications` (+الأجهزة والتفضيلات)، `me/referrals`،
`loyalty`/`promo-codes/:code/validate` — خصائص شخصية مقيّدة بـ`user_id` ومحتاجة تشتغل للدورين.
القاعدة للي بعدنا: **غياب `@Roles` لازم يكون مكتوب إنه مقصود**، وغير كده يبقى سهو.

---

## تصريح «حُط رمزك» — مصدر واحد للاسترجاع والتنشيط (ADR-0111)

`pin-setup.ts` فيه `issuePinSetupCode()` — **الميكانيزم الوحيد** لأي حساب محتاج يعيّن رمز وهو مش
قادر يدخل:

| الحالة | المُنادي | `clearExistingPin` |
|---|---|---|
| مستخدم نسي رمزه | `AuthService.adminResetPin` | `true` (الرمز القديم لازم يموت) |
| **موظف جديد** الأدمن عمله | `AdminEmployeesService.create` | `false` (مفيش رمز أصلاً) |

الاتنين بيتستهلكوا من **نفس** `POST /auth/pin/reset/redeem`، والمستخدم **بيختار رمزه بنفسه**.

**ليه دالة حرة مش خدمة**: `AuthModule` بيستورد `AdminModule` (PermissionsService لـMfaPolicyService)،
فحقن `AuthService` في موديول الإدارة كان بيعمل دايرة محتاجة `forwardRef` — والكودبيس بيتجنّبه عمدًا.
الدالة بتاخد `EntityManager` فالمُنادي بيتحكم في المعاملة، وكل الأطراف بتشارك نفس الكود.

**الطريق المسدود اللي اتقفل**: موظف اتعمل بـ`pin_hash = NULL` مكانش يقدر يدخل بأي طريقة — الدخول
برمز `401`، OTP `410` (`login_method='pin'`)، تسجيل جديد `409`، `POST /auth/pin` `401` (محتاج
جلسة). دلوقتي الكود بيتولّد **جوّه معاملة الإنشاء** فيستحيل يبقى فيه موظف بلا مدخل.

## الـPasskey بيطلب تحقق المستخدم (ADR-0111 §4)

`userVerification: 'required'` في التسجيل والدخول، و`requireUserVerification: true` **صريحة** في
نداءات التحقق (الافتراضي في @simplewebauthn v13 هو `true`، بس ترقية تقدر تغيّره وساعتها Passkey
بلا بصمة يعدّي في صمت).

كانت `'preferred'`، وده **مكانش ثغرة** (السيرفر كان بيرفض بلا `flags.uv`) لكنه كان **تفاوت بيولّد
فشل غامض**: العميل يتخطى التحقق، السيريمونى تخلص، وبعدين السيرفر يرفض برسالة عامة ومفيش مخرج.

**الأجهزة الضعيفة مش مقفولة**: `residentKey: 'required'` + مفيش `authenticatorAttachment` مقيّد =
بصمة محلية، أو QR من الموبايل (`hybrid`)، أو مفتاح أمان بـPIN — كلهم متاحين.

---

## تحقّق رقم العميل عند أول طلب (ADR-0112)

| الملف | مسؤوليته |
|---|---|
| `phone-verification.policy.ts` | **الشرط** — دوال حرة بلا DI يشاركها موديول `orders` وموديول `auth` |
| `phone-verification.service.ts` | إصدار الكود، تأكيده، وتغيير الرقم |
| `AuthService.issueOtpCode` | **نواة** إصدار الـOTP، اتطلّعت من `requestOtp` عشان المسارين يشاركوها |

**مفيش حالة جديدة**: `users.phone_verified_at` لوحده بيعطي «أول طلب بس». التسجيل بالرمز بيسيبه
`NULL` (ADR-0109 §4) والتسجيل القديم بالـOTP كان بيحطّه، فالحسابات الحالية **مابتتسألش**.

**البوابة على السيرفر** (`OrderCreationService.create`) مش في الواجهة — نقطة خنق واحدة بتغطي كل
كلاينت. الرفض `AUTH_009` + `reason: 'phone_verification_required'`.

**المسار العام للـOTP مافتحش**: `POST /auth/otp/request` لسه `410` لما الدخول بالرمز. مسار التحقّق
بينادي `issueOtpCode` من جوّه، فمفيش سطح عام جديد.

**تغيير الرقم = عاملين مستقلين**: رمز الدخول الحالي (إنه صاحب الحساب) + كود على الرقم الجديد (إنه
بيتحكم فيه). واحد منهم لوحده مش كفاية — جلسة مسروقة كانت تقدر تحوّل الرقم وتاخد الحساب.

**التحقّق العادي مابيطلبش رمز**: الكود رايح لرقم صاحب الحساب، فجلسة مسروقة عمرها ما هتشوفه.

## سقف تسجيل الحسابات (`registrationThrottleLimit`)

`POST /auth/pin/register` محسوب بالـ**IP** (main 61acc31d) مش بالرقم — التسجيل مفتوح لأي رقم فالرقم
مايصلحش حد ضد إنشاء حسابات جماعي. **بس ده كسر السويتات الحية** (+٢٠ ملف بيسجّلوا من نفس localhost).
السقف بقى قابل للرفع بـ`AUTH_REGISTRATION_THROTTLE_LIMIT` **بره الإنتاج بس** — الدالة نفسها بتفرض
ده، فمستحيل الإنتاج يتخفّف بمتغيّر بيئة. للاختبار الحي: شغّل الـAPI بـ
`AUTH_REGISTRATION_THROTTLE_LIMIT=5000`.
