...
`/addresses` و`/wallet` **مالهمش `@Roles`**، وده كان السبب المكانيكي إن الفني كان بياخد تطبيق عميل
«نصّ شغّال». مع ADR-0110 بقت غير مؤذية لأن الدور النشط في التوكن بيحدد السياق فعلاً، **بس الحارس
لسه ناقص** — أي مسار حساس يتضاف جنبهم بلا `@Roles` بيرجّع نفس فئة البَقّة. إضافتهم محتاجة فحص كل
مستهلك في التطبيقين الأول (docs/08 §180).
