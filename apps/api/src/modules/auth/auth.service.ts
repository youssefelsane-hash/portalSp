...
export class AuthService {
  ...
  async requestOtp(dto: RequestOtpDto, requestIp: string | null): Promise<{ expires_in_seconds: number }> {
    ...
    const result = await this.smsDispatcher.send({
      ...
      // ⚠️ **النص ده محسوب بالحرف عشان يدخل في رسالة SMS واحدة** — متغيّرش من غير ما تعيد القياس.
      //
      // أي نص فيه عربي بيتبعت بترميز **UCS-2**، وحدّه **٧٠ وحدة UTF-16** للرسالة الواحدة
      // (و٦٧ لكل جزء في الرسالة المجزأة، لأن رأس التجزئة بياخد من المساحة). النص القديم
      // (`كود التحقق — OSTA\nكودك: … — صالح لمدة … دقيقة. متشاركوش الكود ده مع حد.`) كان
      // **٧٦ وحدة** — يعني **جزئين، وتكلفة مضاعفة على كل كود تحقق**، وهو أكتر رسالة بتتبعت
      // في المنصة كلها. وزيادة ٦ وحدات بس هي اللي كانت بتعمل كده.
      //
      // النسخة دي **٦١ وحدة** مع كود من ٦ أرقام ومدة من رقم واحد — فاضل ٩ وحدات هامش يستحمل
      // `OTP_EXPIRY_MINUTES` لحد ٤ خانات. `otp-sms-length.spec.ts` بيقيس ده فعليًا على مدد
      // مختلفة، فأي تعديل بيكسر الحد بيرسّب قبل ما يشحن.
      //
      // **ليه العنوان والنص منفصلين**: بوابات الـSMS (CEQUENS وTwilio) بتلزق الاتنين بسطر
      // جديد بينهم — فالقياس لازم يشمل العنوان والسطر الجديد، مش النص لوحده.
      titleAr: 'أسطى',
