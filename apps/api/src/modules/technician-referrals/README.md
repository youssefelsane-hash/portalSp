...
## رابط الـQR ولماذا الرفض يُشرح (docs/08 §165)

### الـQR بيشفّر رابط مش توكن

`GET /t/:token` (خارج البادئة العامة، نفس `/p/:code` و`/r/:code`) بيحوّل الزائر للمتجر أو
صفحة الهبوط حسب جهازه، والكود ماشي معاه في `?tref=`. قبل كده الـQR كان بيشفّر التوكن الخام،
فالعميل اللي بيصوّره بكاميرا الموبايل العادية بيشوف `TECH-000004` ومايعرفش يعمل بيه إيه.

الوجهة نفسها مشتركة مع كل رابط ذكي (`common/marketing/smart-link-destination.ts`) — مفيش
نسخة تالتة من منطق «أندرويد للمتجر وغيره للهبوط».

**مفيش تسجيل زيارات هنا عن قصد**: ترشيح الفني مالوش جدول زيارات، والغرض إن المسح يوصّل
لمكان مفيد مش إنه يقيس حملة. لو اتطلب القياس، مكانه جدول زي `promo_code_link_hits`.

### سبب الرفض بيتعرض للفني

قواعد الرفض كلها مقصودة وحماية حقيقية من الحسابات الوهمية:

| القاعدة | الإعداد |
|---|---|
| أول طلب مؤهّل لنفس العميل بس | `referral_qr.reward_mode = first_order_only` |
| نفس الجهاز لعميل تاني اتكافأ عليه، **أو نفس جهاز الفني** | `referral_qr.reject_duplicate_device` |
| قيمة الطلب أقل من الحد | `referral_qr.min_order_amount_cents` |
| فترة تهدئة من آخر مكافأة | `referral_qr.min_minutes_between_bonuses` |
| تجاوز الحد الشهري | `referral_qr.max_monthly_bonus_cents_per_technician` |

**⚠️ القواعد دي ماتتخفّفش.** اللي اتصلّح إن `rejection_reason` — المتخزّن والمرجّع في
`TechnicianReferralBonusResponseDto` من زمان — بقى **معروض في تطبيق الفني**. قبل كده الفني
بيشوف «مرفوضة» وبس، فمايعرفش عمل إيه غلط ولا إيه اللي يمنع تكرارها.

**ملاحظة للاختبار**: تجربة الترشيح بحساب فني وحساب عميل على **نفس الموبايل** بتقع في قاعدة
الجهاز المكرر وبترجّع «نفس جهاز الفني حاليًا». ده سلوك صحيح مش بَقّة.
