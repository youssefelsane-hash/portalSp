...
| الإيراد النسبي | الحصة الشخصية في `order_earning_shares` (أو حصة القائد التاريخية للطلبات القديمة) ناقص قيود Refund المدينة الفعلية لنفس الطلبات — مقارنة بمتوسط المشاركين الشهر ده، وتشمل الكاش وأعضاء الطاقم |
...
الحصة الإجمالية هي المستخدمة في KPI حتى في طلب الكاش، لأن القائد قبضها خارج المحفظة ثم ظهر أثر
عمولة المنصة كمديونية داخل المحفظة. الاعتماد على `ORDER_EARNING` credits وحدها كان يخفي طلبات
الكاش وأرباح أعضاء الطاقم. أما Refund فيُطرح من قيود دفتر المحفظة الفعلية لكل مشارك، فلا يُخصم من
شخص لم يتحمل الاسترداد ولا يظل دخل مسترد محسوبًا كإيراد.
...
- فني شاف ملخّصه الشخصي (`GET /technician/kpi`) — تظهر له الحالات التي راجعتها الإدارة فقط
  (`approved`/`paid`/`rejected`) ولا يرى snapshot ما زال `calculated`. و`approval_notes` ظهرت `null` رغم إن الأدمن كتب
