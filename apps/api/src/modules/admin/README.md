...
- اتعمله اختبار end-to-end فعلي: `dashboard/stats` قبل أي طلبات (كله أصفار)، دورة كاش كاملة حقيقية (طلب → قبول → اكتمال → تحصيل كاش)، وبعدها الإحصائيات والتقرير طابقوا الأرقام الحقيقية في القاعدة بالظبط (إيراد 200 جنيه، عمولة 30 جنيه = 15%)، تقييم وشكوى حقيقيين ظهروا في `average_rating`/`complaints_open`، فلترة تاريخ فاضية رجّعت مصفوفة فاضية مش خطأ، `group_by` غلط اترفض بوضوح، وعميل/فني اتمنعوا من كل مسارات التقرير (403). `users`/`financial.pending_payouts` اتأكدوا حياً مطابقين لعدّ مباشر بـ`psql` بالظبط (14 مستخدم: 9 أدمن/4 فني/1 عميل، صرف واحد تحت المراجعة بقيمة 1500 جنيه). `reports/technicians` اتأكد إن متوسط زمن الرد (`9.78474` ثانية لفني حقيقي) طابق حساب يدوي بالظبط لصفّين `accepted` بس (18.55 و1.02 ثانية) وتجاهل صف `timeout` (30 ثانية) عن قصد، وإن الترتيب بـ`sort_by` غلط اترفض بوضوح (`VAL_001`) بدل ما يوصل لأي SQL خام.
