...
- **`service_addons`**: إدارة كاملة عبر `POST/PATCH/DELETE /admin/services/:id/addons` و`/admin/services/addons/:addonId` (soft-delete حقيقي عبر `@DeleteDateColumn`)، بالإضافة لـ `GET /services/:id/addons` **عام** (`@Public()`) للتطبيقات تقرأ الإضافات المتاحة وقت إنشاء الطلب. **مسار `order_items`/`AWAITING_QUOTE_APPROVAL` نفسه اتقفل** (تفاصيل كاملة والاختبار الحي في `../orders/README.md`) — بس ده مسار الفني بيقترح بنود إضافية *أثناء* الشغل (قطع غيار/أجرة إضافية)، **مش** اختيار العميل لإضافة كتالوج جاهزة *وقت إنشاء الطلب*. الربط المباشر بين `service_addons` (الكتالوج) ومسار إنشاء الطلب نفسه (`POST /orders` ياخد `addon_ids[]` مثلاً ويحطهم كـ `order_items` بـ `item_type=addon` من البداية) لسه فجوة موثّقة أصغر متبقية.
