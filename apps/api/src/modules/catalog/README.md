...
## إرشادات السلامة والتعامل لكل خدمة (migration 0337)

`services.safety_guidance_ar` نص عربي يديره الأدمن من شاشة تفاصيل الخدمة. يظهر في كتالوج
الخدمة، ويمر في استجابة تفاصيل طلب العميل حتى يظل زر الإرشادات متاحًا أثناء التنفيذ وبعد إغلاق
الطلب. القراءة تستخدم `findServiceForDisplay()` عمدًا، لذلك لا تختفي الإرشادات من طلب تاريخي إذا
توقفت الخدمة أو حُذفت حذفًا منطقيًا. إذا لم يكتب الأدمن نصًا، يعرض تطبيق العميل إرشاد السلامة
العام بدل إخفاء الزر.
...
`migration 0316` وحّد التسعير إلى مصدر واحد بعد تدقيق حي أثبت تعارض جدول المستوى التشغيلي مع
جدول الفئة التجارية. التفاصيل والبدائل القديمة في `docs/adr/0025-technician-pricing-tier.md`.

- **`TechnicianPricingTier`**: ثلاث قيم فقط (`beginner`/`standard`/`expert`) باسم المستخدم
  «مبتدئ/قياسي/خبير»؛ هي نفس تصنيف سياسة المستحقات.
- **`technician_profiles.pricing_tier`**: يحدد فئة مهارة تسعير الفني صراحةً عبر
  `PATCH /admin/technicians/:id/pricing-tier`، بينما `current_level` يظل تشغيليًا فقط.
- **`service_pricing_tier_pricing`**: مصدر مضاعف السعر الوحيد؛ قاعدة unique تمنع صفين لنفس
  الخدمة والفئة. جرى نقل الإعدادات القديمة إليه وتعطيل `service_level_pricing` كسجل تاريخي.
- **`CatalogService.resolveLevelPriceMultiplier()`**: يقرأ الفئة الموحدة فقط. `technician_level`
  في الطلبات القديمة يتحول إلى الفئة المناظرة للتوافق، ولا يوجد fallback لجدول آخر.
...
- **إدارة الفئة نفسها**: `GET/PUT /admin/services/:id/pricing-tier-pricing`،
  `DELETE /admin/services/pricing-tier-pricing/:pricingId`.
- **`apps/admin`**: قسم «فئة مهارة التسعير» في بروفايل الفني وفورم واحد في صفحة تفاصيل الخدمة؛
  لا توجد شاشة أو endpoint موازية لتسعير المستوى القديم.
- **اختبارات حية**: `technician-pricing-tier.spec.ts` (4/4 — الفئة الموحدة هي المصدر الوحيد،
  توافق المستوى القديم يتحول للفئة، وسجل المستوى وحده لا يغير السعر، وفئة بلا صف نشط = 1)،
...
## كلمات البحث: السقف ٣٠٠ والفاصلة العربية (docs/08 §150 بند ٥)

بلاغ المالك: «الكلمات المسموحة قليلة شوي، عايزين نخلي الفيلد ده ياخد كلمات كتيرة».

`search_keywords` سقفه اتغيّر من **٣٠ لـ٣٠٠** وطول الكلمة من ٦٠ لـ٨٠. ٣٠ كان بيتملي من خدمة
واحدة عادية لأن البحث حرفي بلا أي اشتقاق (`ILIKE`)، فكل صياغة عامية محتاجة صفّها. السقف نفسه
فضل موجود عشان مايبقاش باب إدخال بلا حدود. **مفيش migration**: العمود `text[]` والفهرس GIN
مافيهمش أي قيد — الـDTO كان الحد الوحيد.

الـDTO كمان بقى **بيطبّع**: قص مسافات + توحيد المسافات الداخلية + حذف المكرر والفاضي، فالإدخال
المتكرر مايستهلكش من السقف بلا فايدة بحثية.

**بَقّة حقيقية اتلقطت في نفس الشغل**: فورم الأدمن كان بيفصل على `,` اللاتينية بس. الأدمن بيكتب
عربي، والفاصلة اللي بتطلع من كيبورده هي `،` — فكانت بتعدّي **كجزء من الكلمة نفسها**، يعني كلمة
بحث مالهاش أي مطابقة أبدًا. الفاصل بقى `[,،\n]`، والخانة بقت `Textarea` بدل سطر واحد (٣٠٠
كلمة في `Input` مستحيل تتقرا أو تتراجع).

## `GET /services/:id/technicians` — الميعاد الناقص بيترفض، مابيتخمّنش (docs/08 §169، بلاغ مالك 2026-09-19)

محرك الأهلية بيقرا الميعاد بـ`COALESCE(scheduled_at, now())`: الغياب بيتفسّر **حرفيًا** كتوافر
اللحظة دي. ده صح تمامًا للطوارئ/ASAP، وغلط تمامًا لحجز مجدول لسه العميل ماختارش ميعاده — القايمة
بتتقاس على «النهارده» فيختفي فني مؤهّل للموعد الحقيقي، والعميل يشوف «مفيش فنيين متاحين في منطقتك»
قبل ما يختار ميعاد أصلاً.

الحارس في `listTechniciansForService()`:

```
service.allowsScheduling && !isEmergency && !scheduled_at  ⇒  400 VAL_001
```

- **الطوارئ** (`isSameDayUrgent(scheduledAt)` أو `booking_mode = 'emergency'`) و**الخدمات اللي
  `allows_scheduling = false`** مستثنيين بالحرف — `null` عندهم معناها «دلوقتي» عن قصد.
- **مقصود إن الحارس على الميعاد الغايب بس**، مش على منتصف الليل لخدمة `start_time`:
  `apps/customer-app` بيبعت تاريخ بلا ساعة عن قصد (`bookingScheduledAt()`)، ورفضه كان هيكسر
  شاشة اختيار المنفّذ في التطبيق بلا علاقة بالبلاغ.

**لما القايمة ترجع فاضية في غير الإنتاج**، الـcontroller بينادي
`TechniciansService.diagnoseBookingCandidatePool()` وبيطبع سطر لوج واحد فيه أول مرحلة وقعت لصفر
(نطاق · تأهيل · GPS · حصرية شركة · مستوى الفريق · حجز ذاتي · تعارض جدولي · سقف يومي). فشل
التشخيص ممسوك — أداة مساعدة مايصحّش تحوّل رد ٢٠٠ سليم لعطل.

التغطية: `manual-provider-schedule-context.spec.ts` (٨ اختبارات حية).

## مضاعفات السعر — الجرد الكامل بالترتيب (docs/08 §170)

```
ناتج معادلة التسعير
  × (1 + service_zone_pricing.modifier_percentage / 100)   ← المنطقة (محتاج `zoneId` يوصل!)
  × مضاعف الذروة                                            ← **مثبّت على 1، العمود مش مقروء**
  × service_pricing_tier_pricing.price_multiplier          ← فئة الفني (أو معامل الشركة، بديل)
  ثم القصّ على min/max
  + رسم المعاينة
  + رسم الطوارئ (نسبة من المجموع + رسم المعاينة)
```

- **`zoneId` اختياري في التوقيع، وده فخ**: أي كولر ينساه بيرجّع سعر قبل نسبة المنطقة، بينما
  `POST /orders` بيبعته دايمًا — فالعميل بيشوف رقم ويتحصّل منه رقم تاني. حصلت فعلاً في
  `customer-web` (docs/08 §170-أ).
- **مضاعف الذروة إعداد ميت**: `surge_multiplier` موجود على `service_zones` وعلى
  `service_zone_pricing`، ومحرك التسعير مابيقراهوش. الخانة اتشالت من شاشة الأدمن. أي تفعيل
  محتاج **قاعدة** الأول (الذروة بتشتغل إمتى وإزاي العميل يعرف).
- **الفئة ≠ الرتبة**: `pricing_tier` (مبتدئ/قياسي/متقدم/خبير) بتحدد سعر العميل؛
  `current_level` بيحدد وزن حصة الفني في الطاقم. `pricingTierForOperationalLevel()` احتياطي
