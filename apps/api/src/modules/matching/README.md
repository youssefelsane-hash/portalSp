...
## وزن المسافة الديناميكي حسب سياق الطلب (ADR-0062، طلب مالك 2026-09-02)

**المسافة ماكانتش وزن أصلاً.** `ORDER BY rank_score DESC, distance_km ASC` معناه إنها كاسر تعادل
بس: أي فرق في `rank_score` مهما كان صغير (فني `verified` وزن 10 مقابل `professional` وزن 20)
بيتغلّب على أي فرق في المسافة مهما كان كبير (200 متر مقابل 14 كيلو). ده صح كقاعدة عامة، وغلط
تمامًا في التلات سياقات اللي المالك سمّاها بالاسم: الطوارئ (القيمة كلها في وقت الوصول)، الشغل
خلال 48 ساعة (مفيش مساحة لإعادة توزيع)، والشغل الرخيص (تكلفة الانتقال بتاكل الهامش).
...
والوزن الفعّال بيتحدد من سياق الطلب، كل سياق إعداد مستقل:

| الإعداد | بيسري لما |
|---|---|
| `matching.distance_weight` | الأساس — أي طلب |
| `matching.distance_weight_emergency` | `booking_mode = emergency` |
| `matching.distance_weight_near_term` | الموعد خلال `matching.near_term_request_hours` (ASAP كمان — بلا موعد = دلوقتي) |
...
- **لما أكتر من سياق ينطبق، الأعلى بياخد — مش المجموع.** طلب طوارئ رخيص خلال ساعة بينطبق عليه
  تلاتة؛ الجمع كان هيدّي وزن 3× مالوش معنى مقصود وبيخلّي أثر تغيير إعداد واحد غير متوقّع للأدمن.
- **كل الأوزان صفر افتراضيًا** = السلوك القديم بالحرف. نفس فلسفة `fairness_weight`/
  `reliability_weight`: تفعيل صريح مطلوب، صفر تغيير سلوك صامت.
- **الوحدة كيلومتر مش قيمة مطبَّعة** — وزن 1 معناه «كل كيلومتر بيساوي نقطة ترتيب»، رقم الأدمن
  يقدر يفكّر بيه. التطبيع (0..1) كان هيخلي نفس الوزن يدّي أثر مختلف حسب توزيع المرشّحين في الدفعة.
- الحساب في `matching-weights.ts` (`resolveDistanceWeight`)، و`MatchingExplainabilityService`
  بينادي **نفس الدالة** — التفسير مستحيل ينحرف عن القرار الفعلي.
- `distance_penalty` بقى عمود في نتيجة الاستعلام وبيظهر في تفكيك `rank_score` عند الأدمن مع اسم
  السياق اللي حدّد الوزن.
- إعدادات مجموعة `matching` كلها بقى ليها **قسم مخصّص** في `/admin/settings` (مقسّم بالمعنى:
  أوزان المسافة، أوزان الترتيب، الجولات والنطاق، القدرة الاستيعابية) بدل صفوف وسط جدول عام —
  ومستبعدة من الجدول العام، فمسار التحرير واحد بالظبط.

اختبار حي: `matching-distance-weight.spec.ts` — الفني البعيد أعلى مستوى عمدًا، فالنتيجة بتقلب
فعلاً لما الوزن يتفعّل، وكل سياق بيتفحص لوحده.
...
## التثبيت الصارم لإعادة الزيارة (ADR-0051، docs/08 §96)

`requested_technician_id` معناه المستقر هنا هو **تفضيل بيتجاهَل بأمان** ("إعادة الحجز" واعتماد
شركة): الجولة الأولى بس بتحاول الفني/الشركة المطلوبة، ولو مفيش مؤهّل متاح بيرجع فورًا للبث
العادي في نفس الجولة. **متغيّرش ده** — فيه مسارين حقيقيين معتمدين عليه.

إعادة الزيارة مسؤولية مش تفضيل، فليها مسار منفصل تمامًا:

- `dispatchNextRound()` فيها فرع أول خالص: لو `isRevisitPinActive(order)`، المرشّحين بيتحصروا في
  الفني المثبّت (`ignoreActiveOrderConflict = true` — مشغول دلوقتي مش سبب لتحويل إعادة زيارة
  لحد تاني)، وكل fallbacks التوزيع العادي بتتخطى.
- الفرع idempotent: بيرجع `noop` لو التثبيت اتقفل (رفض/إلغاء/عدّت المهلة) أو لو العرض اتبعت
  للفني ده خلاص. عشان كده **`MatchingRecoveryService` اتسابت زي ما هي عن قصد** — لازم تفضل
  بتعيد المحاولة عشان لو الفني مكانش مؤهّل لحظة الإنشاء يوصله العرض أول ما يبقى مؤهّل، والفرع
  بيمنع تكرار الإشعارات.
- `reject()` **ما بتعيدش البث** لطلب مثبّت (بعكس أي رفض عادي) — التحرير قرار أدمن وراه خصم مالي.
- `dispatchOrAutoConfirm()` بتوجّه الطلب المثبّت لـ`dispatchNextRound()` دايمًا: التأكيد التلقائي
...
## طلب الشركة بيختار عضو **متاح** (ADR-0094، docs/08 §150 بند ٢)

`firstScheduledCandidate()` كانت بتقرا `members[0]` وبس لطلب عليه شركة، وبعدين
`classifyCandidate()` بتتفحصه لوحده — فعضو واحد مزنوق كان **بيقفل الشركة كلها** عن التأكيد
التلقائي والطلب يروح لجولات عروض، حتى لو باقي الطاقم فاضي. دلوقتي بنمشي على القايمة (لحد
`COMPANY_MEMBER_SCAN_LIMIT = 10`) وناخد أول عضو `LIGHT`، ولو مفيش بنرجّع الأعلى ترتيبًا زي
الأول بالظبط فالحارس القديم مايتشالش.

**الفرق عن مسار الفني المستقل مقصود**: هناك العميل قفل على **شخص**، فمفيش بديل يتشاف وتحويل
الطلب لعرض هو الصح (ADR-0020). هنا العميل قفل على **شركة**، والاختيار جوّاها تلقائي أصلاً.

مقيس في `company-member-availability.spec.ts` **بضابط تجربة** (العضو الأعلى فاضي ⇒ بياخده هو)،
واتعمله اختبار سلبي بسقف فحص = ١.

**فخّان مكتوبين في الاختبار عشان ما يتعادوش**: العضو الاحتياطي لازم يكون حد مستواه المالي فوق
سعر الطلب (`new` حده ٢٠٠ ج.م، فكان بيتشال من الأهلية أصلاً والاختبار يقيس حاجة تانية)، وتحميل
`AppModule` كامل بيعلّق السويت فالخدمة بتتركّب بإيد.

## مفتّش المطابقة لازم يقول نفس كلام المحرك (docs/08 §151)

`MatchingExplainabilityService` مبني على مبدأ صريح: **نفس شروط `findEligibleTechnicians()`
بالحرف، صفر خوارزمية تشخيصية موازية**. المقارنة بند-بند في 2026-09-15 طلّعت تلات شروط في
المحرك وناقصة من المفتّش:

1. `requires_technician_lead` (ADR-0086) — خدمة مشترطة فني كامل كانت بتدّي «مؤهّل بالكامل»
   لمساعد المحرك بيرفضه.
2. `company_exclusive` (ADR-0080) — الحصري للشركة كان بيبان مؤهّل لطلب عام.
3. صف عرض حي في `technician_work_opportunities` — المفتّش كان بيفحص `order_assignments` بس،
   وهما **جدولين مستقلين**.

التلاتة من **نفس الفئة** اللي §36.6 و§107 اتصلحوا عشانها: شرط بيتضاف للمحرك ويتنسى هنا.
فالحماية اتغيّرت من «افتكر تحدّث المفتّش» لـ**اختبار بيقيس الثابت**:
`explainability-matches-engine.spec.ts` بيتأكّد إن `explain.eligible` تساوي «هل المحرك رجّعه
فعلاً؟» — أي شرط جديد بينسى بيفشل تلقائيًا.

**قبل ما تضيف شرط جديد لـ`findEligibleTechnicians()`**: ضيفه هنا كمان، والاختبار ده هيقولك لو
نسيت.

### `is_eligible_now` في قايمة المرشّحين **سؤال تاني** — مش انحراف

`GET /orders/:id/explain-candidates` بيحسبها من قايمة **التعيين الإداري**، يعني «الأدمن يقدر
يعيّنه دلوقتي؟» مش «المحرك كان هيختاره؟». السؤالين مختلفين فعلاً ومقصود إنهم يفضلوا كده، بس
مكتوب هنا عشان ما يتقريش على إنهم نفس الحاجة.

## بوابة الحمل: ليه الطلب البعيد كان بيروح Request (docs/08 §156)

بلاغ مالك: «بعض الطلبات تذهب كـRequest رغم أن الموعد ليس قريبًا، والفني يبدو متاحًا».

مُعاد إنتاجه حيًا (`node scripts/verify-dispatch-route-reasons.js`) — العتبة نفسها كانت سليمة
(٤٨س ⇒ جولات، ٤٩س ⇒ تأكيد تلقائي)، والمشكلة في متجاوزَين بعدها:

| السبب | كان بيحصل | الجذر |
|---|---|---|
| `existing_requests` | عرض **انتهت مهلته من ساعتين** بيخلّي طلب بعد أسبوع يفضل في الجولات للأبد | `count()` بلا أي فلتر حالة — تاريخ ميت بيغيّر قرار الحاضر |
| `same_day_workload` | **ساعتين** شغل من ٧٢٠ دقيقة، بلا أي تقاطع، بتمنع التأكيد التلقائي | `tier !== 'LIGHT'` — و`MEANINGFUL` معناها «مؤهّل بالكامل» |

القرارين اتجمعوا في `resolveWorkloadGate()` (دالة خالصة في `dispatch-route.ts`) — نفس فلسفة
باقي الملف: التنفيذ والشرح والاختبار بيقروا **نفس السطور**.

- `existing_requests` بقى بيعدّ `sent`/`viewed` بس — **نفس تعريف «الحيّ»** المستخدم في
  `matching.service.ts` أصلاً (سطور ٨٥٤ و٩٦٢)، مش قايمة جديدة.
- `same_day_workload` بقى `HEAVY`/`BLOCKED` بس. `matching.auto_confirm_requires_idle_technician`
  بيرجّع السلوك القديم بضغطة.

⚠️ **ملاحظة مقاسة**: `HEAVY`/`BLOCKED` مستبعدين أصلاً من `technicianAvailabilityCondition`،
فـ`firstScheduledCandidate()` مابترجعهمش. يعني بعد التضييق، `same_day_workload` عمليًا بيتفعّل
عبر الإعداد بس. **ده نتيجة التحليل مش سهو**: البوابة كانت بتكرّر بوابة الإتاحة، ولما اتشال
التكرار مافضلش غير اللي البوابة بتعمله. الفرعين متسيبين كحد صح للقاعدة.
