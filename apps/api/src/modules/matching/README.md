...
## فجوة موثّقة صراحة — سباق نادر جداً بين `reject()` والمهلة المنتهية ممكن يعمل جولة توزيع مكرّرة
...
لو الاتنين حصلوا في نفس اللحظة بالظبط لنفس الطلب (فني بيرفض آخر عرض في الجولة **بالتزامن** مع
انتهاء مهلة نفس الجولة) — سباق نظري: الاتنين ممكن يقروا نفس `MAX(assignment_round)` قبل ما أي
واحد يكتب، فيحسبوا نفس `nextRound` ويضيفوا صفوف `order_assignments` مكرّرة لنفس الجولة (فنيين
اتبعتلهم عرض مرتين). **الأثر الفعلي محدود**: `accept()` عندها قفل ذرّي (`pessimistic_write` على
صف الطلب) بيمنع أي فني (حتى لو استلم عرض مكرر) من فوز أكتر من مرة أو تعارض حقيقي — أسوأ سيناريو
هو صف مكرر في `order_assignments`/إشعار مكرر للفني، مش حجز مزدوج أو خطأ مالي. مؤجَّل عمداً: أي
إصلاح محتاج قفل صف الطلب جوّه `dispatchNextRound()` نفسها زي `accept()` بالظبط، بس `cancelForNoTechnicians()`
بتفتح transaction منفصلة خاصة بيها — لف الاتنين في نفس القفل محتاج إعادة هيكلة حذرة (تمرير
`manager` بدل `dataSource.transaction()` مستقل) عشان نتجنب deadlock بين transaction بره وتانية
جواها على نفس صف الطلب. مش هينفّذ من غير اختبار حي كامل لسيناريو deadlock الأول.
