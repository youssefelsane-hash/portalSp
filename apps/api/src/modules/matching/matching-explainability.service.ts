...
export class MatchingExplainabilityService {
   ...
   */
  /**
   * **حالة المطابقة التشغيلية الكاملة لطلب واحد** — الجولات بتفاصيلها، والحالة الحالية، والخطوة
   * الجاية ووقتها، وهل الـworkflow متأخر.
   *
   * موجودة هنا مش في خدمة جديدة لأن الملف ده هو بالفعل «الجهة اللي بتفسّر مطابقة الطلب»
   * (`explainTechnicianForOrder` + `explainOrderFunnel`) — فصل «الحالة» عن «التفسير» في خدمتين
   * كان هيخلي الاتنين يقروا نفس الجداول بطريقتين.
   *
   * استعلام واحد للجولات + استعلام واحد للتاريخ الإداري — بلا N+1 مهما كان عدد الجولات.
   * الاشتقاق نفسه (الخطوة الجاية/التأخير/المصدر) بيتم في `matching-workflow-state.ts` عشان
   * Operations وException Center يقروا **نفس** القاعدة بالحرف.
   */
  async getOrderMatchingState(order: Order): Promise<OrderMatchingState> {
    const [rows, maxRounds, graceSeconds, adminHistory] = await Promise.all([
      this.dataSource.query<RawAssignmentRow[]>(
        `SELECT oa.id, oa.assignment_round, oa.technician_id, tp.technician_code, u.full_name,
                oa.assignment_status, oa.rejection_reason_code, oa.distance_km, oa.estimated_eta_minutes,
                oa.sent_at, oa.viewed_at, oa.responded_at, oa.expires_at
           FROM order_assignments oa
           JOIN technician_profiles tp ON tp.id = oa.technician_id
           JOIN users u ON u.id = tp.user_id
          WHERE oa.order_id = $1
          ORDER BY oa.assignment_round ASC, oa.sent_at ASC`,
        [order.id],
      ),
      ...
      this.dataSource.query<{ exists: boolean }[]>(
        `SELECT EXISTS(
                  SELECT 1 FROM order_status_history
                   WHERE order_id = $1 AND change_source = 'admin'
                     AND new_status IN ('technician_assigned', 'accepted')
                ) AS exists`,
        [order.id],
      ),
    ]);

    const counts: OrderAssignmentStatusCounts = {
      sent: 0,
      viewed: 0,
      accepted: 0,
      rejected: 0,
    ...
    const roundMap = new Map<number, OrderMatchingRound>();
    for (const r of rows) {
      if (r.assignment_status in counts) counts[r.assignment_status as keyof OrderAssignmentStatusCounts] += 1;
      const attempt: OrderAssignmentAttempt = {
        assignmentId: r.id,
        round: r.assignment_round,
        ...
        etaMinutes: r.estimated_eta_minutes,
        sentAt: r.sent_at,
        viewedAt: r.viewed_at,
        respondedAt: r.responded_at,
        broadcastExpandsAt: r.expires_at,
      };
      const existing = roundMap.get(r.assignment_round);
      if (existing) {
        existing.attempts.push(attempt);
        // بداية الجولة = أول إرسال فيها، ووقت التوسيع = آخر مهلة فيها (الجولة بتتوسّع لما آخر
        // عرض فيها يعدّي معاده، مش أولهم).
        if (r.sent_at < existing.startedAt) existing.startedAt = r.sent_at;
        if (r.expires_at > existing.broadcastExpandsAt) existing.broadcastExpandsAt = r.expires_at;
      } else {
        roundMap.set(r.assignment_round, {
          round: r.assignment_round,
          startedAt: r.sent_at,
          broadcastExpandsAt: r.expires_at,
          attempts: [attempt],
    ...
    const currentRound = rounds.length > 0 ? rounds[rounds.length - 1].round : 0;
    const current = rounds.length > 0 ? rounds[rounds.length - 1] : null;
    const pendingInCurrentRound =
      current?.attempts.filter((a) => a.status === 'sent' || a.status === 'viewed').length ?? 0;

    const workflow = deriveMatchingWorkflowState({
      orderStatus: order.orderStatus,
      currentRound,
      roundExpansionDueAt: current?.broadcastExpandsAt ?? null,
      pendingInCurrentRound,
      maxRounds,
      graceSeconds,
      now: new Date(),
    });
    ...
    return {
      orderId: order.id,
      orderStatus: order.orderStatus,
      assignmentSource: deriveAssignmentSource({
        technicianId: order.technicianId,
        providerLockSource: order.providerLockSource,
        revisitPinnedTechnicianId: order.revisitPinnedTechnicianId ?? null,
        hasAdminAssignmentHistory: adminHistory[0]?.exists === true,
      }),
      assignedTechnicianId: order.technicianId,
      currentRound,
      maxRounds,
      // عدد الفنيين **المختلفين** اللي وصلهم الطلب — نفس دلالة `order_technician_count` الموجودة
      // في Operations، عشان الرقمين مايختلفوش بين شاشتين.
      techniciansContacted: new Set(rows.map((r) => r.technician_id)).size,
      counts,
      workflow,
      rounds,
    };
...
// ─────────────────────────────────────────────────────────────────────────────
// حالة المطابقة التشغيلية للطلب — «إيه اللي حصل، وإيه اللي جاي، وهل النظام متأخر»
// ─────────────────────────────────────────────────────────────────────────────

export interface OrderAssignmentAttempt {
  assignmentId: string;
  round: number;
  technicianId: string;
  ...
  sentAt: Date;
  /** null للصفوف اللي قبل migration 0255، أو اللي ماتشافتش. الواجهة بتفرّق بين الاتنين. */
  viewedAt: Date | null;
  respondedAt: Date | null;
  /** وقت **توسيع البث** مش انتهاء صلاحية العرض (ADR-0018 §5). */
  broadcastExpandsAt: Date;
}

export interface OrderMatchingRound {
  round: number;
  startedAt: Date;
  broadcastExpandsAt: Date;
  attempts: OrderAssignmentAttempt[];
}

export interface OrderMatchingState {
  orderId: string;
  orderStatus: OrderStatus;
  assignmentSource: OrderAssignmentSource;
  assignedTechnicianId: string | null;
  currentRound: number;
  maxRounds: number;
  techniciansContacted: number;
  counts: OrderAssignmentStatusCounts;
  workflow: MatchingWorkflowState;
  rounds: OrderMatchingRound[];
}
