...
**الفجوة دي اتقفلت (§117)**: كانت «مفيش مصفوفة E2E حقيقية بمتصفح كاملة للـepic كله» — كل مرحلة
اتحققت لوحدها حيًا لكن التحقق كان **تشغيلة عابرة نتيجتها بتتكتب في README وبتموت**. دلوقتي فيه
سكريبت مُلتزم وقابل لإعادة التشغيل:

```bash
API_LOG=<مسار لوج الباك-إند> npm run test:e2e:operations --prefix apps/admin
```

`apps/admin/test/operations-center.e2e.mjs` — 20 تأكيد على متصفح حقيقي بتسجيل دخول OTP حقيقي:
كل التبويبات الستة بتحمّل محتوى فعلي بلا رسالة خطأ، مفيش سكرول أفقي للصفحة في أي تبويب (الجداول
العريضة بتسكرول جوّه حاويتها)، مفيش أي خطأ JavaScript في أي شاشة، **والأرقام المعروضة مطابقة
للـAPI**، **وصفحة الطلب وصفحة الفني والتوزيع اللحظي بيتفقوا على نفس الطلب** — وde التأكيد اللي
بيثبت ادعاء §117 المركزي (مصدر اشتقاق واحد): لو حد اشتق الحالة في الواجهة، السكريبت بيسقط.

**درس من كتابة السكريبت نفسه**: أول نسخة من تأكيد «العدّاد بيعرض الإجمالي مش عدد الصفوف» كانت
**فاضية** — من غير قصّ ومن غير فلترة `items.length` بيساوي `total_searching` بالصدفة، فحقنت
الرجعة (رجّعت العدّاد يعرض `items.length`) والاختبار عدّى. اتصلح بإضافة خطوة بتضغط «المتأخر بس»
فعليًا في المتصفح وبتتأكد إن الإجمالي **مااتغيّرش** بينما الصفوف قلّت — التأكيد ده بيسقط على
الرجعة دي فورًا (اتجرّب في الاتجاهين).
...
### السقف مُعلَن مش مسكوت عنه

الاستعلام بيقصّ على `LIVE_DISPATCH_MAX_ROWS` (200) وبيرجّع مع الصفوف `totalSearching` (من
`COUNT(*) OVER()`، يعني **قبل** القصّ) و`truncated`. من غيرهم الشاشة كانت هتقول «200 طلب
بيدوّر» وهي مش شايفة النص التاني — وde أسوأ حاجة ممكنة في شاشة شغلها «ورّيني اللي واقف».

الفلترة على `isDelayed` بتتم في TypeScript مش في `WHERE`: القاعدة في مكان واحد
(`deriveMatchingWorkflowState`)، وكتابتها تاني في SQL كانت هترجّعنا لنسختين بتفترقوا مع أول
تعديل في مهلة السماح أو سقف الجولات. الثمن — إن الفلتر بيشتغل على النافذة المحمّلة — **معلَن
للواجهة** (`truncated`) بدل ما يكون مخفي، والواجهة بتقوله للأدمن بالنص.

### شكل الرد متعشّش عمدًا (`summary` + `orders:{items,meta}`)

نفس سبب `dispatch-delivery`: `items`+`meta` على المستوى الأول بيخلّي `ResponseInterceptor`
يفردهم لـ`data: items` **ويقطع `summary` بصمت تام** — وهي بالظبط بَقّة §90 الحقيقية.
`ApiMeta` (`page`/`per_page`/`total` بس) مالوش مكان لعلامة القصّ، فالتعشيش هو الحل الصح مش
توسيع عقد مشترك عشان حقل واحد.

### اختبارات على مستوى الـcontroller

`admin-operations.controller.spec.ts` بيغطّي طبقة الـmapping نفسها (اللي بَقّة §90 عاشت فيها،
واختبار الخدمة لوحده مش بيلمسها): كل حقل بتقراه الواجهة، وصول `summary`، وإن الرد **مالوش**
`items`/`meta` على المستوى الأول. اتأكدنا إنهم بيمسكوا فعلًا بكسر الـmapping مرتين عمدًا
(شيل `workflow_phase_ar`، وفرد `items`/`meta` لفوق) — الاتنين فشلوا، وبعد الرجوع عدّوا.
