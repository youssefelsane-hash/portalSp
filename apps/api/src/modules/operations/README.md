...
## §119 — تتبّع المطابقة + الاستثناءات اللي كانت بتتحسب وتترمي

الشغل ده كله **فوق** اللي موجود: صفر محرك مطابقة تاني، صفر مركز عمليات تاني، صفر جدول جديد،
صفر scheduler. المصدر الوحيد في كل حتة هو `order_assignments` نفسه.

### 1. `order_assignments.viewed_at` (migration 0255) — الفرق بين «ما ردّش» و«ما فتحش»

الحالة `viewed` كانت موجودة من زمان، بس **من غير أي طابع زمني**: النظام يعرف إن الفني شاف العرض،
ومايعرفش إمتى. يعني مكانش ينفع تقيس «قعد قد إيه بيبص قبل ما يرفض»، ولا تفرّق في شاشة الأدمن بين
فني فتح وسكت وفني مافتحش أصلاً — والاتنين محتاجين تصرّف مختلف تمامًا.

الكتابة في مسارين بس، الاتنين بـ`COALESCE(viewed_at, now())` عشان أول مشاهدة هي اللي تتسجّل مش آخر
واحدة: `OrdersService.markViewedByTechnician()` و`MatchingService` (سحب قايمة العروض). العمود
nullable من غير backfill — الطلبات القديمة `viewed_at IS NULL` وده صحيح: مفيش قيمة حقيقية نحطها.

### 2. `AdminOrderTraceService` — نفس البيانات، السؤال التاني

`AdminDispatchDeliveryService` بيدّي feed **مسطّح مرتّب زمنيًا**: «إيه اللي حصل». مايجاوبش «الطلب
ده وصل لمين، في أنهي جولة، ومستني إيه دلوقتي». الخدمة الجديدة بتجمّع نفس `order_assignments` حسب
الطلب → الجولة → الفني. الاتنين فاضلين — كل واحد سؤال مختلف على نفس الصفوف.

`nextAction` **مشتقّة** من حالة الطلب والجولات، مش counter مخزّن ولا scheduler:

| الحالة | `nextAction` |
|---|---|
| في عرض مقبول | `assigned` |
| الطلب مش `searching_technician` | `no_matching_required` |
| مفيش جولات لسه | `waiting_technician_response` |
| مهلة آخر جولة لسه جاية | `waiting_technician_response` |
| المهلة عدّت و`currentRound < maxRounds` | `expand_next_round` (+ `delaySeconds`) |
| المهلة عدّت وخلصت الجولات | `matching_exhausted` |

**استعلام واحد لكل الطلبات** (`WITH target_orders … LEFT JOIN order_assignments`)، مش استعلام لكل
صف — ده الفرق بين شاشة بتفتح وشاشة بتقع.

**تسمية مهمة**: `order_assignments.expires_at` هي **مهلة توسيع الجولة**، مش انتهاء صلاحية العرض.
العرض بيفضل قابل للقبول بعد الوقت ده لحد ما فني تاني ياخد الطلب أو الفني ده يرفض صراحة (راجع
`matching-round-expiry.processor.ts`). كل الواجهات دلوقتي بتقول «مهلة التوسيع» بالنص، مش «انتهت».

المسارات: `GET /admin/operations/order-traces` (كل الطلبات الباحثة، تبويب «تتبّع الطلبات» في
`/operations`) و`GET /admin/operations/order-traces/:orderId` (طلب واحد، جوّه «مفتّش المطابقة»
الموجود أصلاً في صفحة الطلب — توسيع للعدّادات المسطّحة اللي فوقه، مش كارت جديد).

### 3. بَقّة حقيقية: استثناءات كانت بتتحسب بالكامل والـcontroller بيرميها

`AdminExceptionCenterService.getExceptions()` كانت بترجّع خمس مجموعات؛ الـcontroller كان بيمرّر
تلاتة بس. `stalledRevisits` (ADR-0051 — إعادة زيارة مثبّتة على فني مبقاش عنده الطلب) كانت
بتتحسب باستعلام كامل في كل نداء و**بتترمي قبل ما توصل لأي واجهة**. يعني إعادة زيارة ممكن تفضل
معلّقة للأبد ومحدش يشوفها، والـendpoint اللي بيحرّرها (`POST /admin/orders/:id/release-revisit`)
كان موجود ومختبر ومن غير أي مستهلك في الواجهة خالص.

نفس فئة بَقّة §90 بالظبط (اللي فوق) — علشان كده الاختبار الجديد اتحط في نفس ملف
`admin-operations.controller.spec.ts`: اختبار على مستوى **الـcontroller** بيتأكد إن كل مجموعة
محسوبة توصل الرد فعلاً. اختبار على مستوى الخدمة عمره ما هيمسك ده، لأن الخدمة كانت بترجّعه صح.

كمان: `overdue_orders` كانت بتوصل الـJSON بس الواجهة ماكانتش بتعرضها أصلاً — ودي **أعجل** بند في
المركز حسب `docs/08 §56 بند 4`. دلوقتي معروضة أول حاجة.

### 4. `matchingWorkflowDelayed` — استثناء جديد: المحرك واقف، مش الفني

مختلف عمدًا عن `staleDispatch`:

* `staleDispatch` = «عرض معيّن فات معاده بلا رد» → **سلوك فني**، حاجة طبيعية بتحصل.
* `matchingWorkflowDelayed` = «مهلة الجولة عدّت والمحرك ما فتحش الجولة اللي بعدها» → **عطل
  workflow**. الشرط: أكبر `expires_at` في آخر جولة عدّى، و`current_round < max_rounds`، والطلب
  لسه `searching_technician`.

خلطهم كان هيخفي أخطر الاتنين. `admin-exception-center.spec.ts` فيه `describe` كامل بيثبت إنهم
فعلاً مش نفس الحاجة (طلب بيظهر في واحد ومش في التاني، في الاتجاهين).

### 5. اختبار حي بمتصفح حقيقي

اتأكد end-to-end بـPlaywright + CDP virtual WebAuthn authenticator (نفس منهجية
`apps/admin/README.md` §MFA) على `apps/admin` مبني (`next start`) + الباك-إند الحقيقي + Postgres
حقيقي، ببيانات محقونة (طلب بجولتين، مهلة الجولة عدّت) واتمسحت فورًا بعد الفحص. اتأكد فعليًا:
تبويب «تتبّع الطلبات» بيعرض الجولتين وعمود «فتح العرض» و«مهلة التوسيع» و«المفروض يوسّع لجولة
جديدة»؛ صفحة الطلب بتعرض نفس الجولات جوّه مفتّش المطابقة؛ ومركز الاستثناءات بيعرض «المطابقة نفسها
واقفة» بالطلب الصح.

**بَقّة عرض حقيقية اتلقطت من اللقطة دي**: عمود التاريخ في جدول التسليم كان بيعرض `22026/9/` بدل
`2026/9/2`. السبب مش تنسيق — `ar-EG` بيحقن علامات `U+200F` (RLM) جوّه سلسلة التاريخ نفسها، وهي
بتترتّب صح في سياق RTL بس، والخلايا دي بتورّث `direction: ltr` فعليًا (اتأكدت بقراءة
`getComputedStyle` من صفحة حية، مش بالتخمين). الإصلاح `dir="rtl"` على عنصر التاريخ.
**محاولة أولى فشلت وتوثّقها هنا مقصود**: لفّ السلسلة في عزل ثنائي اتجاه (`U+2066…U+2069`) بيكسرها
أكتر — العزل بيتعارض مع الـRLM مش بيساعده. اتأكد بصفحة اختبار معزولة قبل ما يترجع.


## مصفوفة E2E بمتصفح لمركز العمليات (قابلة لإعادة التشغيل)

```bash
API_LOG=<مسار لوج الباك-إند> npm run test:e2e:operations --prefix apps/admin
```

`apps/admin/test/operations-center.e2e.mjs` — 17 تأكيد على متصفح حقيقي بتسجيل دخول OTP حقيقي.
الفجوة اللي بيقفلها مكانتش «مفيش تحقق»، كانت إن **كل تحقق بصري قبل كده كان تشغيلة عابرة نتيجتها
بتتكتب في README وبتموت**.

بيغطّي: كل التبويبات الستة بتحمّل محتوى فعلي بلا رسالة خطأ · مفيش سكرول أفقي للصفحة في أي تبويب ·
مفيش أي خطأ JavaScript في أي شاشة · **المرحلة المعروضة = المرحلة اللي الـAPI اشتقّها** (لو حد
اشتق في الواجهة بيسقط) · **تبويب التتبّع وصفحة الطلب وصفحة الفني بيتفقوا على نفس الطلب**.

**درس من كتابة السكريبت نفسه**: أول نسخة من تأكيد رقمي كانت **فاضية** — حقنت الرجعة والاختبار
عدّى، لأن الرقمين بيتساووا بالصدفة في البيانات الحالية. الدليل إن الاختبار نافع مش إنه بيعدّي،
الدليل إنه **بيسقط لما الحاجة تتكسر** — واتجرّب في الاتجاهين.

## ملاحظة مفتوحة: «مستني رد الفنيين» لطلب محدش اتبعتله

`deriveNextAction()` بترجّع `waiting_technician_response` لما `currentRound === null` — يعني طلب
لسه بيدوّر و**محدش اتبعتله أصلاً** بيتقال عليه «مستني رد الفنيين»، مع إن مفيش فني اتواصل معاه
عشان نستنى رده. في قاعدة التطوير الحالية ده وضع **50 من 50** طلب بيدوّر.

قسم الجولات في صفحة الطلب متخفي في الحالة دي (`rounds.length === 0`) وde قرار سليم — مفيش جولات
تتعرض. لكن تبويب التتبّع بيعرض النص المضلّل ده. الاقتراح: حالة سادسة `not_dispatched` بنص زي
«لسه ما اتوزّعش على أي فني». **مش متنفّذة هنا عمدًا**: بتلمس enum مشترك + قاموسي نصوص +
اختبارات في كود اتدمج من سيشن تانية، فالقرار للمالك.

## مركز المراجعة والشواذ (ADR-0084، docs/08 §139)

`admin-review-center.service.ts` + `GET /admin/operations/review-center`.

**ليه خدمة تانية جنب `admin-exception-center.service.ts` بدل ما نوسّعها؟** الاتنين بيعرضوا
«حاجة محتاجة انتباه» بس بيجاوبوا سؤالين مختلفين، والخلط بينهم بيخلي «الأدمن بيبص فين؟» سؤال
ليه إجابتين:

| | مركز الاستثناءات | مركز المراجعة |
|---|---|---|
| السؤال | النظام مااتحركش | حد اتحرّك بطريقة محتاجة حكم |
| مثال | طلب معداه عدّى، مطابقة واقفة | الفني زوّد فلوس، بلّغ نشو، قال ماستلمش كاش |
| الشغل المطلوب | **حرّك** | **راجع واحكم** |

### الأقسام الأربعة ومصادرها

| القسم | المصدر |
|---|---|
| `technician_money_actions` | `order_items` (بنود مقترحة) + `order_quotes` (عروض) بـUNION |
| `failed_visits` | `orders.order_status='disputed'` + آخر `order_status_history` + تصنيف الشكوى |
| `cash_disputes` | `technician_cash_not_received_at` (+ `customer_cash_confirmed_at` = تضارب صريح) |
| `unresolved_complaints` | `complaints` المفتوحة، **بأطرافها بالاسم** |

### فخّان حقيقيان اتلقطوا أثناء البناء

1. **مفردات الحالة مختلفة بين المصدرين.** `order_items.proposal_status='pending'` بينما
   `order_quotes.status` بتبقى `pending_admin_review`/`pending_customer`. فلترة على نص
   `'pending'` حرفيًا كانت هتسقط **كل عروض الأسعار** من قايمة المعلّق بصمت — وهي أهم بند في
   الشاشة. التوحيد بقى في الاستعلام نفسه (`is_pending`)، والواجهة بتقرا راية واحدة.
2. **`complaints` مالهاش عمود `deleted_at`.** الاستعلام الأول كان بيفلتر عليه بافتراض إن كل
   الجداول عندها soft delete — والاستعلام كان بيقع بالكامل.

### القراءة بس

صفر قرار هنا. كل فعل (موافقة عرض، حل زيارة فاشلة، حسم كاش) ليه endpoint مخصّص بصلاحيته
وstep-up بتاعه، والشاشة دي **مابتستبدلهومش** — بتوصّل للطلب وخلاص.
