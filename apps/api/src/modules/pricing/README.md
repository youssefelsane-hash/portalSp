...
## طريقة الحساب الوحيدة + القوالب (ADR-0060، docs/08 §113)

`services.pricing_model` بقى قيمتين بس: `formula` (كل خدمة بيتحسبلها سعر) و`inspection_then_quote`
(مفيش سعر وقت الحجز أصلاً). اللي كان اسمه «سعر ثابت / بالساعة / بالوحدة / شهري» بقى **قوالب** في
`pricing-templates.ts`: زرار بيزرع الحقول اللي القالب محتاجها ويكتب شجرة `final_price` مرة واحدة،
وبعدها الخدمة معادلة عادية بالكامل. مفيش أي فرع في الكود بيسأل «دي كانت أنهي قالب؟».

اللي اتشال بالكامل: `evaluatePreset()`، `pricing-methods.ts`، `missingPricingInput()`،
`unitsForZoneOverride()`.

**نقطتين مهمتين للي جاي بعدنا:**

1. **القالب الشهري بيزرع حقلين تاريخ عاديين في الفورم** وبيقرا منهم بـ`kind: 'field'` — مش من
   `period_start`/`period_end` النظاميين. ده مقصود: مصدر التواريخ لازم يفضل **واحد**، وإلا بترجع
   بَقّة «أربع حقول تاريخ على نفس الشاشة» اللي المالك بلّغ عنها.
2. **خدمة بلا معادلة = سعر ثابت عند `base_price_cents`** (`prepareEvaluation()`), وشجرتها هي حرفيًا
   شجرة قالب «سعر ثابت». ده مش مسار حساب تاني — لو حد فكّر يشيله، هيرجع خطأ «الخدمة دي مفيهاش
   معادلة سارية» لأي خدمة جديدة قبل ما الأدمن يخلص إعدادها.

أي اختبار حي بيمرّ على `estimate()` محتاج محرك شغّال — استخدم
`realPricingEngineService(dataSource)` من `pricing-engine.testing.ts` بدل `{} as never`.

## إعادة الزيارة تحت الضمان: الزيارة مجانية، الشغل الجديد عليها **مش** مجاني

**بلاغ المالك 2026-09-11**: «الطلب راح بزيرو فلوس… الفني اكتشف إنه محتاج قطعة غيار… الفلوس دي
المنصة ما بتاخدش منها أي حاجة خالص، السعر كله بيدخل في الحصالة بتاعته هو، المنصة كزيرو».

البلاغ اتأكد بالأرقام (`scripts/revisit-commission-audit.js`) — قطعة بـ300 ج.م على خدمة عمولتها
20%: الفني 300.00 والمنصة 0.00.

**السبب**: `order-creation.service.ts` بيثبّت `commission_rate_applied = 0` على طلب إعادة الزيارة
(`originalOrder ? 0 : service.commissionPercentage`). ده **صح للشغل المجاني نفسه** — المنصة
بتتنازل عن عمولتها على إعادة شغلانة خدت عمولتها مرة. بس النسبة دي snapshot على الصف، وكل شغل
**جديد مدفوع** بيتضاف بعد كده (قطعة غيار، أجرة إضافية، سعر بعد المعاينة) كان بياخد نفس الصفر.

**الإصلاح**: `OrderFinancialFinalizationService.applyRevisitCommissionRate()` — أول ما شغل مدفوع
يدخل **وعاء العمولة** على طلب إعادة زيارة نسبته صفر، النسبة بترجع لنسبة الخدمة.

آمن على الوعاء كله لأن وعاء إعادة الزيارة بيبدأ من **صفر** (كل مكوّناته صفر وقت الإنشاء)، يعني
مافيهوش غير الشغل الجديد المدفوع — الجزء المجاني مش موجود في الوعاء أصلاً عشان يتلمس.

**ليه جوّه `increasePrice` مش عند المستدعي؟** لأن دي نقطة الكتابة الوحيدة لأي زيادة سعر، فالإصلاح
بيشمل تلقائيًا كل المسارات (`additional_work`, `inspection_quote`, `diagnosis_revision`,
`level_premium`, `onsite_assessment_fee`) — ومسار جديد يتكتب بكرة مش ممكن ينساه. لو اتنقل
للمستدعيين، كل مسار جديد بيبقى بَقّة مالية صامتة مستنية تحصل.

**مقيّد بإعادة الزيارة تحديدًا**: خدمة عمولتها صفر أصلاً، أو طلب الأدمن صفّر عمولته بقرار، ما
بيتلمسوش (`order_type = 'revisit'` شرط صريح). ونسبة غير صفرية اتثبتت على إعادة زيارة ما بتتكتبش
فوقها.

**التحقق**: `scripts/revisit-commission-audit.js` (حي، بيمشي على الـAPI الحقيقي: الفني يقترح ←
العميل يوافق) + 7 تستات في `order-financial-finalization.spec.ts`. النتيجة بعد الإصلاح:
الفني 240.00 / المنصة 60.00.

> ⚠️ النسخة الأولى من سكربت التدقيق كانت بتحاكي الزيادة بـ`UPDATE` خام. دي بتثبت البَقّة بس
> **مستحيل تتحقق من الإصلاح** لأنها مابتعديش على الكود اللي بيصلّح. أي تدقيق مالي هنا لازم
> يعدّي من الـAPI.
