...
- **متوسط وقت الوصول (`avg_arrival_minutes`) ومتوسط مدة التنفيذ (`avg_completion_minutes`) في `GET /technicians/:id/profile` — ✅ اتشالوا نهائيًا (طلب مالك صريح، docs/08 §83، 2026-08-28)**: كانت مقاييس جديدة (تاريخيًا) مبنية على أعمدة موجودة من زمان في `orders` — وقت الوصول = المتوسط بين `technician_departed_at` ("طالع للعميل") و`technician_arrived_at` (وصل فعليًا)، مدة التنفيذ = المتوسط بين `work_started_at` و`work_completed_at`. المالك اعتبرهم "مش دقيقين وما لهمش لازمة" — الخدمة مش معتمدة على السرعة ("إحنا مش أوبر")، ومدة التنفيذ أصلاً بتختلف حسب حجم الشغلانة نفسها فمقارنة فنيين بيها مضلّلة. القرارين اتشالوا بالكامل: الحساب اتشال من `getPublicProfile()`، والحقلين من `PublicTechnicianProfileResponseDto` ومن شاشة/صفحة البروفايل في `apps/customer-app` و`apps/customer-web`. **نطاق القرار محدود عمدًا لصفحة البروفايل بس** — `avg_arrival_minutes` (اسم مختلف، حساب مختلف: بيتحسب inline في استعلام قايمة اختيار الفني `listForServiceBooking()`) لسه موجود في `GET /services/:id/technicians` كـ"وصول متوقع" وقت اختيار الفني — ده سياق مختلف (يساعد قرار فوري بين فنيين متاحين دلوقتي، مش إحصائية تسويقية ثابتة)، ومحدّش طلب شيله.
  - **ملاحظة صريحة من الاختبار الحي (تاريخية)**: مراجعة `on_time_rate` نفسه (المذكور في `docs/08` كأنه "دايمًا null") لقيت إن الحساب أصلاً **صحيح ومش بَقّة** — بيرجّع `null` لسبب سليم (مفيش طلبات مجدولة `scheduled_at` اتنفّذت لسه في بيانات الاختبار)، مش خطأ في المنطق. اتسجّل هنا صراحة عشان محدش "يصلّح" حاجة مش فعلاً عطلانة. `on_time_rate` نفسه **فضل موجود** — القرار كان بس على متوسطي الوصول/التنفيذ.
...
فئة مهارة التسعير الموحدة هي (`beginner`/`standard`/`expert`) وتظهر بالعربية
«مبتدئ/قياسي/خبير»، بنفس لغة سياسة المستحقات. تظل مستقلة عن `TechnicianLevel` التشغيلي
(الترقية، أولوية المطابقة، وأهلية قيادة الفريق)، لكن مصدر السعر نفسه واحد فقط.
...
## «شغال دلوقتي» و«أوفلاين» في مركز العمليات (docs/08 §72، بلاغ مالك 2026-08-27)

**«شغال دلوقتي»** كانت بتتحسب من `ACTIVE_TECHNICIAN_ORDER_STATUSES` اللي بتشمل `accepted` — طلب
اتقبل/اتأكّد تلقائيًا والفني لسه ما تحركش ليه. فالأدمن كان يشوف «شغال دلوقتي» لفني قاعد في بيته.
بقت من `ENGAGED_TECHNICIAN_ORDER_STATUSES` (ADR-0018 §9): في الطريق/واصل/بينفّذ/مستني موافقة عرض.
حجم شغل النهاردة لسه في عمود «القدرة النهاردة» المنفصل.

**`online`** = اتصال socket لحظي مفتوح، وتطبيق الفني بيفتحه **جوّه شاشة تنفيذ الطلب بس**. يعني
فني فاتح التطبيق على الشاشة الرئيسية = «أوفلاين» — صح تقنيًا ومضلّل إداريًا. العمود في الواجهة بقى
**«آخر ظهور»**: «متصل لحظيًا» لو فيه socket، وإلا آخر نشاط جلسة حقيقي.

**فجوة موثّقة صراحة**: مفيش presence حقيقي (الفني فاتح التطبيق دلوقتي) — محتاج heartbeat خفيف من
التطبيق + قرار سياسة (بطارية/تكرار)، مش موجود دلوقتي.

## الرقم القومي كهوية دائمة للفني (ADR-0045، docs/08 §74-ب)

طلب المالك: «لو فني حصل منه مشكلة، ما يروحش جاي رايح مسجل برقم تليفون تاني».

**اكتشاف وقت التنفيذ**: العمود `national_id_encrypted` موجود في الـschema من migration 0005
(وبقى nullable في 0018) لكنه **ما كانش بيتكتب ولا بيتقرا من أي مكان في الكود** — خانة موجودة
ووظيفة مش موجودة. يعني الهوية الوحيدة الفعّالة كانت رقم التليفون، وهي قابلة للتبديل بالكامل.

### التصميم: قيمة مشفّرة + فهرس أعمى

- `national_id_encrypted` — AES-256-GCM. دي اللي المراجع البشري بيشوفها عشان يقارنها بصورة
  البطاقة، وليها endpoint منفصل بصلاحية صريحة (`GET /admin/technicians/:id/national-id`).
- `national_id_hash` — HMAC-SHA256 حتمي. **ده** اللي عليه `UNIQUE` وبيتقارن.

**ليه الاتنين؟** التشفير randomized (IV عشوائي)، فنفس الرقم بيدّي ciphertext مختلف كل مرة ⇒
مستحيل تعمل عليه UNIQUE ولا تقارن بيه. والهاش one-way ⇒ تسريب الجدول مايديش البطاقات. ده النمط
القياسي المعروف بـblind indexing.

**التطبيع قبل الهاش حرج**: أرقام عربية/فارسية → لاتينية، وشيل المسافات/الشرطات. من غيره نفس
البطاقة مكتوبة من كيبورد عربي بتدّي هاش مختلف والتفرّد بيتلف بالكامل.

### التفرّد مشروط بحالة الحساب

`uq_technician_national_id_active` فهرس **جزئي** على `deleted_at IS NULL`:
- فني نشط/معلّق/**محظور** ⇒ رقمه محجوز. **ده جوهر الطلب** — الحساب المحظور ما بيتحذفش، فبيفضل
  ماسك رقمه والحظر يفضل نافذ رغم تغيير التليفون.
- فني حسابه اتشال فعلًا ⇒ الرقم يتحرّر (قرار إداري صريح بالسماح بالرجوع).

الفهرس هو الضمان الحقيقي ضد السباق؛ الفحص في `TechnicianIdentityService` فوقه بس عشان رسالة
خطأ مفهومة.

### مين بيكتبه

- **الفني**: `PATCH /technician/national-id` — **قبل الاعتماد بس**. لو قدر يغيّره بعد الاعتماد،
  الحماية كلها بتتلف (اتحظر ⇒ غيّر رقمه ⇒ سجّل من تاني).
- **الأدمن**: `PATCH /admin/technicians/:id/national-id` — في أي وقت (تصحيح غلطة إدخال لازم
  يفضل ممكن)، بصلاحية `technicians.manage` وبـaudit كامل.

الرقم نفسه **ما بيتسجّلش في الـaudit log** — مقنّع لآخر 4 أرقام. سجل التدقيق دايم وبيتقرا من
ناس كتير.

### محاولة رقم محجوز

الرد على المحاوِل **عام بلا أي تلميح** لمين الرقم (وإلا أي حد بيجرّب أرقام يقدر يستخرج قاعدة
هوية). التفاصيل الكاملة بتعيش في `security_events` بنوع `duplicate_identity_attempt`، وبخطورة
`high` لو الحساب المتعارض محظور/معلّق — ده بالظبط السيناريو اللي المالك خايف منه.

### بَقّة اتلقطت بالاختبار الحي

`revealNationalId()` كان بيرجّع `null` للحساب **المتشال** لأن TypeORM بيفلتر الـsoft-deleted
تلقائيًا في الـquery builder — وده بالظبط أهم حالة للأدمن («الشخص ده كان عندنا قبل كده؟»).
اتصلحت بـ`.withDeleted()`.

### قيد الاعتماد

`technicians.require_national_id_for_approval` (افتراضي `true`) — الأدمن ما يقدرش ينقل فني
لـ`approved` وهو بلا رقم قومي. مفتاح إعداد مش hardcode عشان الاستثناء يتاخد بقرار واعٍ مسجّل.

### فجوة موثّقة

**تغيير `PII_ENCRYPTION_KEY` بعد التشغيل بيبطّل فك التشفير وكل الهاشات المخزّنة** — يعني
التفرّد بيتلف والأرقام القديمة مش هتتقرا. مفيش آلية دوران مفاتيح (key rotation) دلوقتي؛ اتعامل
مع المفتاح كسر دايم.

## حجب خدمات بعينها عن فني (ADR-0049، docs/08 §86، migration 0214)

`technician_excluded_services` — **قائمة حجب**: وجود الصف = ممنوع، وغيابه = مسموح. الفني المعتمد
في فئة بياخد كل خدماتها افتراضيًا (ADR-0018 §8 زي ما هو)، والأدمن بيحجب اللي يختاره من صفحة الفني.

**الفرض في مكان واحد**: `technicianServiceQualificationCondition()` في
`technician-eligibility.sql.ts`. الشرط ده كان مكرر نسخ-ولصق في **تسع** استعلامات، والتوحيد اتعمل
كجزء من الميزة مش كتحسين منفصل — تطبيق الحجب على تمنية من تسعة معناه تسريب صامت: الفني المحجوب
يفضل بيوصله الشغل من المسار المنسي والأدمن شايف إنه محجوب.

المواضع التسعة: التوزيع الفعلي، عضو الشركة في التوزيع، تلات قوايم اختيار للعميل
(`listForServiceBooking` وسوق الفنيين وقايمة الشركات)، تعيين الأدمن القسري، إضافة عضو فريق،
مطابقة المساعدين، وشاشتين تشخيص المطابقة.

**تلاتة مابيطبّقوش الحجب عمدًا** (`admin-workload-forecast`/`admin-operations-overview`/
`admin-coverage-intelligence`): بتحسب تغطية بمستوى الفئة مش أهلية لطلب بعينه.

**تعيين الأدمن القسري بيحترم الحجب** — الأدمن اللي عايز يعيّن فني محجوب بيرفع الحجب الأول (خطوة
واحدة بتسيب أثر في الـaudit). السماح بالتجاوز الصامت كان هيخلّي الحجب اقتراح مش قاعدة.

اختبار حي: `technician-service-exclusions.spec.ts` (6/6) — بينفّذ الشرط الموحّد نفسه على DB حقيقية،
وفيه تأكيد إن فرع الـalias وفرع الـEXISTS بيدّوا نفس النتيجة بالظبط.

## القدرة الاستيعابية اليومية (ADR-0059)

`technician-day-capacity.sql.ts` هو **المصدر الوحيد** لسؤال «الشخص ده عنده متسع للشغل ده؟».

```
لكل طلب ملتزم بيه (قائد أو عضو طاقم):
  يوم البداية (القاهرة) + عدد الأيام (estimated_duration_days) → دقايق على كل يوم
تعارض  ⟺  فيه يوم في مدى الطلب الجديد فيه:  المشغول + الجديد > matching.daily_capacity_minutes
```

**السقف الافتراضي 720 دقيقة (12 ساعة)** — إعداد، مش رقم مدفون.

### ليه الشكل ده بالذات

القاعدة القديمة (ADR-0018 §2) كانت بوليان: «فيه طلب تاني نفس اليوم وشاغل يوم كامل؟». فيها
بَقّتين بنيويتين:

1. **الشغل الممتد كان بيقفل يوم بدايته بس** — شغل 5 أيام يوم 1، وأيام 2-5 الفني «فاضي».
2. **القاعدة كانت غير متماثلة** — بتسأل «هل الطلب **القديم** كبير؟» وبتقيس الجديد بمعيار تاني،
   فالإجابة بتتغيّر حسب مين بيسأل. بلاغ مالك موثّق: «أ» ظاهر لـ«ب» و«ب» مش ظاهر لـ«أ».

النموذج الجديد بيقيس الطرفين بنفس الوحدة (دقايق/يوم)، فالتماثل **خاصية بنيوية** مش إصلاح.

### قاعدة مهمة: الجدولة بالساعة، والشغل الممتد بس هو اللي بياخد اليوم كله (ADR-0077)

الترتيب اللي `perDayMinutesExpr` بيمشي بيه — **المعلومة الأدق الأول**:

| الحالة | الحمل على اليوم |
|---|---|
| `estimated_duration_days > 1` | اليوم بالكامل (شغل ممتد حقيقي) |
| مدة حقيقية بالدقايق معروفة (`duration_minutes`/`duration_hours`) | الدقايق دي بالظبط |
| `estimated_duration_days >= 1` بلا أي مدة بالدقايق | اليوم بالكامل (قالب «باليوم») |
| مفيش أي تقدير | افتراضي الخدمة، وإلا 60 دقيقة |

**ليه `> 1` مش `>= 1`** (البَقّة اللي ADR-0077 قفلها): مصدر `estimated_duration_days` هو
`CatalogService.estimateDuration()` = `Math.ceil(units / productivity)`، والـ`ceil` بتطلّع **1**
لأي شغلانة أقصر من يوم. يعني شغلانة ساعة وشغلانة 12 ساعة كانوا بيوصلوا بنفس الرقم، والقاعدة
القديمة كانت بتحمّل الاتنين يوم كامل. بلاغ المالك: «اديت الفني ساعة الصبح، جيت أديله ساعة الساعة
3 العصر، لقيت السيستم بيقول مش متاح».

حجز الشهر (`estimated_duration_days = 30`) لسه بيبان محجوز الشهر كله — ده الفرع الأول.

**«المدة الحقيقية» و«افتراضي الخدمة» حقلين منفصلين**: `classifyTechnicianCapacity()` بتاخد
`candidateDurationMinutes` (ناتج المحرك، `null` لو مش معروف) و`serviceDurationMinutes` (fallback).
دمجهم في رقم واحد كان بيخلي شغلانة «باليوم» بلا مدة تاخد رقم الخدمة الافتراضي وتتعامل كأنه مدة
حقيقية. اقرا الاتنين من `orderCandidateLoadFields(order)` — نقطة القراءة الوحيدة.

### تقاطع الوقت الحقيقي لازم يوصل لكل بوابة (ADR-0077 §3)

فوق السقف اليومي فيه فرع تاني في `technicianAvailabilityCondition()`: تقاطع نافذة الوقت بنطاق
نصف مفتوح. الفرع ده بيشتغل **بس** لما الكولر يبعت `preciseDurationHoursExpr`. التوزيع والتعيين
ومطابقة المساعدين كانوا بيبعتوه، وقوايم اختيار الفني الأربعة في `technicians.service.ts` **لأ** —
فالقايمة كانت بتعرض فني محجوز في نفس الساعة بالظبط والتعيين بعدها يرفضه. الأربعة بقوا بيبعتوه.

الاختبار الحي: `hourly-schedule-blocking.spec.ts`.

### الصنايعي والمساعد نفس الشيء

الحساب بيقرا الالتزامات من `orders.technician_id` **و**`order_team_members.technician_id`
(UNION). المساعد بطبيعته عضو طاقم مش قائد، فقراءة `orders` لوحدها كانت بتخليه شفاف تمامًا
(ADR-0057). صفر تمييز في الجدولة — الفرق الوحيد بين الاتنين هو التسعير.

### أول طلب في اليوم

`classifyTechnicianCapacity()` بترجّع `LIGHT` لما يكون الحمل صفر في اليوم ده، و`LIGHT` هي
...
## الشركة مظلّة لأعضاء عاديين (ADR-0080)

**قرار المالك (2026-09-06)**: الشركة **مالهاش** نطاق تشغيل خاص. أعضاؤها فنيون عاديون بنطاقهم
هم (خدماتهم/فئاتهم/مناطقهم)، والشركة مجرد مظلّة.

> «الشركة ممكن يكون عندها نجارين وبوابين وحدادين… مش منطقي إني أدي للشركة كل الفئات — ما كده
> ممكن شغل البواب يروح للحداد.»

ADR-0079 (نطاق على مستوى الشركة) اتشال بالكامل في migration `0278` — نطاق الشركة = **اتحاد**
تخصصات أعضائها، وده بالظبط اللي بيسمح بالخلط اللي المالك رفضه.

### اختيار الشركة = توزيع جوّاها بنفس المحرك

`requested_technician_company_id` بيقيّد `findEligibleTechnicians()` على `tp.company_id`. مفيش
محرك تاني «جوّه الشركة»: نفس بوابة الأهلية (خدمة/فئة + منطقة + توافر) على مجموعة أضيق. فشغل
النجارة في منطقة معيّنة يستحيل يوصل لحدّاد — مش لأن فيه قاعدة بتمنعه، لكن لأن **مفيش قاعدة
بتسمح بيه**.

الشركات بقت تظهر في **كل** حجز يقدر العميل يختار فيه منفّذ، مش «اعتماد» بس (كان
`!isTeamBooking || !includeCompanyEntities`، و`allows_team` مقفول على كل الخدمات فعليًا ⇒
الشركات كانت غير مرئية بالكامل). الطوارئ مستثناة من الكولر.

### الشغلانة تتحسب لمين

`orders.assigned_company_id` بيتملى **لو وبس لو** العميل اختار الشركة. كان بيتقرا من
`technician_profiles.company_id` وقت التعيين — يعني أي شغلانة لأي عضو ترسي على الشركة حتى لو
العميل اختار الشخص نفسه. ده كان بيحوّل الانتماء لملكية.

### «حصري للشركة» — `technician_profiles.company_exclusive`

القاعدة سطر واحد مشترك: `technicianIndividualVisibilityCondition()`.

```
(tp.company_exclusive = false OR <parameter الشركة>::uuid IS NOT NULL)
```

**الاستثناء هو المهم**: العلم بيتجاهَل لما الاستعلام يكون مقيّد بشركة أصلاً. من غيره الفني
الحصري كان هيختفي حتى من توزيع شركته هو ⇒ مايشتغلش خالص.

| المسار | الشرط ساري؟ |
|---|---|
| التوزيع التلقائي العام (`findEligibleTechnicians` بلا شركة) | ✅ |
| قايمة اختيار العميل (الأفراد) | ✅ |
| قايمة «مؤهّل بس متعارض» | ✅ |
| «فيه حد متاح اليوم ده؟» | ✅ (إلا لما السؤال عن فني بعينه) |
| فرع الشركات في قايمة الحجز | ❌ عمدًا |
| التوزيع بـ`requested_technician_company_id` | ❌ عمدًا |

القيد `chk_technician_profiles_company_exclusive_needs_company` بيمنع تفعيله لفني مستقل،
و`removeStaff()` بيطفيه تلقائيًا لما العضو يخرج من الشركة.

**مين بيقلّبه**: مالك/مدير الشركة (`PATCH /technician-companies/staff/:userId`) والأدمن
(`PATCH /admin/technicians/:id/company-exclusive`). الفني بيشوفه ومابيغيّروش.

**مش إيقاف**: العضو الحصري معتمد وشغّال بالكامل، بيترقّى وبياخد أرباحه زي أي حد.

الاختبار الحي: `company-as-provider.spec.ts`.

## اقتراح المواعيد: صادق ومتسلسل (ADR-0096، docs/08 §150 بند ١)

اتغيّر حاجتين في `booking-slot-suggestion.service.ts`:

1. **`suggestTimes` بقى بيبتدي من بوابة الحجز الحقيقية** (`technicianAvailabilityCondition`).
   قبل كده كان بيفحص **التداخل الساعي بس**، ففني عدّى سقفه اليومي — مش قابل للحجز في اليوم ده
   خالص — كان بيتعدّ «فاضي الساعة ٢». ده مصدر بلاغ المالك «اتنين في ستة فاضيين» وفيهم واحد
   مشغول. دلوقتي عدّاد الساعة **مجموعة جزئية** من عدّاد اليوم بنيويًا.
2. **`suggestDays` بقى بيرجّع `idleTechnicians` جنب `availableTechnicians`** — الأول «مين ما
   بدأش يومه»، والتاني «مين لسه تحت السقف». الترتيب بيوزن بينهم بـ
   `booking.suggestion_sequencing_weight` (٠.٥ افتراضيًا، صفر = سلوك ADR-0088 بالحرف). ده اللي
   بيوزّع الشغل بدل ما يرصّه على نفس الناس.

**حمل اليوم بيتقري من `technicianDayLoadSubquery()`** — نفس الدالة اللي التوزيع والتصنيف
بيستخدموها. استعلام حمل جديد هنا كان هيبقى مصدر حقيقة رابع، وهو بالظبط العيب اللي البند ١
بيصلحه.

**فخ SQL اتلقط حيًا**: `scheduledAtParam` بيتلحقه `::timestamptz` جوّه الشرط، فتعبير فيه
`AT TIME ZONE` لازم يتلف بأقواس — من غيرها الكاست بيقع على اسم المنطقة الزمنية نفسه.

مقيس في `booking-suggestion-sequencing.spec.ts` (٣ حالات)، واتعمل اختبار سلبي للبند ١: شيل
البوابة ⇒ الحالة بتفشل فعلاً.

## أداء اقتراح الأيام — أضعف نقطة في مسار الحجز (docs/08 §152)

القياس (٤٠ فني، نصهم عليه شغل، `scripts/booking-path-latency.js`): `GET /booking-slots/days`
هو **أغلى نداء في الرحلة** — p50 8ms (من الكاش) لكن أسوأ حالة كانت **2879ms**. باقي المسار
كله تحت 135ms.

تلات حاجات اتعملت، والأولى هي الأهم:

1. **دمج الحسابات المتوازية (single-flight)** — الكاش بيغطّي الطلب التاني، لكن الطلبات
   المتوازية على مفتاح **بارد** كلها بتلاقيه فاضي فكلها بتحسب. مقاس: ٢٠ نداء متوازي =
   **2309ms** مقابل 91ms على مفتاح ساخن. دلوقتي أول واحد يحسب والباقي يستنى نفس الوعد ⇒
   **661ms**. الخريطة في الذاكرة (لكل نسخة) مش قفل موزّع — مقصود، والمسار استشاري أصلاً.
2. **المجمّع وحمل الأيام مرة واحدة مش مرة لكل يوم** — شروط الخدمة/النطاق/الموقع مالهاش علاقة
   باليوم وكانت بتتنفّذ ٢٢ مرة. وعدّاد التسلسل (§150) كان **118ms من 396** كاستعلام مترابط
   لكل (يوم × فني)؛ بقى `JOIN` واحد على CTE.
3. أسوأ حالة بقت **454ms**.

**قبل أي تعديل على الاستعلام ده**: شغّل `scripts/booking-path-latency.js` الأول وبعدين تاني —
الرقم ده بيكبر بعدد الفنيين، فأي إضافة مترابطة جوّه الـLATERAL تكلفتها بتتضاعف ٢٢ مرة.

## مؤشر الوصول — مفهومين حسب أفق الطلب (ADR-0099، docs/08 §153)

«وصول متوقع» القديم مكانش تنبؤًا: كان متوسط `arrived_at − departed_at` على **كل** تاريخ الفني
وفي **كل** المناطق — فمالوش علاقة بالمسافة جنبه، ومالوش معنى لطلب بعد أسبوع، وصفره ناتج بيانات
ناقصة.

دلوقتي `technician-arrival-metrics.ts` بيقرر:
- **فوري/قريب** ⇒ `expected_arrival_minutes` = متوسط الانتقال **في نطاق الطلب** (fallback للعام).
- **مجدول** ⇒ `punctuality` = نسبة + حجم عيّنة + متوسط تأخير على المتأخرين وحدهم.

**القيمة اللي مش بتاعة الوضع الحالي بترجع `null` من السيرفر** — مش بتتبعت وتتجاهل في الواجهة.

العتبة هي **نفس** `matching.near_term_request_hours` بتاعت التوزيع ووزن المسافة — مقصود إنها
نفس المفتاح، وإلا الأدمن يغيّرها ويلاقي المؤشر بيقول حاجة والتوزيع بيعمل حاجة تانية.

**قاعدتين لازم تفضلوا**: صفر دقيقة مابيتعرضش (بيانات ناقصة مش قيمة)، ونسبة الالتزام ليها حد
عيّنة (`matching.min_punctuality_sample`) — بس ٠٪ من عيّنة كفاية **بيتعرض**، ده خبر حقيقي.

### `service_completed_count` مقابل `total_completed_count`

الأول للخدمة دي وحدها (`technician_services.completed_count`)، والتاني إجمالي المنصّة
(`technician_profiles.completed_orders_count`). كانوا حقل واحد اسمه `completed_orders_count`
بقيمة الأول — فالكارت كان بيعرض «0 طلب مكتمل» جنب تقييم عام، وبيقرا كتناقض.

### اسم المستوى بيتبعت من السيرفر

`technician_level_label_ar` من `technician_level_config.display_name_ar`. التطبيق كان عنده
خريطة ثابتة بتقول «مميز» والأدمن ضابط «بريميوم» — قيمتين لنفس الحاجة.

## الاقتراح بيقيس الشغل كامل، مش يوم بدايته (ADR-0100، docs/08 §155)

طلب المالك: السؤال اتغيّر من **«كام فني فاضي في يوم X؟»** لـ**«لو الشغل بدأ في يوم X، كام منفّذ
مؤهّل يقدر يشيل الشغل كاملًا حسب مدته ومتطلباته؟»**.

### أهم حاجة تفهمها قبل ما تلمس الملف ده: المحرك كان سليم، السؤال كان ناقص

ماكانش فيه أي بَقّة في منطق المدى. كل الماكينة موجودة وشغّالة:

- `dailyCapacityExceededExpr()` بتمشي على `generate_series` لكل أيام المرشّح، وبترفضه لو **أي
  يوم** عدّى السقف. يعني «يوم البداية فاضي وبعد يومين متعارض» **متغطّية بنيويًا**.
- `technicianDayLoadSubquery()` بتفرد كل طلب قائم على أيامه كلها، فالشغل الممتد مرئي في كل يوم.
- `blockedExistsExpr()` بتمتد على نافذة المرشّح كلها (إجازة في نص المدة بتتلقط).

**المدى نفسه** بيتشتق من مدة المرشّح (`candidateSpanDaysFromSource()`)، و`suggestDays` كانت
بتستقبل `null` في المدة **دايمًا** لأن التطبيق ماكانش بيبعتها:

```sql
GREATEST(COALESCE(CEIL(NULL::numeric)::int, 1), 1)  =  1
```

فالمدى يطلع يوم واحد، والماكينة كلها تشتغل على يوم البداية وبس. المشكلة كانت في **ترتيب رحلة
الحجز** (المدة مستحيل تكون معروفة قبل ما تفاصيل الشغل تتجمع، وتفاصيل الشغل كانت بتتجمع **بعد**
الموعد) — مش في الـSQL.

### اللي اتغيّر هنا فعلاً

بند واحد بس في الموديول ده: **`suggestTimes` كانت بتحقن `estimatedDurationDaysExpr: 'NULL::numeric'`
نصًا**، فمداها كان يوم واحد مهما كانت المدة — حتى لو الكولر بعت المدة. دلوقتي بتاخد نفس مصدر
الحمل بتاع `suggestDays` (`$10`/`$11`).

ومعاه فصل ضروري كان مدموج في رقم واحد (`$7`):

| المفهوم | الپارامتر | بيتحصر على | بيُستخدم في |
|---|---|---|---|
| نافذة الساعة | `$7` (`slotWindowMinutes`) | السقف اليومي | تقاطع الطلبات القائمة |
| حمل الشغلانة | `$10`/`$11` | مفيش | `candidateSpanDaysFromSource()` |

دمجهم كان بيكسر الاتجاهين لشغل ممتد: نافذة تقاطع بـ٦٠ ساعة من ناحية، ومدى بيوم واحد من ناحية.
والحد في الـDTO (`Max(1440)`) كان **بيمنع** إن المدة الحقيقية توصل أصلاً — بقى `10_080` زي
`SuggestedDaysQueryDto`.

### التحقق

`node scripts/verify-whole-job-suggestions.js` — **١٥ فحص حي**، كل واحد بيتقرا **مرتين على نفس
البيانات**: مرة بلا مدة (الضابط) ومرة بالمدة الحقيقية. الضابط جزء أصيل: من غيره «المنفّذ اتشال»
ممكن يبقى صح لأي سبب تاني.

أهم الفحوص: شغلانة ٣ ساعات تستبعد اللي مالوش ٣ ساعات فاضية؛ شغل ٥ أيام يستبعد **اللي فاضي ٤
أيام ومشغول في الخامس**؛ المدة بالأيام (بلا دقايق) بتفحص المدى برضه؛ العنوان التاني بيدّي نتيجة
مختلفة؛ **شغلانة الساعة نفس ناتجها بالحرف قبل وبعد**.

**الاختبار العكسي اتعمل**: إرجاع `estimatedDurationDaysExpr` لـ`NULL` (مع إبقاء الپارامتر مربوط
بـ`NULLIF` عشان مايبقاش خطأ بناء) خلّى الضابط ينجح والفحصين الممتدين **يفشلوا** — المنفّذ المشغول
رجع يتعدّ فاضي. فالفحوص بتقيس حاجة فعلاً.

### فخ في كتابة اختبار للاقتراح

الاقتراح بيرجّع **٣ أيام مفرودة** من أفق ٢١ يوم (`suggestion_count` + `pickSpread`)، فسؤال
«اليوم ده اتشال؟» مالوش إجابة: اليوم ممكن يغيب لأنه مستبعد، أو لأن `pickSpread` اختار غيره.
السكربت بيرفع `suggestion_count`/`horizon_days` ويصفّر التقارب والكاش **وبيستنى لحد ما الإعداد
يبان في الرد فعلاً** (أول تشغيل بعد تغيير الإعدادات رجّع ٣ أيام وفشّل فحصًا بلا سبب حقيقي).
والقيم الأصلية بترجع مكانها في `finally`.

### انفجار الأداء اللي المدى الحقيقي كشفه (ADR-0100 §6)

أول قياس بعد ما المدة بقت بتتبعت (٤٠ فني، أفق ٢١ يوم، بلا كاش):
...
`technicianDayLoadSubquery()` **مترابطة** (correlated) بمعرّف الفني، فجوّه
`dailyCapacityExceededExpr` بتتنفّذ **مرة لكل يوم في المدى**. والأسوأ إن `suggestDays` كانت
**أصلاً** بتحسب نفس الأرقام مرة واحدة لكل (فني، يوم) في CTE اسمه `day_load` — بس بتستخدمه
لعدّ الفاضيين بس، فنفس الحساب كان بيتعمل مرتين وواحدة منهم متكررة.

**الإصلاح**: `dailyCapacityExceededExpr` خدت `dayLoadRelation` اختيارية — علاقة SQL بترجّع
`(busy_day, busy_minutes)` بنفس عقد `technicianDayLoadSubquery` بالحرف، والافتراضي هو الدالة
نفسها. `suggestDays` بتبعت `day_load` الجاهز.

⚠️ **لو بتضيف كولر جديد**: مابتبعتش الخيار = السلوك القديم بالحرف. وابعته **بس** لو عندك فعلاً
حمل محسوب لكل (فني، يوم) بنفس الپارامترات (`activeStatusesParam`/`excludeOrderIdParam`/
`dailyCapacityParam`) — علاقة بپارامترات مختلفة هتدّي إجابة غلط بصمت.

النتيجة: مدى ٥ أيام من 4635ms لـ**43ms**، و`/booking-slots/days` p95 من 961ms لـ**96ms**.

## سياسة تجنيد الشركة (ADR-0086 + تعديل ١، docs/08 §163)

`technician_companies.allows_external_recruitment` بيحدد: لما طلب الشركة يحتاج فنيين أو
مساعدين زيادة، القائد يقدر يدعو مين؟

| القيمة | المرشّحين | مين بيقرر |
|---|---|---|
| `true` (**الافتراضي** من migration 0353) | أي فني/مساعد مؤهّل على المنصة، وأعضاء الشركة فوق القايمة وعليهم بادج | الوضع التلقائي |
| `false` | أعضاء الشركة بس | قرار أدمن صريح، متسجّل في `audit_logs` |

**القيد بيسري على طلبات الشركة بس**: الشرط بيتفعّل لما `orders.assigned_company_id` = الشركة
دي. نفس الفني لما ييجي له طلب **خاص بيه** بيتعامل كمستقل تمامًا (ADR-0086 بند ٩،
`MatchingService.resolveAssignedCompanyId()`).

### ليه الافتراضي اتعكس

ADR-0086 خلّاه `false` عمدًا («فتح الباب قرار إداري صريح»)، بس **الزرار اللي ياخد القرار ده
مكانش موجود**: الـendpoint موجود ومعاه audit، لكن مفيش واجهة أدمن، و`CompanyResponseDto`
المشترك مكانش بيعلن الحقل أصلاً. فالقفل الافتراضي بقى دائم وصامت. التعديل بيعكس الافتراضي
**ويبني الزرار** — الاتنين مع بعض، لأن افتراضي مقفول مالوش معنى من غير طريقة تفتحه.

### الشرط نفسه

`companyRecruitmentScopeCondition()` في `technician-eligibility.sql.ts` — fragment مسمّى
عشان `OrderTeamService.listRecruitCandidates()` والاختبار يشغّلوا **نفس النص**. السياسة فضلت
بلا أي اختبار من ADR-0086 لحد §163؛ دلوقتي `company-recruitment-scope.spec.ts` بيغطيها حي
على Postgres (والاختبار اتُختبر سلبيًا بتعطيل الشرط).

### ⚠️ عضوية الشركة بتتقاس على شركة الطلب

`isLeaderTeamMember` بقى `tp.company_id = COALESCE(o.assigned_company_id, $leaderCompanyId)`.
