...
- **`GET /technicians/:id/schedule`** (عام، `@Roles(CUSTOMER)`) — نسخة العميل "أخضر/أحمر" بس (`is_available` boolean)، مفيش `order_id` ولا `notes_ar` داخلية (`PublicScheduleSlotResponseDto` منفصل عمداً عن `ScheduleSlotResponseDto`).
