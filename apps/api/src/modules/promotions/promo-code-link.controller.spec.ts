...
describe('PromoCodeLinkController', () => {
  it('يسجل زيارة الرابط الصالح ثم يحوّل إلى رحلة العميل بالكود', async () => {
    const links = {
      ...
      findActiveByCode: jest.fn().mockResolvedValue({ id: 'promo-id', code: 'SAVE20' }),
    ...
    expect(links.resolveDestination).toHaveBeenCalledWith('web', 'SAVE20');
  ...
  it('لا يعرض خطأ للزائر إن كان الرابط موقوفًا، ولا يضيف كودًا موقوفًا للرحلة', async () => {
    ...
    expect(links.resolveDestination).toHaveBeenCalledWith('web', '');
