...
## الجولات والتصعيد عند انتهاء المهلة (ADR-0061 §4، 2026-09-02)

`AssistantOfferExpiryProcessor` (BullMQ، نفس نمط `MatchingRoundExpiryProcessor` بالحرف) —
بيعلّم العروض المعلّقة `expired`، وبعدين **بيبعت الدفعة اللي بعدها** (`matching_round + 1`،
migration 0245). التصعيد (`ASSISTANT_MATCHING_ESCALATED_EVENT`) بيحصل **بس** لما جولة ترجّع صفر
مرشّح جديد = المجمع خلص فعلاً.

قبل كده كان التصعيد بعد الجولة الأولى على طول — الأدمن بيتنده حتى لو فيه عشرات المساعدين
المؤهلين المتاحين لسه ماتسألوش أصلاً. تصعيد كاذب بيحوّل شغل الغطاء التلقائي لشغل يدوي.

مفيش سقف صناعي لعدد الجولات: كل مرشّح اتبعتله عرض بيتسجّل ويتستبعد من الدفعة اللي بعدها
(`excludeIds`)، فعدد الجولات محدود ببنيته بحجم المجمع. ومعرّف job المهلة بقى فيه رقم الجولة —
BullMQ بيتجاهل `add()` لمعرّف موجود حتى لو الـjob خلص، فمعرّف واحد لكل الجولات كان معناه إن
مؤقّت الجولة التانية مايشتغلش خالص والطلب يعلّق للأبد. اختبار حي: `assistant-matching-rounds.spec.ts`.
...
- ~~فحص تعارض جدولة صريح (`technician_schedule_slots`) للمساعد~~ — **اتقفلت (2026-08-13)**،
  و**اتصلحت من جذرها (ADR-0061 §3، 2026-09-02)**. النسخة القديمة كانت مكتوبة بإيد هنا وتعليقها
  بيقول «نفس منطق `findEligibleTechnicians()` بالحرف» — وماكانتش. تلات فروق حقيقية اتلقطت:
  (1) أي طلب نشط للمساعد في **أي يوم مهما كان قصير** كان بيستبعده — بَقّة ADR-0018 §32 اللي
  اتصلحت للقائد وفضلت عايشة هنا؛ (2) السقف اليومي بالدقايق (ADR-0059) ماكانش مطبَّق خالص، فمساعد
  يومه مليان 12 ساعة ومساعد فاضي كانوا نفس الشيء؛ (3) كان بيفحص `status = 'booked'` بدل
  `'blocked'`، يعني **إجازة المساعد الصريحة ماكانتش بتتقرا أصلاً**. دلوقتي الاتنين بينادوا
  `technicianAvailabilityCondition()` نفسها. اختبار حي: `assistant-availability-parity.spec.ts`.

  **الدرس**: تعليق بيقول «نفس المنطق» مش ضمانة. الضمانة الوحيدة هي نداء نفس الدالة.
...
## بَقّة حقيقية — ~20 إشعار "فرصة مساعدة" دفعة واحدة لنفس الفني (docs/08 §92، طلب مالك مباشر)

المالك بلّغ إن فني وصله حوالي 20 إشعار "فرصة مساعدة جديدة" مرة واحدة، وطلب صراحة تباعد
(مش أكتر من كل ~5 دقايق). السبب: `AssistantMatchingRecoveryService` (sweep كل 60 ثانية) بتعالج
لغاية 25 طلب محتاج مساعد في نفس الجولة (`BATCH_SIZE`)، وكل طلب مؤهّل بينادي `broadcastToPool()`
مستقل — لو نفس الفني قريب/مؤهّل لعدد كبير من الطلبات دي في نفس اللحظة (وارد جدًا لو الرصيد قليل،
أو بعد تراكم backlog)، `for` loop في `sweep()` بيبعتلهم كلهم ورا بعض بلا أي تباعد. كل عرض
(`order_assistant_offers`) شرعي ومحتاج يتخلق فعلاً — المشكلة كانت في **الإزعاج**، مش في صحة
العروض نفسها. زيادة على كده: النص نفسه ("حد محتاج مساعد على شغلانة قريبة منك") مكانش فيه أي هوية
للطلب، فحتى لو كانت فرص حقيقية مختلفة كانت بتبان للفني كأنها نفس الإشعار متكرر بالظبط.

**الإصلاح** (`AssistantOpportunityNotificationListener`، موديول notifications):
1. النص بقى فيه رقم الطلب (`AssistantOpportunityOfferedEvent` اتضافلها `orderNumber`، مُمرّرة من
   `broadcastToPool()` وقت الـemit) — لو حصل تباعد طبيعي، الفني يقدر يميّز فرصة عن التانية.
2. **تهدئة 5 دقايق لكل فني**: لو الفني أخد إشعار "فرصة مساعدة" خلال آخر 5 دقايق، الإشعار الجديد
   بيتخطّى (العرض نفسه لسه بيتسجّل في `order_assistant_offers` عادي — الفني يقدر يشوفه لو فتح
   قايمة عروض المساعدة بنفسه، الإشعار بس اللي بيتأجّل). مفيش تعديل على `broadcastToPool()`/
   `sweep()` نفسهم — التهدئة في طبقة الإشعار بس، عشان محرك المطابقة يفضل زي ما هو.

اختبار حي جديد: `assistant-opportunity-notification.listener.spec.ts` (3/3).
