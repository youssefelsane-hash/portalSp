...
**بَقّة كانت موثّقة هنا واتصلحت (2026-08-13)**: استعلامات استبعاد "الفني عنده طلب نشط بالفعل"
(هنا في `isCandidateEligible()`/`broadcastToPool()` وفي `matching.service.ts`'s
`findEligibleTechnicians`) كانت بتفحص `order_status` بس، **من غير فلترة `deleted_at IS NULL`** —
لو صف طلب اتعمله soft-delete (نادر، مش مسار عادي) لكن `order_status` فضل على قيمة نشطة (`accepted`
مثلاً بدل حالة إلغاء صريحة)، الفني كان بيفضل "محبوس" كمشغول للأبد رغم إن الطلب نفسه مش ظاهر لحد.
**الإصلاح**: `AND deleted_at IS NULL` (أو `AND o.deleted_at IS NULL` لاستعلامات الـjoin على
...
اختبار regression حي ضد Postgres حقيقي في `../matching/matching.service.spec.ts` بيثبت السلوك
(طلب soft-deleted مبيستبعدش الفني تاني، وطلب حقيقي لسه بيستبعده زي الأول).
