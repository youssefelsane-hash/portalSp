...
**فجوة تانية اتلاحظت أثناء الاختبار، موثّقة صراحة (مش سهو، ومش جزء من نطاق هذا الـADR)**: استعلامات
استبعاد "الفني عنده طلب نشط بالفعل" (هنا وفي `matching.service.ts`'s `findEligibleTechnicians`
الأصلية) بتفحص `order_status` بس، **من غير فلترة `deleted_at IS NULL`** — لو صف طلب اتعمله
soft-delete (نادر، مش مسار عادي) لكن `order_status` فضل على قيمة نشطة (`accepted` مثلاً بدل
حالة إلغاء صريحة)، الفني بيفضل "محبوس" كـmشغول للأبد رغم إن الطلب نفسه مش ظاهر لحد. مش بَقّة في
الكود الجديد هنا تحديدًا (اتّبعنا نفس نمط `matching.service.ts` المستقر بالحرف)، لكن جدير التوثيق
عشان أي سيشن جاية تلاقيه لو قابلته تاني.
