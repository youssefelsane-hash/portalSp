...
**موديول `orders` شغال (S3 + S5) — الإنشاء، الإلغاء، ودورة عمل الفني:**
...
- اتعمله اختبار end-to-end فعلي كامل: إنشاء طلب → قبول فني → المسار الكامل depart→arrive→start (مع رفض محاولة `complete` قبل الأوان) → رفع صورة PNG حقيقية اتكتبت على القرص فعلاً → complete → تاريخ حالات كامل (7 خطوات) مطابق للقاموس.
