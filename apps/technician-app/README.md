...
- **باقي الميزات الفعلية**: ملاحة (تتبع موقع حي عبر `OrderTrackingGateway` — الباك-إند شغال فعلاً، التطبيق لسه مش موصّله)، رفع صور قبل/بعد (`POST /technician/orders/:id/media` — الباك-إند شغال، التطبيق لسه مش موصّله)، الأرباح، طلب الصرف.
