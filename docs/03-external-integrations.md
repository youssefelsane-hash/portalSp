...
#### ⚠️ تلات قيود بتفشل **وقت الإرسال** مش وقت البناء (تدقيق 2026-09-11)

بوابة الـSMS هي القناة الوحيدة لتسليم كود التحقق — أي واحد من التلاتة دول معناه **صفر تسجيل
دخول لأي مستخدم حقيقي**، والسيرفر بيفضل بيقول "healthy". اتقفلوا بالكود والاختبار
(`cequens-contract.spec.ts`):

**١. طول الرسالة — كانت بتتبعت جزئين وتكلفتها مضاعفة**

أي نص فيه عربي بيتبعت بترميز **UCS-2**، وحدّه **٧٠ وحدة UTF-16** للرسالة الواحدة (و٦٧ لكل
جزء في المجزأة، لأن رأس التجزئة بياخد من المساحة). نص كود التحقق القديم كان **٧٦ وحدة**:
...
يعني **كل كود تحقق كان بيتحاسب عليه جزئين** — وهو أكتر رسالة بتتبعت في المنصة كلها. وزيادة
٦ وحدات بس هي اللي كانت بتعمل كده. النص الحالي **٦١ وحدة** (فاضل ٩ هامش يستحمل
`OTP_EXPIRY_MINUTES` لحد ٤ خانات)، والاختبار بيقيسه فعليًا على مدد مختلفة فأي تعديل بيكسر
الحد بيرسّب قبل ما يشحن.

**٢. طول اسم المُرسِل (Sender ID)**

معيار GSM بيحدّ الاسم **الأبجدي بـ١١ محرف** والـ**رقمي بـ١٥ رقم**. قيمة أطول المزوّد بيرفضها
وقت الإرسال. `CEQUENS_SENDER_NAME` بقى عليه فحص في `env.validation.ts` بيمنع الإقلاع أصلاً
بدل ما نكتشف بفشل كل كود تحقق.

**٣. شكل رقم المستلم — `CEQUENS_RECIPIENT_FORMAT`**

إحنا بنخزّن E.164 بالـ`+` (`+201000000777`)، وأمثلة CEQUENS في التوثيق بتستخدم الرقم الدولي
**من غير `+`** (`201000000777`). مابنراهنش على واحد:

| القيمة | اللي بيتبعت | إمتى |
|---|---|---|
| `msisdn` (الافتراضي) | `201000000777` | شكل أمثلة CEQUENS |
| `e164` | `+201000000777` | لو الحساب طلب الـ`+` |

**ده أول حاجة تتأكد منها في أول إرسال حقيقي**: ابعت كود تحقق لرقمك، ولو مجاش جرّب القيمة
التانية — بلا أي تعديل كود. نفس فلسفة `CEQUENS_BASE_URL`/`CEQUENS_AUTH_URL`.
