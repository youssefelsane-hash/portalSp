...
### P0-2 — مفتاح Google Maps ظاهر داخل Git — **اتقفل في الكود (2026-08-29)**

المفاتيح **اتشالت من git بالكامل** وبقت تتقرا من مصادر غير متتبَّعة:

| المنصة | المصدر الجديد |
|---|---|
| Android | `android/maps.properties` (غير متتبَّع، فيه `maps.properties.example` كقالب) → `manifestPlaceholders` |
| iOS | مفتاح `GoogleMapsApiKey` في `Info.plist` جاي من `$(GOOGLE_MAPS_API_KEY)` في xcconfig غير متتبَّع |
| Web | `__GOOGLE_MAPS_WEB_API_KEY__` بيتستبدل وقت النشر؛ من غير استبدال السكربت مابيتحمّلش أصلاً |

الغياب = الخرائط معطّلة بتحذير واضح، **مش** رجوع لمفتاح مكتوب في الكود — وده مقصود: أفضل من
مفتاح مسرّب شغّال.

**اللي لسه مطلوب منك (مش كود)**: تدوير المفتاحين القديمين من Google Cloud (إخفاؤهم دلوقتي
مايلغيش إنهم اتسرّبوا في تاريخ git بالفعل)، وتقييد الجداد بالـpackage ID + بصمة توقيع Play
للأندرويد، وبالدومينات للويب، ووضع Quotas وBudget Alerts.
...
### P0-3 — إصدار Android وتوقيعه — **البند الحرج اتقفل (2026-08-29)**

«البناء الحالي يرجع لتوقيع Debug إذا غاب `key.properties`. إصدار المتجر يجب أن **يفشل** بدل أن
ينتج نسخة Debug بصمت» — اتنفّذ في التطبيقين: حارس `gradle.taskGraph` بيفصل الحالتين بالظبط،
`bundleRelease` (ناتج المتجر الوحيد) بيفشل فورًا برسالة عربية واضحة، و`flutter run --release`
المحلي بيفضل شغّال بتوقيع debug زي ما هو.

باقي بنود P0-3 (تثبيت package IDs، إنشاء الـkeystore وحفظه، Play App Signing، رفع `versionCode`)
خطوات تشغيلية عندك مش كود.
...
### P0-4 — صلاحية Full Screen في تطبيق الفني — **اتقفلت (2026-08-29)**

`USE_FULL_SCREEN_INTENT` **اتشالت من الـmanifest**، و`fullScreenIntent` بقى `false`،
و`category` اتغيّرت من `call` (ادّعاء إنه نداء مكالمة — وده بالظبط اللي المراجعة بتضرب فيه)
لـ`event`.

**الإلحاح ما ضاعش**: `Importance.max` + `Priority.max` + `visibility: public` بيخلّوا العرض
يبان كـheads-up فوق أي شاشة وبمحتواه مقروء على شاشة القفل. الفرق الوحيد إن الشاشة مبتفتحش
بالكامل قسرًا — تنازل مقبول مقابل إن التطبيق يعدّي المراجعة أصلاً. لو الأعمال قررت بعدين إنها
محتاجة الشاشة الكاملة فعلاً، لازم declaration مبرَّر مع الرفع.
