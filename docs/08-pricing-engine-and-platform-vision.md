...
### تحديث تنفيذ §20 — بند 12 (proof-of-work — الباك-إند لازم يفرض الثوابت المهمة) — 🔲 `قرار عمل مطلوب من المالك، صفر كود` (تحقيق كامل، مفيش سياسة اتخترعت)
...
## 🔖 CURRENT HANDOFF — §20 Financial Operations Audit (checkpoint 2026-08-14، جلسة لاحقة — §20.12 اتحقّق بالكامل، مقفول على قرار مالك)
...
### ✅ Completed and verified (§20.1–§20.11), 🔲 §20.12 investigated fully (blocked on owner decision, zero code) — DO NOT reopen unless §20.13+ finds a real regression touching them
...
- **§20.11** — فجوة عرض حقيقية اتقفلت (endpoint + UI جديدين، صفر جدول/عمود جديد): `platform_
  commission_cents`/`technician_earning_cents` محسوبين ومخزّنين من زمان بس صفر مكان كان بيعرضهم.
  `PaymentsService.getFinancialSummaryForOrder()` جديدة بتلمّ العمولة/الأرباح/رسوم الإلغاء من صف
  الطلب + كل `payments`/`refunds` المرتبطة (`WHERE order_id`)، بلا أي حساب جديد. `GET /admin/
  orders/:id/financial-summary` endpoint جديد. `apps/admin` (`/orders/:id`) بقى فيه كارت "الملخص
  المالي" مستقل. مُختبر حيًا (`order-financial-summary.spec.ts`، اختبارين).
- **§20.12 — 🔲 اتحقّق بالكامل، صفر كود (مقفول على قرار مالك)**: تفاصيل كاملة فوق في "### تحديث
  تنفيذ §20 — بند 12". **اكتشاف مهم**: افتراض §20's الأصلي كان "الواجهة بتفرض proof-of-work بس
  الباك-إند لأ" — ده **غلط**. التحقيق أثبت إن **مفيش فرض خالص لا في الباك-إند ولا في الواجهة**:
  `OrdersService.complete()` بتنتقل لـ`WORK_COMPLETED` بلا أي شرط وجود صور، زرار "إنهاء الشغل" في
  `apps/technician-app` مش مربوط بوجود `after_photo`، و`signature` (توقيع العميل) معرّف في الـschema
  بس صفر أي استخدام في أي تطبيق. **صفر كود اتكتب عمداً** — أي تنفيذ محتاج إجابات مالك على 5 أسئلة
  سياسة عمل مش معرّفة حاليًا (كام صورة إجبارية، لأي أنواع طلبات، هل التوقيع مطلوب فعلاً، إلخ) —
  قائمة كاملة فوق. لو المالك يجاوب، التنفيذ نفسه بسيط (فحص واحد + إعداد `SettingsService`).

**آخر commit مدفوع فعليًا لكل شغل §20.1-11**: `f75ff15`. §20.12 مالوش commit كود (صفر تغيير) — بس
لو التوثيق فوق اتدفع، دور على commit `docs(§20 بند 12)` أو زيه في `git log`.

**آخر full test gate نجح**: 40 test suite / 209 test (apps/api)، `tsc --noEmit`/`next build` نضيفين
في `apps/admin` (39 صفحة) — نفس baseline بند 11 بالظبط (§20.12 صفر كود). **ملحوظة تشغيلية ثابتة
(سبقت §20.10، هتفضل تتكرر)**: `apps/admin` محتاجة `cd packages/shared-types && npm run build` يدوي
قبل `tsc --noEmit` بتاعتها كل مرة تتعدّل فيها أنواع مشتركة (`packages/shared-types`) — مفيش watch
mode (فجوة معروفة من `CLAUDE.md`). لو شفت أخطاء غريبة في `apps/admin` زي `Module has no exported
member`، ده السبب الأرجح، مش بَقّة حقيقية.

### النقاط اللي لسه محتاجة تحقيق (§20.13-§20.14) — صفر تحقيق اتعمل فيهم لحد دلوقتي

§20.13 (اختبار ثوابت A-N من طلب المالك الأصلي — النص الكامل "محفوظ في سجل المحادثة" حسب هاندوف سابق،
لسه محتاج استخراج/توضيح لو مش موجود بالفعل في مكان تاني بالملف ده)، §20.14 (30 سيناريو واقعي من طلب
المالك، تصنيف PASS/PARTIAL/GAP لكل واحد، سد الـGAP الحقيقي بس) — كلهم زي ما هم من أول الجلسة، صفر
تقدّم حقيقي.

### Next exact action لأي سيشن جديدة

1. ابدأ بـ§20.13 (اختبار ثوابت A-N). **الخطوة الأولى قبل أي حاجة**: دوّر كويس في `docs/08` كامل
   وباقي ملفات `docs/` عن أي ذكر لـ"ثوابت A-N" أو نص مشابه — لو مش موجود، ده مش تحقيق تقني، دي فجوة
   معلومات من المالك. **وثّقها صراحة كقرار/معلومة مطلوبة (إعادة إرسال نص الثوابت) بدل ما تخترع
   ثوابت بنفسك** — بالظبط نفس نمط §20.12 اللي فوق. لو النص موجود فعلاً (يمكن في جزء تاني من الملف
   ده اتقرا في سيشن سابقة ومش موجود في السياق الحالي)، ابدأ تنفيذه بند ببند.
2. نفس الكلام لـ§20.14 (30 سيناريو) — لو النص مش موجود، وثّق الفجوة بدل الاختراع.
3. اتّبع نفس الانضباط المتبع في §20.1-12: تحقيق كود حقيقي أول (مش افتراض) → إصلاح/بناء لو فيه فجوة
   حقيقية بس (صفر سياسة عقابية مخترعة، صفر إعادة تصميم غير مطلوب) → `tsc --noEmit` (اعمل
   `cd packages/shared-types && npm run build` الأول لو محتاج) → `nest build`/`next build` →
   `jest`/اختبار الشاشة يدويًا → توثيق في `docs/08` (نفس نمط "### تحديث تنفيذ §20 — بند N") + README
   الموديول المتأثر → commit → push.
4. كمّل بالترتيب لحد §20.14، كل بند لوحده commit+push، مايتأجّلش التوثيق للآخر.
