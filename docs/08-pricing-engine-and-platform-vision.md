...
## 🔖 CURRENT HANDOFF — §20 Financial Operations Audit (checkpoint 2026-08-14، جلسة لاحقة — §20.7 اتقفل)
...
### ✅ Completed and verified (§20.1–§20.7) — DO NOT reopen unless §20.8+ finds a real regression touching them
...
**آخر commit مدفوع فعليًا لكل شغل §20.1-6**: `ca81434`. **§20.7's commit**: اتحقق من `git log` بعد
الـpush التالي مباشرة لهذا الـcheckpoint — لو مش لاقيه، يبقى لسه ماتدفعش، كمّل من نقطة "Next exact
action" تحت.

**آخر full test gate نجح (بعد §20.7)**: 38 test suite / 205 test — كلهم عدّوا، `tsc --noEmit`
و`nest build` نضيفين. الملف الجديد: `apps/api/src/modules/orders/orders-cancel-prepaid-refund.spec.ts`
(4 اختبارات حية).
...
1. ابدأ بـ§20.8 (تعليق صرف الفني وقت نزاع) — دقّق: هل `PayoutsService`/`linkOrderItemsForPayout()`
   بتستبعد طلبات فيها complaint/dispute مفتوح (`complaints` table، `ComplaintStatus`) قبل ضم أرباحها
   لدفعة صرف؟ لو لأ، ده ممكن يكون الفجوة الحقيقية المقصودة في نص §20 الأصلي (نقطة 8: "الشكاوى/
   النزاعات — فلوس متنازع عليها لازم تفضل قابلة للتتبع، مش تختفي") — لاحظ إن ده **سؤال مختلف** عن
   نقطة 7 اللي اتقفلت في §20.7 (اللي كانت عن استرداد *بعد* الصرف، مش نزاع *وقت* الصرف).
2. اتّبع نفس الانضباط المتبع في §20.1-7: تحقيق كود حقيقي أول (مش افتراض) → إصلاح لو فيه بَقّة حقيقية
