...
في `apps/admin`. **فجوة موثّقة صراحة (نفس فجوة §36.2/§36.3)**: التحقق الحي الكامل بمتصفح حقيقي
(Playwright) ما اتعملش في السيشن دي — اتحاول تحقق مباشر عبر HTTP (curl + JWT حقيقي بنفس سر `.env`
الفعلي، مستخدم موجود فعلاً بالداتابيز) لكن اتوقّف على رفض `JwtStrategy` غير مفسَّر بالكامل (السبب
الدقيق محتاج تحقيق منفصل — خارج نطاق §36.4، مش سبب يوقف تسليم الميزة نفسها بما إن منطق SQL/الخدمة
مُثبَت بالكامل بالاختبارات الحية).
