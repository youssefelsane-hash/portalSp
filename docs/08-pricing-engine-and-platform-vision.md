...
**لسه من غير** (موثّق صراحة، مش سهو): واجهة العميل/الفني البسيطة (§14-16 من طلب المالك)، توسيع الملخص
المالي في الأدمن ليشمل محاولات دفع الزيادة (§17)، إشعار العميل الفعلي (الحدث `AdditionalWorkPaymentResolvedEvent`
مبثوث بالفعل، بس صفر listener/إعداد إشعار جديد اتوصل بيه لسه — نفس نمط الحدث-بلا-مستهلك الموثّق قبل
كده في feature-flags، مش خطأ، فرصة توصيل بسيطة لاحقًا)، endpoint اختيار وسيلة الدفع الافتراضية وقت
الدفع الأول نفسه (حاليًا أول كارت محفوظ = افتراضي تلقائيًا، تغييره بعد كده عبر `PATCH
/payment-methods/:id/default` الموجود).
