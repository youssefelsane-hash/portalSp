...
- Migration `0069_order_team_productivity.sql`: `orders.standard_data_id`/`required_technicians`/`required_assistants`/`estimated_duration_days` (كلهم NULL لو الطلب ملوش بيانات قياسية — مش كل خدمة عندها `ServiceStandardData`).
...
**نطاق مؤجَّل صراحة (مش سهو)**: حل يدوي إداري بعد التصعيد (إشعار بس دلوقتي، مش endpoint تعيين يدوي جديد)، إعادة بث فوري عند رفض قبل انتهاء المهلة، فحص تعارض جدولة صريح للمساعد (اكتفينا بفحص "مفيش طلب نشط" نفس دقة الفني القائد). واجهة `apps/technician-app` (شاشة "فرص المساعدة" قبول/رفض) اتبنت ومتوصّلة؛ الفحص كان عبر تحليل توازن الأقواس بدل `flutter analyze` (Flutter SDK لسه مش متاح في بيئة السيشن دي).
