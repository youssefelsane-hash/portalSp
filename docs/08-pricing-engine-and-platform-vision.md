...
**التفسير الدقيق** (المطلوب مختلف عن أي صياغة سابقة غامضة في سيشنز فاتت عن "30 يوم مقابل 60
يوم" — النص ده أوضح بكتير): وقت ما فني جديد يعمل `login`/يتفعّل، النظام يعمل seed تلقائي لـ
`technician_schedule_slots` (نفس الجدول الموجود، `technician-schedule.service.ts`) بـ**"فاضي"**
(available) لـ60 يوم قدّام من تاريخ التفعيل — الفني بيقدر يعدّل عليه لاحقًا (يقفل أيام بعينها) لكن
مايبدأش من صفر. وبعدين **rolling window حقيقي**: كل يوم (أو أسبوع، التفاصيل تتحدد وقت البناء) يوم
جديد بيتضاف في الآخر بحيث الأفق يفضل ~60 يوم دايمًا (مش نافذة ثابتة بتخلص وتسيب الفني من غير أي
إتاحية بعد 60 يوم من التفعيل). **يحتاج**: (أ) hook على `technician_profiles` approval/أول login
يعمل seed الـ60 يوم، (ب) job مجدول (`@Cron` يومي، زي أنماط موجودة بالفعل في `notification-workflow-
reminder.service.ts`) يمدّد الأفق كل يوم، (ج) الحفاظ على أي تعديلات يدوية سابقة من الفني (اليوم
الجديد المضاف بس هو اللي "فاضي افتراضيًا"، مش إعادة كتابة الجدول كله).
