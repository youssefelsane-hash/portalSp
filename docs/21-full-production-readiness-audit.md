...
| 22 | Ratings & Reviews | Ratings | FIXED | `assertRatable` يفرض COMPLETED وملكية الطلب متحقق منها. اتصحح القيد القديم إلى `UNIQUE(order_id, rating_type)` حتى لا يمنع تقييم أحد الطرفين تقييم الآخر، واتضافت قائمة pending للعميل وعرض كل أبعاد التقييم للإدارة. | نعم — `pending-ratings.spec.ts` + عقد التقييم القائم | منع طرف من التقييم لو الطرف الآخر قيّم أولًا، وعدم ظهور الأبعاد التفصيلية للإدارة | اتصلحت في migration 0322 ومساري العميل/الإدارة | لا يوجد | commit 0322 |
...
| 26 | Admin RBAC | Admin/Security | VERIFIED | جزئي — راجعت `setRolePermissions()`/`assignRole()`/`isLastSuperAdmin()` بالقراءة الدقيقة (منع self-escalation بمقارنة صريحة ضد صلاحيات الفاعل الحالية + `roles.grant_unrestricted` bypass + `SecurityEventType.UNAUTHORIZED_PERMISSION_GRANT` تسجيل محاولة، قفل `pessimistic_write`)، مفيش استغلال حي جديد اتبنى | نعم — تغطية عميقة جدًا موجودة بالفعل من Script5 (12 مرحلة مخصصة لـRBAC/الجلسات/الأمان) + P0-1/P0-2/P0-3 + `mfa-step-up-enforcement.spec.ts` (createRole/updateRole/cloneRole/deleteRole/setRolePermissions/assignRole كلهم step-up=true) | لا يوجد | لا يوجد | مفيش رصد جديد — الموديول ده اتغطى بعمق استثنائي في سيشنز سابقة (Script5 كامل)، هذا التحقق تأكيدي بس مش إعادة بناء من الصفر | commit سابق + هذا التحقق |
...
| 32 | UI/UX Quality | Customer-app | VERIFIED | جزئي — Script3 (13 مرحلة، design system + كل شاشات الحجز) + Script6 (12 مرحلة، إصلاح UX حقيقي: quick-booking، اختيار الفني، عرض الكتالوج) اتعملوا بالكامل في سيشنز سابقة بمراجعة بصرية حقيقية بمتصفح (Xvfb+fluxbox موثّقة في `apps/customer-app/README.md`)، مفيش استغلال جديد اتبنى هنا | نعم — `test_live/` حي كامل + منهجية render حقيقي موثّقة | لا يوجد | لا يوجد | مفيش رصد جديد | commit سابق |
| 33 | Error Handling / Observability | Cross-cutting | VERIFIED | جزئي — راجعت `ApiException`/`ErrorCode` (كود خطأ موحّد + رسالة عربية واضحة لكل استثناء عبر المنصة كلها، اتأكد من استخدامه المتّسق في كل الملفات اللي اتراجعت طول الـaudit ده)، `AuditLogService.record()`/`SecurityEventsService.recordDenial()` كلاهما best-effort (`try/catch` شامل، فشل التسجيل مايكسرش العملية الأصلية أبداً — نفس الفلسفة اتفحصت ومتّسقة عبر كل الموديولات)، فشل queue/cache بيتلقط ويرجع null/fallback مش يعلّق الطلب (الدرس الموثّق من أول الـaudit ده) | نعم — تغطية موجودة بالفعل عبر كل ملفات الـspec اللي بتتحقق من أكواد الأخطاء الصريحة | لا يوجد | لا يوجد | مفيش رصد جديد — النمط متّسق فعليًا عبر كل الموديولات اللي اتراجعت (15 بَقّة في الـaudit ده كلها استخدمت نفس نمط `ApiException`/`ErrorCode` بلا استثناء) | commit سابق + هذا التحقق |
| 34 | Full Golden-Path Live Test | Cross-cutting | VERIFIED | نعم — `golden-path-cash-booking.e2e.spec.ts` حي كامل ضد Postgres حقيقي: إنشاء (OrdersService.create) → توزيع (محاكى) → قبول → دورة تنفيذ الفني الكاملة (depart/arrive/start — فحص صورة إجباري حقيقي رجّع ORDR_005 قبل ما الصورة تتضاف) → complete → collectCash → settleAndComplete (عمولة/أرباح/ضمان اتفحصوا بالأرقام بالظبط) → قيد محفظة مزدوج حقيقي (COMMISSION_DEDUCTION، delta محفظة المنصة) → تقييم عميل (+ رفض تقييم تاني) → `order_status_history` كامل بالترتيب الصح. أول اختبار regression دائم لرحلة كاملة من الإنشاء للتقييم في المشروع كله (سابقًا كان بس اختبار حي يدوي متكرر بلا أثر دائم) | نعم — اختبار جديد (`golden-path-cash-booking.e2e.spec.ts`، اختبار واحد شامل، 10 تشغيلات متتالية مستقرة) | لا يوجد | لا يوجد | مفيش رصد جديد أثناء بناء الاختبار — كل خطوة اشتغلت زي المتوقع من أول محاولة (بعد تصحيح أخطاء fixture بسيطة: enum values، ترتيب DI parameters، حالة ACCEPTED الوسيطة في state machine) | commit قادم |
| 35 | Financial Reconciliation Test | Wallets/Payments | VERIFIED | نعم — `financial-reconciliation.e2e.spec.ts` حي كامل ضد Postgres حقيقي: سلسلة عمليات مالية متتابعة على نفس المحافظ (`collectCash` → `refundOrder` جزئي → محاكاة أرباح سابقة عبر `doubleEntry` → `requestPayout` → `adminComplete`)، بيثبت مباشرة إن القيد المزدوج مقفول (net الحركات المرتبطة بالطلب = صفر بالظبط) وإن رصيد كل محفظة المخزّن (`balance_cents`) مطابق تمامًا لمجموع دفتر حركاتها الفعلي (`wallet_transactions`) — لمحفظتي العميل والفني (مستخدمين جداد اتنشأوا في الاختبار، تاريخهم كامل هنا فمطابقة مطلقة سليمة)، ودلتا محفظة المنصة المشتركة (قبل/بعد الاسترداد، مش قيمة مطلقة — القاعدة الحاكمة الموثّقة فوق الاختبار نفسه: قاعدة البيانات المشتركة فيها انحراف تاريخي تراكمي من شهور اختبارات سابقة مش مصدر موثوق للتصفير المطلق) | نعم — اختبار جديد (`financial-reconciliation.e2e.spec.ts`، اختبار واحد شامل، 3 تشغيلات متتالية مستقرة + full suite أخضر) | لا يوجد بَقّة جديدة — الهدف من المرحلة إثبات إن الـinvariant قايم فعليًا (زي ما هو متوقع من P0-7/P0-8 والإصلاحات المالية السابقة في هذا الـaudit)، مش بحث عن بَقّة | لا يوجد | فحصت (أثناء بناء الاختبار) إن محاولة reconciliation على قاعدة البيانات المشتركة كلها (مش نطاق الاختبار المعزول) بترجع فرق ضخم (`total_debits=76178000` مقابل `total_credits=32008000`) — شخّصته كضجيج تراكمي من شهور اختبارات على نفس الداتابيز المشتركة مش عيب حي فعلي (سبب مشابه للانحراف الملحوظ في محفظة المنصة تحديدًا)؛ مفيش أداة reconciliation تشغيلية دورية للإنتاج الحقيقي حاليًا — لو الأمر جد لازم job مجدول يقارن `balance_cents` بكل محفظة ضد `SUM(wallet_transactions)` بتاعتها ويرفع alert عند أي انحراف، خارج نطاق هذا الـaudit (فجوة موثّقة) | commit قادم |
