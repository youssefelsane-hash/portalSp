...
**الحالة:** جزئيًا مستبدَل بـ ADR-0031 (2026-08-21) — **Slice B (`TechniciansService.listForServiceBooking()`
العام) يفضل قايم بلا تغيير ومطابق تمامًا لاتجاه ADR-0031 الجديد (مزوّد واحد فقط)**، ده أصلاً
`Service`-level عام مش خاص بالشغالة. **Slice A/C (فحص التعارض/التصفّح الخاصين بحجز الشغالة القديم)
مستبدَلين** — هيتلغوا مع بنية `DomesticWorkersModule` بالكامل (ADR-0031 Slice E)؛ منطق الفحص بدقة
الساعة نفسه (`findSchedulingConflict()`) هيتعمّم لأي فني عادي بدل ما يتلغي (ADR-0031 §3). Slice D
(واجهة Flutter) الجزء الخاص بالفني (`TechnicianMarketplaceScreen`) يفضل قايم، الجزء الخاص بتصفّح
الشغالة (`DomesticWorkersScreen`) هيتلغي مع Slice F.
**التاريخ:** 2026-08-21
