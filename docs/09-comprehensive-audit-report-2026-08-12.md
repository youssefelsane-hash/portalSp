...
| 4 | **بروفايل الفني العام — حقول ناقصة في الموبايل** | ✅ صحيح | `TechnicianPublicProfile` (موديل Flutter) معندهوش `certificates`/`avgArrivalMinutes`/`avgCompletionMinutes` — الـ API بيرجّعهم، الموديل مش بيقرأهم. |
