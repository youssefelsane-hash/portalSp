...
| `AUTH_PIN_PEPPER` | ≥32 حرف عشوائي، وممنوع الافتراضية. **مايتغيّرش بعد التشغيل** — هو السر الخارجي المستخدم لتصليب رموز الدخول قبل bcrypt |
...
## ٣-ب. الدخول الحالي — رقم الموبايل + رمز دخول

- إنشاء الحساب متاح لكل الأرقام؛ لا توجد whitelist ولا `OTP_TEST_MODE` في رحلة الدخول الحالية.
- أي رمز جديد ستة أرقام بالضبط، مع رفض الأرقام المتكررة والتسلسلات السهلة.
- بعد خمس محاولات خاطئة يبدأ قفل تدريجي (دقيقة، ثم ٥، ثم ١٥، ثم ساعة، ثم ٤ ساعات).
- الرموز القديمة ذات ٤–٥ أرقام تظل صالحة للدخول مؤقتًا، وتُعاد حمايتها بالـpepper تلقائيًا بعد
  أول دخول ناجح. أي تغيير أو استرجاع لاحق يفرض ستة أرقام.
- `OTP_TEST_MODE` لازم يظل `false` أو محذوفًا في الإنتاج. بوابة SMS اختيارية للإشعارات فقط،
  وليست جزءًا من تسجيل الدخول.
- فتح التسجيل بلا SMS قرار منتج مقصود، لكنه يعني أن أول تسجيل لا يثبت ملكية رقم الهاتف. لو
  أصبح إثبات الملكية مطلوبًا لاحقًا، يلزم SMS/WhatsApp أو وسيلة تحقق مستقلة؛ لا يمكن للـPIN وحده
  إثبات أن الرقم يخص الشخص الذي كتبه.
