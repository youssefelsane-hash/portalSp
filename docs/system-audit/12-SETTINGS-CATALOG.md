...
> **مُولَّد من جدول `settings` الحيّ في `baytak_main`** — 180 مفتاحًا في 26 مجموعة.
...
### أ) `orders.auto_cancel_after_minutes` — **موجود وغير مستخدَم**

القيمة `20` موجودة في المجموعة `limits`، لكن **مفيش كود بيقراها**. قرار مالك صريح: طلب
`searching_technician` **مايتلغيش تلقائيًا خالص** مهما طالت المدة.

اللي بيتلغى تلقائيًا هو `pending_payment` بس، بمهلة `orders.payment_timeout_minutes` (١٥ دقيقة).
...
| المفتاح | النوع | القيمة | الرؤية | الوصف |
|---|---|---|---|---|
| `orders.auto_cancel_after_minutes` | number | `20` | داخلي | إلغاء تلقائي لو مفيش فني قبل |
