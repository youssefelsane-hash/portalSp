...
### 3.4 رسايل الإدارة للعميل — `order_customer_notices` (ADR-0071)

الأدمن لما بيطلب تفاصيل إضافية أو بيحوّل الطلب لمعاينة موقعية، بيكتب نصًا **للعميل**. النص ده
بقى له **مكان دايم على الطلب نفسه**، مش في الإشعار بس.
...
**السبب الجذري اللي اتصلح**: النص كان بيتسجّل في `audit_logs` (مسار أدمن) و
`order_status_history.reason` (مش معروض للعميل) وفي حدث الإشعار. والإشعار **عابر** — بيتقري
مرة وبيروح، وممكن يتمسح، وممكن أصلًا مايوصلش (إذن مرفوض، جهاز متبدّل).

> النتيجة كانت: **نص كتبه الأدمن علشان العميل، ومالوش أي مكان دايم يقراه العميل منه.** مش
> خطأ عرض — الحقل نفسه مكانش موجود.

**الإشعار مالوش أي تغيير** — بقى إخطار بالرسالة، مش المكان الوحيد اللي فيها.
`created_by_user_id` بيتسجّل للتدقيق لكنه **مايتعرضش للعميل** (خصوصية الموظفين).

---

### 3.5 فرق دقيق: ثلاث أنواع إلغاء
...
| `ENGAGED_TECHNICIAN_ORDER_STATUSES` | نفسها **بدون** `accepted` | **مابقتش تحكم أي استبعاد** (ADR-0070) |
...
**تحديث ADR-0070 (2026-09-04)**: `ENGAGED` كانت بتستبعد الفني «الشغّال دلوقتي» من الطوارئ
ومن طلبات نفس اليوم. القاعدة **اتشالت بالكامل** بطلب مالك صريح — الحارسان الباقيان هما تقاطع
النافذة الحقيقي والسقف اليومي (اللي بقى يسري على الطوارئ كمان). المجموعة نفسها لسه معرَّفة
لكنها مابقتش تحكم أي استبعاد. التفصيل في
