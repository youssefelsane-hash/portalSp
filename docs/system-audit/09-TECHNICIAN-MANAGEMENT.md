...
`LIGHT` (تأكيد تلقائي) · `MEANINGFUL` (محتاج قبول) · `HEAVY` (السقف اتخطى) · `BLOCKED` (إجازة).

> **تحديث ADR-0070**: الفني «الشغّال دلوقتي» بقى `MEANINGFUL` مش `HEAVY` — بيقول للأدمن «عنده
> شغل» من غير ما يدّعي إنه مستبعد، ومتفق مع محرك الأهلية اللي بيعتبره مؤهّلًا.
