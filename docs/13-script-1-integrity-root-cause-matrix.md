...
| 42 | Retried admin wallet adjustment repeated money; reproduced. A separate review also reproduced the risk of a committed adjustment with no audit row. | Durable source claim, ledger links, and mandatory audit row now share one transaction; same key/same payload returns original, changed payload returns `409`. | Duplicate-key, failure-after-ledger rollback/retry, audit-failure rollback/retry, wallet-create, and ledger reversal PostgreSQL cases. | `0116` `wallet_adjustments`; required `Idempotency-Key`. | None for local durability. | `VERIFIED FIXED` |
...
| 44 | Reject could overwrite a compensated resolve; reproduced. Audit failure after compensation was also a permanent-trail risk. | Resolve and reject use the same complaint lock/state machine; compensation, decision, and mandatory audit commit together. Any injected wallet or audit failure rolls everything back and retry creates one effect and one audit. | Resolve-times-reject, wallet-effect failure/retry, and audit-failure rollback/retry PostgreSQL cases. | No migration. | None for local durability. | `VERIFIED FIXED` |
...
| 75 | Final regression, cleanup, checksums, handles, and secret hygiene required independent revalidation. | Bounded full suite, `detectOpenHandles`, builds/typecheck, migrations, invariants, diff/secret checks, and remote verification form the completion gate. | Final totals are recorded in `docs/14-script-1-final-financial-proof.md` after the last run. | Migrations through `0121`; no `.env` tracked. | Lint executable and admin Next dependencies are absent and reported, not marked passed. | `VERIFIED FIXED` subject to stated tooling limits. |

## Independent final-review additions — 2026-08-17

| Finding | Reproduction and root cause | Final implementation | Proof / migration | Remaining limitation | Final status |
| --- | --- | --- | --- | --- | --- |
| Active technician while awaiting quote approval | Reproduced conceptually and at the database boundary: `AWAITING_QUOTE_APPROVAL` is entered directly from `IN_PROGRESS` and returns to it, but the shared active set and `0118` index omitted it. A second assignment could therefore acquire the same technician. No other omitted status always owns the field resource: `WORK_COMPLETED`/`AWAITING_PAYMENT` release it, `TECHNICIAN_ASSIGNED` is only an offer, and `DISPUTED` is polymorphic. | Added the status to `ACTIVE_TECHNICIAN_ORDER_STATUSES`, so matching, active-order reads, assistant policy, and `TechnicianAssignmentGuardService` share the correction. | `0121` safely replaces the partial unique index. `matching-accept-concurrency.spec.ts` proves acceptance, admin reassignment, and direct SQL all reject Order B while Order A awaits quote approval, and proves `IN_PROGRESS ↔ AWAITING_QUOTE_APPROVAL` remains valid. | Production preflight must resolve any legacy duplicate before applying, by design. | `VERIFIED FIXED` |
| Audit durability after a sensitive commit | Fault injection reproduced the former window: business/ledger transaction could commit, standalone `AuditLogService.record()` could fail silently, and an idempotent retry could return the existing operation without recreating audit. | Sensitive Script 1 operations choose policy A: audit uses the same `EntityManager`, propagates failure, and rolls back with the business operation. This covers wallet adjustments, payment/refund decisions, payouts, complaint decisions, standard/QR referral reward and revoke, domestic earning decisions, promo create/deactivate, manual loyalty credit, admin order cancel/reassign/price, technician cancellation penalties, and domestic-worker review. Standalone best-effort audit remains only for non-transactional legacy/non-financial paths. | Real PostgreSQL fault tests prove rollback then exactly-once retry for wallet adjustment, complaint compensation, payout rejection, standard referral promo reward, and technician referral wallet reward. Existing refund, payout, earning, order, and concurrency suites prove neighboring paths. No schema migration is needed because `audit_logs` already exists. | Notification/WebSocket delivery is not an audit record and remains best effort except the separately resumable webhook payment effects. | `VERIFIED FIXED` |
| CI parallel isolation and Jest shutdown | Reproduced the exact CI command. Multiple suites derived short shared identifiers from `Date.now()`, cleanup attempted empty UUIDs after setup failure, recurring-order mocks could sweep each other's temporarily active templates, and the technician-referral recovery interval kept a worker lifecycle ambiguous. These produced unique/FK failures and Jest force-exit despite serial success. | Fixtures now use UUID namespaces or the seeded country, teardown is guarded and destroys the `DataSource` in `finally`, recurring-order integration windows use a shared PostgreSQL advisory test lock, and the recovery interval is async, cleared on destroy, and `unref()`'d. Its unit test injects a tracked timer handle instead of leaking Jest fake-timer state. | Exact CI-style `npm test`: 61 suites / 362 tests passed in parallel and exited without force-exit. Full `--runInBand --detectOpenHandles`: the same 61 / 362 passed with no reported handles. The originally failed matching/proof/recurring suites also pass independently against PostgreSQL. | Test/lifecycle only; no production schema impact. | Lint cannot run because the repository declares but does not install `eslint`. | `VERIFIED FIXED` |
